...
function normalizeFacebookDirectUrls(values = [], limit = 20) {
  const out = [];
  const seen = new Set();
  for (const value of Array.isArray(values) ? values : []) {
    const url = normalizeFacebookDedupeUrl(value);
    if (!url || !isConcreteFacebookUrl(url) || seen.has(url)) continue;
    seen.add(url);
    out.push({
      url,
      originalUrl: normalizeFacebookUrl(value) || url,
      evidenceKind: facebookEvidenceKind(url),
      dedupeKey: url,
    });
...
function parseFacebookDirectPostPage(html = "", direct = {}, keyword = "") {
  const url = normalizeFacebookDedupeUrl(direct.url || direct.originalUrl || "");
  if (!url || !isConcreteFacebookUrl(url)) return null;
  const evidenceKind = direct.evidenceKind || facebookEvidenceKind(url);
  const title = extractMetaContent(html, ["og:title", "twitter:title"])
    || extractTitle(html)
    || `Facebook ${evidenceKind || "post"} ${url}`;
  ...
  const item = {
    ...
    title,
    content,
    author: extractMetaContent(html, ["article:author", "author"]) || "Facebook 直达公开帖",
    publishedAt,
    rawHtml: String(html || ""),
    metrics: {
      ...
      direct_url: url,
      facebook_direct_url: url,
      facebook_original_direct_url: direct.originalUrl || direct.url || url,
      facebook_direct_url_signal: 1,
      facebook_evidence_kind: evidenceKind,
...
async function insertFacebookItems(items, { keyword, proxyUrl = "", enrich = true, maxDeepPages = 0, domainControls = {}, contentControls = {}, seenItemUrls = null }) {
  ...
  for (const item of items) {
    ...
    const result = insertSentimentItem({
      ...
      evidence: {
        ...(enriched.evidence || {}),
        evidence_type: enriched.evidence?.evidence_type || (finalMetrics.collection_mode === "facebook_direct_url" ? "facebook_direct_post" : "facebook_public_search_result"),
        metrics: {
          ...finalMetrics,
...
export async function scrapeFacebookSearch(keywords, { proxyUrl = "", enrich = true, budget = {}, deepBudget = null, since = "", domainControls = {}, contentControls = {}, directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeFacebookDirectUrls(directUrls);
  ...
  for (const keyword of (normalizedKeywords.length ? normalizedKeywords : [""])) {
    ...
    for (const direct of normalizedDirectUrls) {
      ...
      try {
        ...
        const item = parseFacebookDirectPostPage(html, direct, keyword);
        if (!item || !isAfterSince(item.publishedAt, since)) continue;
        const count = await insertFacebookItems([item], {
          keyword,
          proxyUrl,
          enrich: false,
          maxDeepPages: 0,
        ...
        console.warn(`[Sentiment/Facebook] 直达公开帖抓取失敗 url=${direct.url}: ${message}`);
      }
    }
...
export const __test__ = {
  ...
  normalizeFacebookDirectUrls,
  normalizeFacebookUrl,
  parseFacebookDirectPostPage,

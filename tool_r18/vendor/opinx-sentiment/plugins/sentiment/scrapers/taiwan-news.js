...
const TAIWAN_NEWS_DIRECT_HOSTS = [
  "ettoday.net",
  "nownews.com",
  "tw.news.yahoo.com",
  "udn.com",
...
function normalizeTaiwanNewsHost(value = "") {
  return String(value || "").toLowerCase().replace(/^(www|m)\./, "");
}

function taiwanNewsDirectHostAllowed(host = "") {
  const normalized = normalizeTaiwanNewsHost(host);
  return TAIWAN_NEWS_DIRECT_HOSTS.some(pattern => normalized === pattern || normalized.endsWith(`.${pattern}`));
}

function normalizeTaiwanNewsDirectUrl(value = "") {
  const raw = decodeHtml(value || "").trim();
  if (!raw) return "";
  try {
    const url = new URL(raw.startsWith("//") ? `https:${raw}` : raw);
    if (!["http:", "https:"].includes(url.protocol)) return "";
    if (!taiwanNewsDirectHostAllowed(url.hostname)) return "";
    url.hash = "";
    for (const key of ["utm_source", "utm_medium", "utm_campaign", "utm_content", "utm_term", "fbclid", "gclid", "ocid", "ref", "from"]) {
      url.searchParams.delete(key);
    }
    return url.toString();
  } catch {
...
function normalizeTaiwanNewsDirectUrls(values = [], limit = 40) {
  ...
  for (const value of raw) {
    const url = normalizeTaiwanNewsDirectUrl(value);
    if (!url) continue;
    const dedupe = normalizeTaiwanNewsDedupeUrl(url).toLowerCase();
    if (!dedupe || seen.has(dedupe)) continue;
    seen.add(dedupe);
    out.push(url);
    if (out.length >= Math.max(1, Math.min(120, Number(limit) || 40))) break;
  }
  return out;
}

function inferTaiwanNewsDirectSourceName(url = "") {
  const host = normalizeTaiwanNewsHost(hostFromUrl(url));
  const map = [
  ...
  return map.find(([pattern]) => host === pattern || host.endsWith(`.${pattern}`))?.[1] || host || "Taiwan News";
}
...
function titleFromTaiwanNewsUrl(value = "") {
  try {
    const url = new URL(String(value || ""));
    const segments = decodeURIComponent(url.pathname || "")
      .split("/")
      .map(segment => segment.replace(/\.[a-z0-9]{2,5}$/i, ""))
      .map(segment => segment.replace(/[_+.-]+/g, " ").replace(/\s+/g, " ").trim())
      .filter(Boolean);
    return stripTags(segments.slice(-2).join(" "), 180) || inferTaiwanNewsDirectSourceName(value);
  } catch {
    return inferTaiwanNewsDirectSourceName(value);
  }
}

function titleFromHtml(html = "") {
  const source = String(html || "");
  const og = source.match(/<meta[^>]+(?:property|name)=["'](?:og:title|twitter:title)["'][^>]+content=["']([^"']+)["'][^>]*>/i)
    || source.match(/<meta[^>]+content=["']([^"']+)["'][^>]+(?:property|name)=["'](?:og:title|twitter:title)["'][^>]*>/i);
  if (og?.[1]) return stripTags(og[1], 220);
  const title = source.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
  return title?.[1] ? stripTags(title[1], 220) : "";
}
...
async function insertNewsItem(item, { keyword, proxyUrl, enrich, rawItemCount = 0, domainControls = {}, contentControls = {}, failoverAttribution = [], seenItemUrls = null }) {
  ...
  const displayTitle = item.title || titleFromHtml(enriched.raw_html || "") || titleFromTaiwanNewsUrl(item.url || "");
  const sentiment = analyzeSentiment(`${displayTitle} ${content}`);
  const scoredItem = { ...item, title: displayTitle, content, author: enriched.author || item.author };
  ...
  const result = insertSentimentItem({
    ...
    evidence: evidenceWithFailover({
      ...
      metrics: {
        ...
        taiwan_news_direct_url: item.direct_url || "",
        taiwan_news_direct_url_recovery: Boolean(item.direct_url),
        ...narrativeSignals,
      },
    }, failoverAttribution),
    visual_assets: [
...
export async function scrapeTaiwanNewsFeeds(keywords, { proxyUrl = "", enrich = false, budget = {}, since = "", domainControls = {}, contentControls = {}, failoverAttribution = [], directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeTaiwanNewsDirectUrls(directUrls, 40);
  ...
  const directKeyword = normalizedKeywords[0] || "taiwan-news-direct-url";
  for (const url of normalizedDirectUrls) {
    try {
      directInserted += await insertNewsItem({
        title: "",
        url,
        content: "",
        publishedAt: new Date().toISOString(),
        author: inferTaiwanNewsDirectSourceName(url),
        feed_name: `${inferTaiwanNewsDirectSourceName(url)} 原文直抓`,
        feed_url: url,
        source_name: inferTaiwanNewsDirectSourceName(url),
        source_family: normalizeTaiwanNewsHost(hostFromUrl(url)).includes("cnyes")
          || normalizeTaiwanNewsHost(hostFromUrl(url)).includes("moneydj")
          || normalizeTaiwanNewsHost(hostFromUrl(url)).includes("businessweekly")
          || normalizeTaiwanNewsHost(hostFromUrl(url)).includes("cw.com.tw")
          || normalizeTaiwanNewsHost(hostFromUrl(url)).includes("businesstoday")
          || normalizeTaiwanNewsHost(hostFromUrl(url)).includes("wealth.com.tw")
            ? "taiwan_business_media"
            : "taiwan_media",
        feed_item_format: "direct-url",
        direct_url: url,
        metrics: {
          taiwan_news_direct_url: url,
        },
      }, { keyword: directKeyword, proxyUrl, enrich: true, rawItemCount: normalizedDirectUrls.length, domainControls, contentControls, failoverAttribution, seenItemUrls });
    } catch (err) {
      directFailures.push({ target: url, message: formatSourceError(err, proxyUrl) });
    }
  }
...
export const __test__ = {
  ...
  budgetItemsPerFeed,
  listTaiwanNewsFeeds,
  normalizeTaiwanNewsDirectUrl,
  normalizeTaiwanNewsDirectUrls,

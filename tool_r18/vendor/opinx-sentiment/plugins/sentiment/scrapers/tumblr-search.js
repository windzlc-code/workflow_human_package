...
function normalizeTumblrDirectUrls(values = [], limit = 20) {
  ...
  for (const value of raw) {
    const normalized = normalizeTumblrUrl(value);
    if (!normalized || !isConcreteTumblrUrl(normalized)) continue;
    const dedupe = normalizeTumblrDedupeUrl(normalized);
...
function tumblrTitleFromUrl(url = "") {
  try {
    const parsed = new URL(url);
    const segments = parsed.pathname.split("/").filter(Boolean);
    const numericIndex = segments.findIndex(segment => /^\d{4,}$/.test(segment));
    const slug = numericIndex >= 0 ? segments[numericIndex + 1] || "" : segments[segments.length - 1] || "";
    const title = stripTags(String(slug || "")
      .replace(/[-_]+/g, " ")
      .replace(/\s+/g, " ")
      .trim(), 240);
    return title || "Tumblr post";
  } catch {
    return "Tumblr post";
  }
}

function tumblrTitleFromHtml(html = "", fallback = "") {
...
async function insertTumblrItems(items, { keyword, proxyUrl = "", enrich = true, maxDeepPages = 0, domainControls = {}, contentControls = {}, seenItemUrls = null, directCollector = false }) {
  let inserted = 0;
  let deepPagesUsed = 0;
  for (const item of items) {
    ...
    const title = tumblrTitleFromHtml(enriched.raw_html || "", item.title) || item.title;
    ...
    const result = insertSentimentItem({
      ...
      keyword,
      keywords: [keyword, ...tumblrKeywordNeedles(keyword)],
      published_at: enriched.published_at || item.publishedAt,
      ai_summary: enriched.ai_summary || content,
      raw_html: enriched.raw_html || "",
      evidence: {
        ...(enriched.evidence || {}),
        evidence_type: item.evidenceType || enriched.evidence?.evidence_type || "tumblr_public_post_search_result",
        source_key: "tumblrSearch",
        metrics: {
          ...finalMetrics,
...
export async function scrapeTumblrSearch(keywords, { proxyUrl = "", enrich = true, budget = {}, deepBudget = null, since = "", domainControls = {}, contentControls = {}, directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeTumblrDirectUrls(directUrls, 20);
  ...
  if (normalizedDirectUrls.length) {
    const directKeyword = normalizedKeywords[0] || "tumblr-direct-url";
    const directItems = normalizedDirectUrls.map(url => ({
      url,
      title: tumblrTitleFromUrl(url),
      content: "",
      author: "Tumblr 直達帖文",
      publishedAt: new Date().toISOString(),
      evidenceType: "tumblr_direct_post",
      metrics: {
        source_key: "tumblrSearch",
        source_family: "social",
        source_kind: "tumblr_direct_url",
        collection_mode: "tumblr_direct_url",
        deep_collector: "tumblr-direct-url",
        source: "tumblr_direct_post",
        direct_url: url,
        tumblr_direct_url: url,
        tumblr_evidence_kind: tumblrEvidenceKind(url),
      },
    }));
    try {
      inserted += await insertTumblrItems(directItems, {
      ...
      failures.push({ keyword: directKeyword, target: "tumblr-direct-url", message });
      console.warn(`[Sentiment/Tumblr] 直達帖文抓取失敗 keyword=${directKeyword}: ${message}`);
    }
  }

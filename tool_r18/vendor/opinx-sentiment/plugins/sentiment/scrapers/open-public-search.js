...
const PUBLIC_SEARCH_ENGINES = ["duckduckgo", "bing_rss", "mojeek", "wiby"];
...
const GDELT_QUERY_PROFILES = [
  ...
  { key: "middle_east_ar_en", region: "middle_east", language: "ar", terms: ["Middle East", "Gulf", "UAE", "Saudi Arabia", "Qatar", "Kuwait", "مصر", "السعودية", "الإمارات", "الخليج"] },
  { key: "africa_en", region: "africa", language: "en", terms: ["Africa", "South Africa", "Nigeria", "Kenya", "Egypt", "Ghana", "Ethiopia"] },
  { key: "turkey_tr", region: "turkey", language: "tr", terms: ["Turkey", "Türkiye", "Istanbul", "Ankara"] },
  { key: "eastern_europe_en", region: "eastern_europe", language: "en", terms: ["Eastern Europe", "Poland", "Ukraine", "Romania", "Czech Republic", "Hungary"] },
...
const GDELT_PROFILE_COUNTRY_HINTS = new Map([
  ...
  ["middle_east", ["AE", "SA", "QA", "KW", "BH", "OM", "EG", "JO", "IL", "IR", "IQ", "LB", "SY", "YE"]],
  ["africa", ["ZA", "SF", "NI", "NG", "KE", "EG", "ET", "GH", "TZ", "UG", "MA", "MO", "DZ", "AG", "TN"]],
  ["turkey", ["TR", "TU"]],
  ["eastern_europe", ["PL", "UP", "UA", "RO", "EZ", "CZ", "HU", "BU", "BG", "LO", "SK", "HR", "SI", "LH", "LT", "LG", "LV", "EN", "EE"]],
...
function buildPublicSearchTargets(keyword = "", {
  ...
  for (const profile of activeProfiles) {
    ...
    if (activeEngines.includes("mojeek")) {
      ...
      targets.push({
        ...
        query,
        page,
        url: `https://www.mojeek.com/search?${params.toString()}`,
      });
    }
    if (activeEngines.includes("wiby")) {
      const params = new URLSearchParams({ q: query });
      ...
      targets.push({
        ...
        query,
        page,
        url: `https://wiby.me/?${params.toString()}`,
      });
    }
  }
  return targets;
...
function countMojeekRawResults(html = "") {
  return [...String(html || "").matchAll(/<!--rs-->[\s\S]*?<li[^>]+class="[^"]*r\d+[^"]*"[\s\S]*?<!--re-->/gi)].length;
}

function countWibyRawResults(html = "") {
...
function parseMojeekResults(html, keyword, limit = 10, { profile = PUBLIC_SEARCH_PROFILES[0], query = "" } = {}) {
  const source = String(html || "");
  ...
  for (const block of blocks) {
    ...
    const content = stripTags((block.match(/<p[^>]+class="[^"]*s[^"]*"[^>]*>([\s\S]*?)<\/p>/i) || [])[1] || "");
    const candidate = { url, title, content, author: "Mojeek 公開搜尋" };
    if (!url || !title || !publicSearchMatchesKeyword(`${title} ${content}`, keyword)) continue;
    if (!publicSearchProfileAllows(candidate, profile)) continue;
    results.push({
      url,
      ...
      publishedAt: new Date().toISOString(),
      metrics: {
        ...publicSearchMetrics("mojeek", profile, query),
        public_search_canonical_dedupe_url: normalizePublicSearchDedupeUrl(url),
      },
    });
    if (results.length >= limit) break;
...
function parseWibyResults(html, keyword, limit = 10, { profile = PUBLIC_SEARCH_PROFILES[0], query = "" } = {}) {
  const source = String(html || "");
  ...
  for (const block of blocks) {
    ...
    const title = stripTags(titleLink?.[2] || "");
    const urlText = stripTags((block.match(/<p[^>]+class=["']url["'][^>]*>([\s\S]*?)<\/p>/i) || [])[1] || "");
    const content = stripTags(block
      .replace(/<a[^>]+class=["']tlink["'][\s\S]*?<\/a>/i, " ")
      .replace(/<p[^>]+class=["']url["'][\s\S]*?<\/p>/i, " "));
    const candidate = { url, title, content, author: "Wiby 公開搜尋" };
    if (!url || !title || !publicSearchMatchesKeyword(`${title} ${content} ${urlText}`, keyword)) continue;
    if (!publicSearchProfileAllows(candidate, profile)) continue;
    results.push({
      url,
      title,
      ...
      publishedAt: new Date().toISOString(),
      metrics: {
        ...publicSearchMetrics("wiby", profile, query),
        public_search_canonical_dedupe_url: normalizePublicSearchDedupeUrl(url),
      },
    });
    if (results.length >= limit) break;
    ...
    try {
      ...
      for (let page = 0; page < normalizedBudget.maxPagesPerKeyword && items.length < normalizedBudget.maxItemsPerKeyword; page += 1) {
        ...
        for (const target of buildPublicSearchTargets(keyword, { engines, profiles: profiles.map(profile => profile.key), page })) {
          ...
          if (target.engine === "mojeek" && items.length < normalizedBudget.maxItemsPerKeyword) {
            const res = await fetchPublicSource(target.url, {
              headers: {
                "User-Agent": USER_AGENT,
                "Accept-Language": target.profile.acceptLanguage || "en-US,en;q=0.9",
            ...
            if (!res.ok) {
              failures.push({ keyword, target: `mojeek:page:${page + 1}:${target.profile.key}`, engine: "mojeek", message: httpFailure(res) });
            } else {
              const html = await res.text();
              const rawCount = countMojeekRawResults(html);
              rawPageFound += rawCount;
              const pageItems = parseMojeekResults(
                html,
                keyword,
                normalizedBudget.maxItemsPerKeyword - items.length,
                { profile: target.profile, query: target.query },
          ...
          if (target.engine === "wiby" && items.length < normalizedBudget.maxItemsPerKeyword) {
            const res = await fetchPublicSource(target.url, {
              headers: {
                "User-Agent": USER_AGENT,
            ...
            if (!res.ok) {
              failures.push({ keyword, target: `wiby:page:${page + 1}:${target.profile.key}`, engine: "wiby", message: httpFailure(res) });
            } else {
              const html = await res.text();
              const rawCount = countWibyRawResults(html);
              rawPageFound += rawCount;
              const pageItems = parseWibyResults(
                html,
                keyword,
                normalizedBudget.maxItemsPerKeyword - items.length,
                { profile: target.profile, query: target.query },

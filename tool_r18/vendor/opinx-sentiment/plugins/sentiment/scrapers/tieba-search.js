...
function tiebaThreadIdFromUrl(value = "") {
  const normalized = normalizeTiebaUrl(value);
  if (!normalized) return "";
  try {
    const parsed = new URL(normalized);
    const segments = parsed.pathname.split("/").filter(Boolean);
    return segments[0] === "p" && /^\d{4,}$/.test(segments[1] || "") ? segments[1] : "";
  } catch {
    return "";
  }
}

function normalizeTiebaDirectUrls(values = [], limit = 20) {
  const rawValues = Array.isArray(values) ? values : [values];
  const out = [];
  const seen = new Set();
  for (const value of rawValues) {
    const url = normalizeTiebaUrl(typeof value === "string" ? value : value?.url || value?.href || "");
    if (!url || !isConcreteTiebaUrl(url)) continue;
    const dedupeKey = normalizeTiebaDedupeUrl(url);
    if (!dedupeKey || seen.has(dedupeKey)) continue;
    seen.add(dedupeKey);
    const threadId = tiebaThreadIdFromUrl(dedupeKey);
    out.push({
      url: dedupeKey,
      threadId,
      title: cleanText(typeof value === "object" ? value.title || value.label || "" : "", 240) || `百度貼吧帖子 ${threadId || dedupeKey}`,
      dedupeKey,
    });
...
async function insertTiebaItems(items, { keyword, proxyUrl = "", enrich = true, maxDeepPages = 0, domainControls = {}, contentControls = {}, seenItemUrls = null, forceEnrich = false }) {
  let inserted = 0;
  let deepPagesUsed = 0;
  for (const item of items) {
    ...
    const shouldEnrich = enrich && (forceEnrich || deepPagesUsed < maxDeepPages);
    ...
    if (shouldEnrich && !forceEnrich) deepPagesUsed += 1;
    const content = enriched.content || item.content || "";
    const title = cleanText(enriched.title || item.title, 360);
    ...
    const result = insertSentimentItem({
      ...
      evidence: {
        ...(enriched.evidence || {}),
        evidence_type: item.metrics?.tieba_direct_url ? "tieba_direct_thread" : enriched.evidence?.evidence_type || "tieba_public_search_result",
        metrics: {
          ...finalMetrics,
...
function buildTiebaDirectItems(directUrls = [], keyword = "") {
  return normalizeTiebaDirectUrls(directUrls).map(item => ({
    url: item.url,
    title: item.title,
    content: cleanText(`${keyword} 百度貼吧直達帖子 ${item.threadId || ""}`, 420),
    author: "百度貼吧直達帖子",
    metrics: {
      public_search_engine: "evidence_platform_pivot",
      source_kind: "tieba_direct_url",
      tieba_evidence_kind: "thread",
      collection_mode: "tieba_direct_url",
      deep_collector: "tieba-direct-url",
      direct_url: item.url,
      tieba_direct_url: item.url,
      tieba_thread_id: item.threadId,
...
export async function scrapeTiebaSearch(keywords, { proxyUrl = "", enrich = true, budget = {}, deepBudget = null, since = "", domainControls = {}, contentControls = {}, directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const directItems = buildTiebaDirectItems(directUrls, normalizedKeywords[0] || "");
  if (!normalizedKeywords.length && !directItems.length) return scraperResult(0);
  ...
  if (directItems.length) {
    const directKeyword = normalizedKeywords[0] || "tieba-direct-url";
    try {
      inserted += await insertTiebaItems(directItems, {
        keyword: directKeyword,
        proxyUrl,
        enrich: true,
        maxDeepPages: directItems.length,
        domainControls,
        contentControls,
        seenItemUrls,
        forceEnrich: true,
      ...
      failures.push({ keyword: directKeyword, message });
      console.warn(`[Sentiment/Tieba] 直達帖子抓取失敗 keyword=${directKeyword}: ${message}`);
    }
  }
...
export const __test__ = {
  isConcreteTiebaUrl,
  buildTiebaDirectItems,
  normalizeBudget,
  normalizeTiebaDirectUrls,

...
const DEFAULT_GITLAB_INSTANCES = ["https://gitlab.com"];
...
function lemmyPostDirectReferenceFromUrl(value = "") {
  const normalized = normalizeFederatedDedupeUrl(value);
  if (!normalized) return null;
  try {
    const parsed = new URL(normalized);
    const segments = parsed.pathname.split("/").filter(Boolean);
    const postIndex = segments.findIndex(segment => segment.toLowerCase() === "post");
    if (postIndex < 0 || !segments[postIndex + 1]) return null;
    const postId = cleanText(segments[postIndex + 1], 80);
    if (!/^\d+$/.test(postId)) return null;
    parsed.hash = "";
    parsed.search = "";
    parsed.pathname = `/post/${postId}`;
    const directUrl = normalizeFederatedDedupeUrl(parsed.toString());
    const instanceUrl = `${parsed.protocol}//${parsed.host}`;
    return {
      directUrl,
      instanceUrl,
      postId,
      apiUrl: `${instanceUrl}/api/v3/post?id=${encodeURIComponent(postId)}`,
    };
  } catch {
    return null;
  }
...
function normalizeLemmyDirectUrls(values = [], limit = 20) {
  ...
  for (const value of raw) {
    const reference = lemmyPostDirectReferenceFromUrl(value);
    if (!reference?.directUrl || seen.has(reference.directUrl)) continue;
    seen.add(reference.directUrl);
    out.push(reference);
...
function normalizeGitLabInstances(instances = DEFAULT_GITLAB_INSTANCES) {
  const raw = Array.isArray(instances)
    ? instances
    : typeof instances === "string"
      ? instances.split(/[,\n，、;；]+/)
  ...
  return out.length ? out : [...DEFAULT_GITLAB_INSTANCES];
}
...
function directEvidenceMatchedKeyword(item = {}, keywords = []) {
  const text = cleanText(`${item.title || ""} ${item.content || ""} ${item.url || ""}`, 4000).toLowerCase();
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  for (const keyword of normalizedKeywords) {
    const value = cleanText(keyword, 180);
    if (value && text.includes(value.toLowerCase())) return value;
  }
  const genericParts = new Set(["gitlab", "github", "issue", "issues", "bug", "report", "incident", "discussion", "comments"]);
  const parts = normalizedKeywords
    .flatMap(keyword => cleanText(keyword, 180).split(/[\s,;|/()[\]{}"'`~!@#$%^&*_+=:：，。！？、-]+/))
    .map(part => cleanText(part, 80))
    .filter(part => part.length >= 2 && !genericParts.has(part.toLowerCase()))
    .sort((a, b) => b.length - a.length);
  return parts.find(part => text.includes(part.toLowerCase())) || normalizedKeywords[0] || "";
}
...
function insertFederatedItem(item, { platform, keyword, domainControls = {}, contentControls = {}, failoverAttribution = [] }) {
  ...
  const directCollector = /direct/i.test(String(item.evidenceType || ""))
    || /direct-url/i.test(String(item.metrics?.deep_collector || ""));
...
function gitLabIssueDirectReferenceFromUrl(url = "") {
  const raw = cleanText(url, 1200);
  if (!raw) return null;
  try {
    ...
    if (dashIndex < 0 || issueIndex < 0 || issueIndex <= dashIndex || segments[issueIndex - 1] !== "-") return null;
    const projectSegments = segments.slice(0, dashIndex);
    const issueIid = cleanText(segments[issueIndex + 1] || "", 80);
    if (projectSegments.length < 2 || !/^\d+$/.test(issueIid)) return null;
    parsed.hostname = host;
    parsed.hash = "";
    parsed.search = "";
    parsed.pathname = `/${projectSegments.join("/")}/-/issues/${issueIid}`;
    return {
      directUrl: normalizeFederatedDedupeUrl(parsed.toString()),
      instanceUrl: `${parsed.protocol}//${host}`,
...
function normalizeGitLabIssueDirectUrls(values = [], limit = 20) {
  const raw = Array.isArray(values)
    ? values
    : typeof values === "string"
      ? values.split(/[\n,，]+/)
  ...
  const seen = new Set();
  for (const value of raw) {
    const reference = gitLabIssueDirectReferenceFromUrl(value);
    if (!reference?.directUrl || seen.has(reference.directUrl)) continue;
    seen.add(reference.directUrl);
    out.push(reference);
    if (out.length >= limit) break;
...
function gitLabIssueNotesUrl({ instanceUrl = "https://gitlab.com", projectId = "", projectPath = "", issueIid = "" } = {}) {
  const projectRef = cleanText(projectId || projectPath, 240);
  const iid = cleanText(issueIid, 80);
  const instance = normalizeSiteUrl(instanceUrl) || "https://gitlab.com";
  if (!projectRef || !iid) return "";
  return `${instance}/api/v4/projects/${encodeURIComponent(projectRef)}/issues/${encodeURIComponent(iid)}/notes`;
}
...
function discourseTopicDirectReferenceFromUrl(url = "") {
  const raw = cleanText(url, 1200);
  if (!raw) return null;
  try {
    const parsed = new URL(raw);
    const segments = parsed.pathname.replace(/\.json$/i, "").split("/").filter(Boolean);
    const tIndex = segments.findIndex(segment => segment.toLowerCase() === "t");
    if (tIndex < 0) return null;
    const after = segments.slice(tIndex + 1);
    if (!after.length) return null;
    const idIndex = after.findIndex(segment => /^\d+$/.test(segment));
    if (idIndex < 0) return null;
    const topicId = cleanText(after[idIndex], 80);
    const slug = cleanText(idIndex > 0 ? after[idIndex - 1] : "", 200) || topicId;
    if (!topicId) return null;
    parsed.hash = "";
    parsed.search = "";
    const topicPath = slug === topicId ? `/t/${topicId}` : `/t/${slug}/${topicId}`;
    parsed.pathname = topicPath;
    return {
      directUrl: normalizeFederatedDedupeUrl(parsed.toString()),
      siteUrl: `${parsed.protocol}//${parsed.host}`,
      slug,
      topicId,
      topicJsonUrl: `${parsed.protocol}//${parsed.host}${topicPath}.json`,
    };
  } catch {
    return null;
  }
...
function normalizeDiscourseDirectUrls(values = [], limit = 20) {
  const raw = Array.isArray(values)
    ? values
    : typeof values === "string"
      ? values.split(/[\n,，]+/)
  ...
  const seen = new Set();
  for (const value of raw) {
    const reference = discourseTopicDirectReferenceFromUrl(value);
    if (!reference?.directUrl || seen.has(reference.directUrl)) continue;
    seen.add(reference.directUrl);
    out.push(reference);
    if (out.length >= limit) break;
...
function parseGitLabIssues(payload, keyword, limit = 10, since = "", instanceUrl = "https://gitlab.com") {
  const items = Array.isArray(payload) ? payload : [];
  const normalizedInstance = normalizeSiteUrl(instanceUrl) || "https://gitlab.com";
  return items.map(item => {
    ...
    return withFederatedKeywordDiagnostics({
      ...
      notesUrl: gitLabIssueNotesUrl({ instanceUrl: normalizedInstance, projectId, projectPath, issueIid }),
      ...
      metrics: {
        ...
        gitlab_instance: normalizedInstance,
...
function parseDiscourseTopicDirect(payload = {}, keyword = "", { siteUrl = "", directUrl = "", limit = 1 } = {}) {
  if (!payload || typeof payload !== "object") return [];
  const normalizedSite = normalizeSiteUrl(siteUrl);
  const posts = Array.isArray(payload?.post_stream?.posts) ? payload.post_stream.posts : [];
  const firstPost = posts[0] || {};
  const reference = discourseTopicReference({
    id: payload.id || payload.topic_id,
    slug: payload.slug,
    url: directUrl,
  }, normalizedSite);
  const title = cleanText(payload.title || payload.fancy_title || "", 300);
  const content = cleanText(firstPost.cooked || firstPost.blurb || payload.excerpt || "", 1200);
  const postsCount = Number(payload.posts_count || posts.length || 0);
  const views = Number(payload.views || 0);
  const likes = Number(payload.like_count || firstPost.like_count || 0);
  const signals = discourseTopicSignals({
    title,
    content,
  ...
  const item = withFederatedKeywordDiagnostics({
    url: cleanText(reference.topicUrl || directUrl, 800),
    title,
    content,
    author: cleanText(firstPost.username || payload.last_poster_username || "Discourse", 160),
    publishedAt: normalizeIsoDate(payload.bumped_at || payload.created_at || firstPost.updated_at || firstPost.created_at),
    topicJsonUrl: reference.topicJsonUrl,
    evidenceType: "discourse_topic_direct",
    metrics: {
      site: normalizedSite,
      source_family: "community",
      source_kind: "product_forum_discussion",
      collection_mode: "discourse_direct_topic_json",
      deep_collector: "discourse-topic-direct-url",
      source: "discourse_topic_direct",
      source_weight_tier: "product-community",
      direct_url: directUrl,
      discourse_direct_url: directUrl,
      discourse_site: reference.site,
      discourse_topic_id: reference.topicId,
      discourse_topic_slug: reference.slug,
      discourse_topic_url: reference.topicUrl || directUrl,
      discourse_posts_count: postsCount,
      discourse_views: views,
      discourse_like_count: likes,
      discourse_engagement_score: postsCount * 2 + Math.floor(views / 25) + likes * 3,
      discourse_closed: payload.closed === true ? 1 : 0,
      discourse_archived: payload.archived === true ? 1 : 0,
      posts: postsCount,
      views,
      like_count: likes,
      discourse_category_id: cleanText(payload.category_id || "", 80),
      discourse_category_name: cleanText(payload.category_name || "", 160),
      ...signals,
    },
  }, keyword);
  return item.url && item.title ? [item].slice(0, limit) : [];
}

async function fetchDirectDiscourseTopic(reference, { proxyUrl = "", keyword = "", maxComments = MAX_FEDERATED_COMMENTS } = {}) {
  if (!reference?.topicJsonUrl) return null;
  const res = await fetchPublicSource(reference.topicJsonUrl, {
    headers: {
      "User-Agent": USER_AGENT,
      "Accept": "application/json",
    },
  ...
  const items = parseDiscourseTopicDirect(payload, keyword, {
    siteUrl: reference.siteUrl,
    directUrl: reference.directUrl,
    limit: 1,
  });
  let item = items[0] || null;
  if (!item && payload?.title) {
    item = parseDiscourseTopicDirect(payload, cleanText(payload.title, 180), {
      siteUrl: reference.siteUrl,
      directUrl: reference.directUrl,
      limit: 1,
    })[0] || null;
  ...
  if (!item) return null;
  const comments = parseDiscourseTopicPosts(payload, maxComments, keyword);
  return {
    ...item,
    comments,
    metrics: {
      ...(item.metrics || {}),
      discourse_reply_count: comments.length,
      community_comment_depth_signal: comments.length ? 1 : 0,
    },
  };
}
...
async function fetchDirectLemmyPost(reference, { proxyUrl = "", keyword = "", since = "", maxComments = MAX_FEDERATED_COMMENTS } = {}) {
  if (!reference?.apiUrl || !reference?.instanceUrl || !reference?.postId) return null;
  const res = await fetchPublicSource(reference.apiUrl, {
    headers: {
      "User-Agent": USER_AGENT,
      "Accept": "application/json",
    },
  ...
  if (!res.ok) throw new Error(httpFailure(res));
  const payload = await res.json();
  const postView = payload?.post_view || payload?.postView || payload?.post || null;
  if (!postView) return null;
  const asSearchPayload = { posts: [postView] };
  let item = parseLemmySearch(asSearchPayload, keyword, {
    instanceUrl: reference.instanceUrl,
    limit: 1,
    since,
  })[0] || null;
  if (!item) {
    const titleKeyword = cleanText(postView?.post?.name || postView?.post?.body || keyword, 180);
    item = parseLemmySearch(asSearchPayload, titleKeyword, {
      instanceUrl: reference.instanceUrl,
      limit: 1,
      since,
    })[0] || null;
  ...
  if (!item) return null;
  const embeddedComments = Array.isArray(payload?.comments)
    ? parseLemmyComments(payload, maxComments, keyword)
    : [];
  const fetchedComments = embeddedComments.length
    ? embeddedComments
    : await fetchLemmyPostComments(item, { proxyUrl, maxComments, keyword });
  return {
    ...item,
    url: reference.directUrl || item.url,
    comments: fetchedComments,
    evidenceType: "lemmy_direct_post",
    metrics: {
      ...(item.metrics || {}),
      ...
      lemmy_direct_url: reference.directUrl,
      lemmy_direct_instance: reference.instanceUrl,
      lemmy_direct_post_id: reference.postId,
      lemmy_comment_depth: fetchedComments.length,
      community_comment_depth_signal: fetchedComments.length ? 1 : 0,
      direct_original_source_recovery: 1,
    },
  };
}

async function fetchDirectGitLabIssue(reference, { proxyUrl = "", keyword = "", since = "", maxComments = MAX_FEDERATED_COMMENTS } = {}) {
  if (!reference?.instanceUrl || !reference?.projectPath || !reference?.issueIid) return null;
  const apiUrl = `${reference.instanceUrl}/api/v4/projects/${encodeURIComponent(reference.projectPath)}/issues/${encodeURIComponent(reference.issueIid)}`;
  const res = await fetchPublicSource(apiUrl, {
    headers: {
      "User-Agent": USER_AGENT,
      "Accept": "application/json",
    },
  ...
  if (!res.ok) throw new Error(httpFailure(res));
  const payload = await res.json();
  const items = parseGitLabIssues([{
    ...payload,
    web_url: payload.web_url || reference.directUrl,
    project_path: payload.project_path || reference.projectPath,
    iid: payload.iid || reference.issueIid,
  }], keyword, 1, "", reference.instanceUrl);
  let item = items[0] || null;
  if (!item && payload?.title && isAfterSince(normalizeIsoDate(payload.updated_at || payload.created_at), since)) {
    const titleKeyword = cleanText(payload.title, 180);
    item = parseGitLabIssues([{
      ...payload,
      web_url: payload.web_url || reference.directUrl,
      project_path: payload.project_path || reference.projectPath,
      iid: payload.iid || reference.issueIid,
    }], titleKeyword, 1, "", reference.instanceUrl)[0] || null;
  }
  if (!item) return null;
  const projectId = cleanText(payload.project_id || item.metrics?.gitlab_project_id || "", 80);
  item.notesUrl = gitLabIssueNotesUrl({
    instanceUrl: reference.instanceUrl,
    projectId,
    projectPath: reference.projectPath,
    issueIid: reference.issueIid,
  });
  const comments = await fetchGitLabIssueNotes(item, { proxyUrl, maxComments, keyword });
  return {
    ...item,
    comments,
    evidenceType: "gitlab_issue_direct",
    metrics: {
      ...(item.metrics || {}),
      collection_mode: "gitlab_direct_issue_api",
      deep_collector: "gitlab-issue-direct-url",
      source: "gitlab_issue_direct",
      direct_url: reference.directUrl,
      gitlab_direct_url: reference.directUrl,
      gitlab_direct_instance: reference.instanceUrl,
      gitlab_project_path: item.metrics?.gitlab_project_path || reference.projectPath,
      gitlab_issue_iid: item.metrics?.gitlab_issue_iid || reference.issueIid,
      gitlab_note_count: comments.length,
      community_comment_depth_signal: comments.length ? 1 : 0,
    },
  };
}

export async function scrapeGitLabIssues(keywords, { proxyUrl = "", budget = {}, deepBudget = null, since = "", domainControls = {}, contentControls = {}, projects = [], projectIds = [], project_ids = [], instances = [], gitLabInstances = [], gitlabInstances = [], gitlab_instances = [], failoverAttribution = [], directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeGitLabIssueDirectUrls(directUrls, 20);
  ...
  const gitlabInstancesToScan = normalizeGitLabInstances(
    instances?.length
      ? instances
      : gitLabInstances?.length
        ? gitLabInstances
        : gitlabInstances?.length
          ? gitlabInstances
          : gitlab_instances,
  ...
  for (const reference of normalizedDirectUrls) {
    const keyword = normalizedKeywords[0] || "";
    try {
      const item = await fetchDirectGitLabIssue(reference, {
        proxyUrl,
        keyword,
        since,
      ...
      if (!item || !dedupeKey || seenItemUrls.has(dedupeKey)) continue;
      const matchedKeyword = directEvidenceMatchedKeyword(item, normalizedKeywords) || keyword;
      const insertedCount = insertFederatedItem(item, { platform: "gitlab_issues", keyword: matchedKeyword, domainControls, contentControls, failoverAttribution });
      if (insertedCount > 0) {
        seenItemUrls.add(dedupeKey);
        directInserted += insertedCount;
      }
    } catch (err) {
      const message = formatSourceError(err, proxyUrl);
      directFailures.push({ keyword, target: reference.directUrl, message });
      console.warn(`[CRM/GitLabIssues] 直抓失敗 url=${reference.directUrl}: ${message}`);
    }
  }

  const tasks = [];
  for (const instanceUrl of gitlabInstancesToScan) {
    for (const keyword of normalizedKeywords) {
      for (const target of targets) tasks.push({ instanceUrl, keyword, target });
    }
  }

  const results = await mapWithConcurrency(tasks, KEYWORD_CONCURRENCY, async ({ instanceUrl, keyword, target }) => {
    let inserted = 0;
    const failures = [];
    try {
      for (let page = 1; page <= normalizedBudget.maxPagesPerKeyword && inserted < normalizedBudget.maxItemsPerKeyword; page += 1) {
        ...
        const url = target.project
          ? `${instanceUrl}/api/v4/projects/${encodeURIComponent(target.project)}/search?${params.toString()}`
          : `${instanceUrl}/api/v4/search?${params.toString()}`;
        const res = await fetchPublicSource(url, {
          headers: {
            "User-Agent": USER_AGENT,
            "Accept": "application/json",
        ...
        if (!res.ok) {
          failures.push({ keyword, target: `${instanceUrl}:${target.label}:page:${page}`, message: httpFailure(res) });
          break;
        }
        const payload = await res.json();
        const rawItems = Array.isArray(payload) ? payload : [];
        const items = parseGitLabIssues(payload, keyword, normalizedBudget.maxItemsPerKeyword - inserted, since, instanceUrl);
        ...
        for (const item of items) {
          ...
          const insertedCount = insertFederatedItem({
            ...item,
            comments,
            metrics: {
              ...(item.metrics || {}),
              search_instance: instanceUrl,
              gitlab_search_instance: instanceUrl,
      ...
      console.warn(`[CRM/GitLabIssues] 抓取失敗 instance=${instanceUrl} keyword=${keyword}: ${message}`);
    }
...
export async function scrapeDiscourseForums(keywords, { proxyUrl = "", budget = {}, deepBudget = null, since = "", sites = [], targetProfiles = [], domainControls = {}, contentControls = {}, failoverAttribution = [], directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeDiscourseDirectUrls(directUrls, 20);
  if (!normalizedKeywords.length && !normalizedDirectUrls.length) return scraperResult(0);
  const normalizedBudget = normalizeBudget(budget);
  const normalizedDeepBudget = normalizeDeepBudget(deepBudget);
  ...
  for (const reference of normalizedDirectUrls) {
    const keyword = normalizedKeywords[0] || "";
    try {
      const item = await fetchDirectDiscourseTopic(reference, {
        proxyUrl,
        keyword,
        maxComments: normalizedDeepBudget.maxCommentsPerItem,
      });
      ...
      if (!item || !dedupeKey || seenItemUrls.has(dedupeKey)) continue;
      const matchedKeyword = directEvidenceMatchedKeyword(item, normalizedKeywords) || keyword;
      const insertedCount = insertFederatedItem(item, { platform: "discourse", keyword: matchedKeyword, domainControls, contentControls, failoverAttribution });
      if (insertedCount > 0) {
        seenItemUrls.add(dedupeKey);
        directInserted += insertedCount;
      }
      ...
      const message = formatSourceError(err, proxyUrl);
      directFailures.push({ keyword, target: reference.directUrl, message });
      console.warn(`[CRM/Discourse] 直抓失敗 url=${reference.directUrl}: ${message}`);
    }
  }
...
export async function scrapeLemmySearch(keywords, { proxyUrl = "", budget = {}, deepBudget = null, since = "", instances = [], domainControls = {}, contentControls = {}, failoverAttribution = [], directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeLemmyDirectUrls(directUrls, 20);
  if (!normalizedKeywords.length && !normalizedDirectUrls.length) return scraperResult(0);
  const normalizedBudget = normalizeBudget(budget);
  const normalizedDeepBudget = normalizeDeepBudget(deepBudget);
  ...
  for (const reference of normalizedDirectUrls) {
    const keyword = normalizedKeywords[0] || "";
    try {
      const item = await fetchDirectLemmyPost(reference, {
        proxyUrl,
        keyword,
        since,
        maxComments: normalizedDeepBudget.maxCommentsPerItem,
      ...
      if (!item || !dedupeKey || seenItemUrls.has(dedupeKey)) continue;
      const matchedKeyword = directEvidenceMatchedKeyword(item, normalizedKeywords) || keyword;
      const insertedCount = insertFederatedItem(item, { platform: "lemmy", keyword: matchedKeyword, domainControls, contentControls, failoverAttribution });
      if (insertedCount > 0) {
        seenItemUrls.add(dedupeKey);
        directInserted += insertedCount;
      }
      ...
      const message = formatSourceError(err, proxyUrl);
      directFailures.push({ keyword, target: reference.directUrl, message });
      console.warn(`[CRM/Lemmy] 直抓失敗 url=${reference.directUrl}: ${message}`);
    }
  }
...
export const __test__ = {
  ...
  discourseTopicDirectReferenceFromUrl,
  normalizeDiscourseDirectUrls,
  normalizeLemmyInstances,
  lemmyPostDirectReferenceFromUrl,
  normalizeLemmyDirectUrls,
  normalizeGitLabInstances,
  normalizeGitLabProjects,
  gitLabIssueDirectReferenceFromUrl,
  normalizeGitLabIssueDirectUrls,

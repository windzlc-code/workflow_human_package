...
function normalizeSubstackDirectUrls(values = [], limit = 20) {
  ...
  for (const value of raw) {
    const normalized = normalizeSubstackUrl(value);
    if (!normalized || !isConcreteSubstackUrl(normalized)) continue;
    const dedupe = normalizeSubstackDedupeUrl(normalized);
...
function substackTitleFromUrl(url = "") {
  try {
    const parsed = new URL(url);
    const segments = parsed.pathname.split("/").filter(Boolean);
    const postIndex = segments.findIndex(segment => segment.toLowerCase() === "p");
    const slug = postIndex >= 0
    ...
    const title = stripTags(String(slug || "")
      ...
      .replace(/\s+/g, " ")
      .trim(), 240);
    return title || "Substack post";
  } catch {
    return "Substack post";
  }
}

function substackTitleFromHtml(html = "", fallback = "") {
...
async function insertSubstackItems(items, {
  ...
  for (const item of items) {
    ...
    const title = substackTitleFromHtml(enriched.raw_html || "", item.title) || item.title;
    ...
    const result = insertSentimentItem({
      ...
      keyword,
      keywords: [...new Set([keyword, ...substackKeywordNeedles(keyword)])],
      ...
      evidence: {
        ...(enriched.evidence || {}),
        source_key: "substackSearch",
        evidence_type: item.evidenceType || enriched.evidence?.evidence_type || "substack_public_search_result",
        metrics: {
          ...finalMetrics,
...
export async function scrapeSubstackSearch(keywords, { proxyUrl = "", enrich = true, budget = {}, deepBudget = null, since = "", domainControls = {}, contentControls = {}, directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeSubstackDirectUrls(directUrls, 20);
  ...
  if (normalizedDirectUrls.length) {
    const directKeyword = normalizedKeywords[0] || "substack-direct-url";
    const directItems = normalizedDirectUrls.map((url) => ({
      url,
      title: substackTitleFromUrl(url),
      content: "",
      author: "Substack 直達文章",
      publishedAt: new Date().toISOString(),
      evidenceType: "substack_direct_post",
      metrics: {
        source_key: "substackSearch",
        source_family: "knowledge",
        source_kind: "substack_direct_url",
        collection_mode: "substack_direct_url",
        deep_collector: "substack-direct-url",
        source: "substack_direct_post",
        direct_url: url,
        substack_direct_url: url,
        substack_evidence_kind: substackEvidenceKind(url),
      },
    }));
    try {
      inserted += await insertSubstackItems(directItems, {
      ...
      failures.push({ keyword: directKeyword, target: "substack-direct-url", message });
      console.warn(`[Sentiment/Substack] 直抓失敗 keyword=${directKeyword}: ${message}`);
    }
  }
...
export const __test__ = {
  ...
  normalizeSubstackDirectUrls,
  normalizeBudget,
  normalizeSubstackDedupeUrl,
  normalizeSubstackUrl,
  substackTitleFromHtml,
  substackTitleFromUrl,

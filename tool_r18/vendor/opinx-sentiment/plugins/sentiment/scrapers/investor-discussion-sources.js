...
const MARKET_ARTICLE_HOSTS = new Set([
  "finance.yahoo.com",
  "seekingalpha.com",
  "marketwatch.com",
  "investors.com",
...
function normalizeDirectUrls(directUrls = []) {
  return [...new Set((Array.isArray(directUrls) ? directUrls : [])
    .map(url => normalizeInvestorDiscussionDedupeUrl(url))
    .filter(url => /^https?:\/\//i.test(url)))].slice(0, 30);
}

function directInvestorDiscussionTarget(rawUrl = "") {
  const normalizedUrl = normalizeInvestorDiscussionDedupeUrl(rawUrl);
  ...
  try {
    ...
    if (host === "stocktwits.com") {
      const messageMatch = path.match(/^\/([^/]+)\/message\/(\d+)$/i);
      if (messageMatch) {
        return {
          platform: "stocktwits",
          type: "message",
          url: normalizedUrl,
          username: cleanText(decodeURIComponent(messageMatch[1] || ""), 120),
          messageId: cleanText(messageMatch[2] || "", 80),
        };
      }
      const symbolMatch = path.match(/^\/symbol\/([A-Z0-9.=-]{1,8})$/i);
      if (symbolMatch) {
        return {
          platform: "stocktwits",
          type: "symbol",
          url: normalizedUrl,
          symbol: cleanText(symbolMatch[1] || "", 20).toUpperCase(),
        };
      }
    }
    if (host === "finance.yahoo.com") {
      const quoteMatch = path.match(/^\/quote\/([A-Z0-9.=-]{1,12})(?:\/|$)/i);
      if (quoteMatch) {
        return {
          platform: "yahoo_finance",
          type: "quote",
          url: normalizedUrl,
          symbol: cleanText(decodeURIComponent(quoteMatch[1] || ""), 20).toUpperCase(),
        };
      }
    }
    if (MARKET_ARTICLE_HOSTS.has(host) && /\/(?:news|article|articles|story|amp|m\/story|analysis|investing|markets?|earnings|stock|stocks|quote|company|research|video|press-release|prnewswire|globenewswire)\b/i.test(path)) {
      return {
        platform: host,
        type: "market_article",
        url: normalizedUrl,
...
function parseStocktwitsDirectMessage(html = "", keyword = "", target = {}, { since = "" } = {}) {
  if (!target?.url || target.platform !== "stocktwits" || target.type !== "message") return null;
  const title = htmlMetaContent(html, ["og:title", "twitter:title"]) || htmlTitle(html) || `Stocktwits investor discussion: ${keyword}`;
  const description = htmlMetaContent(html, ["og:description", "twitter:description", "description"]);
  const bodyFallback = cleanText(String(html || "").replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<style[\s\S]*?<\/style>/gi, " "), 1200);
  const content = cleanText(description || bodyFallback, 1000);
  if (!content) return null;
  const publishedAt = normalizeDate(htmlMetaContent(html, ["article:published_time", "datePublished", "pubdate"])) || new Date().toISOString();
  if (!isAfterSince(publishedAt, since)) return null;
  const mentionedSymbols = symbolListFromMessage({ body: `${title} ${content}` });
  const symbols = [...new Set(mentionedSymbols)].slice(0, 12);
  const sentimentHint = cleanText(htmlMetaContent(html, ["stocktwits:sentiment", "sentiment"]), 40);
  if (!textMatchesKeyword(`${title} ${content} ${target.url}`, keyword, symbols)) return null;
  if (!hasMarketContext(`${title} ${content}`, sentimentHint)) return null;
  const narrativeSignals = investorNarrativeSignals({ content: `${title} ${content}`, sentimentHint, symbols });
  return {
    url: target.url,
    title: title || `Stocktwits investor discussion: ${keyword}`,
    content,
    author: target.username || "Stocktwits user",
    publishedAt,
    metrics: {
      ...
      investor_discussion_direct_url: target.url,
      investor_discussion_direct_platform: "stocktwits",
      investor_discussion_direct_type: "message",
...
function parseDirectMarketArticle(enriched = {}, keyword = "", target = {}, { since = "" } = {}) {
  if (!target?.url || target.type !== "market_article") return null;
  const title = cleanText(enriched.title || target.title || "", 260)
    || cleanText(enriched.evidence?.canonical_url || enriched.evidence?.site_name || "", 260)
    || `Investor market article: ${keyword}`;
  const content = cleanText(enriched.content || enriched.ai_summary || "", 1600);
  if (!content) return null;
  const publishedAt = normalizeDate(enriched.published_at || enriched.evidence?.published_time || enriched.evidence?.metrics?.published_time) || new Date().toISOString();
  if (!isAfterSince(publishedAt, since)) return null;
  const symbols = symbolCandidatesForKeyword(`${keyword} ${title} ${content}`);
  const sentimentHint = /bearish|short|selloff|downgrade|lawsuit|investigation|fraud|下跌|做空|訴訟|调查|調查/i.test(`${title} ${content}`)
    ? "Bearish"
    : "";
  if (!textMatchesKeyword(`${title} ${content} ${target.url}`, keyword, symbols)) return null;
  if (!hasMarketContext(`${title} ${content}`, sentimentHint)) return null;
  ...
  const canonicalUrl = normalizeInvestorDiscussionDedupeUrl(
    metrics.canonical_url || enriched.evidence?.canonical_url || enriched.evidence?.og_url || target.url,
  );
  return {
    url: canonicalUrl || target.url,
    title,
    content,
    author: cleanText(enriched.author || metrics.author || target.host || "Market article", 140),
    publishedAt,
    metrics: {
      source: "public_market_article_direct_url",
      source_family: "finance",
      source_kind: "public_investor_market_article",
      collection_mode: "market_article_direct_html",
      investor_discussion_direct_url: target.url,
      investor_discussion_direct_platform: target.host || target.platform || "",
      investor_discussion_direct_type: "market_article",
      canonical_url: canonicalUrl,
      site_name: cleanText(enriched.evidence?.site_name || metrics.site_name || target.host || "", 160),
      article_body_quality_score: Number(metrics.article_body_quality_score || 0) || 0,
      article_body_text_length: Number(metrics.article_body_text_length || content.length || 0) || 0,
      article_body_paragraph_count: Number(metrics.article_body_paragraph_count || 0) || 0,
      article_body_excerpt: cleanText(metrics.article_body_excerpt || content, 800),
      structured_followup_link_count: Number(metrics.structured_followup_link_count || 0) || 0,
      propagation_followup_link_count: Number(metrics.propagation_followup_link_count || 0) || 0,
      structured_followup_links: Array.isArray(metrics.structured_followup_links) ? metrics.structured_followup_links.slice(0, 8) : [],
      propagation_followup_links: Array.isArray(metrics.propagation_followup_links) ? metrics.propagation_followup_links.slice(0, 8) : [],
      created_at: publishedAt,
      sentiment_hint: sentimentHint,
      mentioned_symbols: symbols,
      symbol: symbols[0] || "",
...
async function scrapeDirectInvestorDiscussionUrls(keyword = "", directUrls = [], { proxyUrl = "", since = "", domainControls = {}, contentControls = {}, seenItemUrls = null, failoverAttribution = [] } = {}) {
  let inserted = 0;
  const failures = [];
  const symbolHints = [];
  const targets = normalizeDirectUrls(directUrls).map(directInvestorDiscussionTarget).filter(Boolean);
  for (const target of targets) {
    if (target.symbol && !symbolHints.includes(target.symbol)) symbolHints.push(target.symbol);
    if (target.type === "market_article") {
      try {
        const enriched = await enrichSearchResultSummary({
          url: target.url,
          title: `Investor market article: ${keyword}`,
          content: keyword,
        }, { proxyUrl });
        const item = parseDirectMarketArticle(enriched, keyword, target, { since });
        if (!item) continue;
        inserted += await insertInvestorItems([item], { keyword, domainControls, contentControls, seenItemUrls, failoverAttribution });
      } catch (err) {
        failures.push({ keyword, target: `market-article-direct:${target.url}`, message: formatSourceError(err, proxyUrl) });
      }
      continue;
    }
    if (target.platform !== "stocktwits" || target.type !== "message") continue;
    try {
      ...
      if (!res.ok) {
        failures.push({ keyword, target: `stocktwits-direct:${target.messageId || target.url}`, message: httpFailure(res) });
        continue;
      }
      const item = parseStocktwitsDirectMessage(await res.text(), keyword, target, { since });
      if (!item) continue;
      inserted += await insertInvestorItems([item], { keyword, domainControls, contentControls, seenItemUrls, failoverAttribution });
    } catch (err) {
      failures.push({ keyword, target: `stocktwits-direct:${target.messageId || target.url}`, message: formatSourceError(err, proxyUrl) });
    }
  }
  return { inserted, failures, symbolHints };
}

export async function scrapeInvestorDiscussionSources(keywords, { proxyUrl = "", budget = {}, since = "", domainControls = {}, contentControls = {}, failoverAttribution = [], directUrls = [] } = {}) {
  ...
  const results = await mapWithConcurrency(normalizedKeywords, SEARCH_CONCURRENCY, async (keyword) => {
    let inserted = 0;
    const failures = [];
    const directResult = await scrapeDirectInvestorDiscussionUrls(keyword, directUrls, {
      proxyUrl,
      since,
      domainControls,
      contentControls,
    ...
    failures.push(...directResult.failures);
    try {
      const symbols = symbolCandidatesForKeyword(keyword);
      for (const symbol of directResult.symbolHints || []) {
...
export const __test__ = {
  ...
  normalizeDirectUrls,
  directInvestorDiscussionTarget,
  ...
  parseStocktwitsDirectMessage,
  parseDirectMarketArticle,

...
function normalizeStatusPageDirectUrls(directUrls = []) {
  ...
  for (const value of raw) {
    const normalized = normalizeStatusPageDedupeUrl(value);
...
function statusPageSummaryUrlFromDirectUrl(rawUrl = "") {
  const normalized = normalizeStatusPageDedupeUrl(rawUrl);
  if (!normalized) return "";
  try {
    const url = new URL(normalized);
    if (/\/api\/v2\/summary\.json$/i.test(url.pathname)) return url.toString();
    const incidentIndex = url.pathname.search(/\/incidents?\//i);
    if (incidentIndex >= 0) {
      url.pathname = url.pathname.slice(0, incidentIndex) || "/";
      url.search = "";
      return statusSummaryUrl(url.toString());
    }
    url.search = "";
    return statusSummaryUrl(url.toString());
  } catch {
...
function statusPageDirectIncidentNeedles(rawUrl = "") {
  const normalized = normalizeStatusPageDedupeUrl(rawUrl);
  const out = [];
  try {
    const url = new URL(normalized);
    const parts = url.pathname.split("/").filter(Boolean);
    const incidentIndex = parts.findIndex(part => /^incidents?$/i.test(part));
    if (incidentIndex >= 0 && parts[incidentIndex + 1]) out.push(decodeURIComponent(parts[incidentIndex + 1]));
    out.push(url.toString());
  } catch {
    if (normalized) out.push(normalized);
  }
  return [...new Set(out.map(item => cleanText(item, 300)).filter(Boolean))];
}

function statusPageIncidentMatchesDirectUrl(incident = {}, directUrl = "") {
  const needles = statusPageDirectIncidentNeedles(directUrl);
  ...
  const haystack = [
    ...
    incident.url,
    incident.name,
  ].map(item => normalizeStatusPageDedupeUrl(item || "") || cleanText(item || "", 300)).join(" ").toLowerCase();
...
async function collectStatusPageDirectUrlItems(directUrls = [], keyword = "", { proxyUrl = "", limit = DEFAULT_MAX_ITEMS_PER_KEYWORD, since = "" } = {}) {
  const normalizedDirectUrls = normalizeStatusPageDirectUrls(directUrls);
  const rows = [];
  const failures = [];
  for (const directUrl of normalizedDirectUrls) {
    if (rows.length >= limit) break;
    const summaryUrl = statusPageSummaryUrlFromDirectUrl(directUrl);
    if (!summaryUrl) continue;
    try {
      const res = await fetchPublicSource(summaryUrl, {
      ...
      const items = parseStatusPageSummary(payload, keyword, {
        target,
        limit: Math.max(1, limit - rows.length),
        since,
      }).filter(item => statusPageIncidentMatchesDirectUrl({
        id: item.metrics?.status_page_incident_id || item.metrics?.status_page_component_id || "",
        shortlink: item.url,
        url: item.url,
        name: item.title,
      }, directUrl)).map(item => ({
        ...
        metrics: {
          ...(item.metrics || {}),
          source_kind: item.metrics?.source_kind === "component_status" ? "status_page_direct_component" : "status_page_direct_incident",
      ...
      console.warn(`[CRM/PublicStatusPage] Direct URL fetch failed keyword=${keyword} target=${directUrl}: ${message}`);
    }
  }
  return { items: rows.slice(0, limit), failures };
}
...
async function insertStatusPageItems(items = [], { keyword, domainControls = {}, contentControls = {}, seenItemUrls = null, failoverAttribution = [] } = {}) {
  ...
  for (const item of items) {
    ...
    if (seenItemUrls instanceof Set && seenItemUrls.has(dedupeKey)) continue;
    ...
    if (seenItemUrls instanceof Set && (result.inserted || result.updated)) {
...
export async function scrapePublicStatusPageSources(keywords, { proxyUrl = "", budget = {}, since = "", targets = [], domainControls = {}, contentControls = {}, failoverAttribution = [], directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeStatusPageDirectUrls(directUrls);
  if (!normalizedKeywords.length && !normalizedDirectUrls.length) return scraperResult(0);
  const normalizedBudget = normalizeBudget(budget);
  const seenItemUrls = new Set();
  const directKeyword = normalizedKeywords[0] || "status-page-direct-url";
  let directInserted = 0;
  const directFailures = [];
  if (normalizedDirectUrls.length) {
    const direct = await collectStatusPageDirectUrlItems(normalizedDirectUrls, directKeyword, {
      proxyUrl,
      limit: normalizedBudget.maxItemsPerKeyword,
      since,
    });
    directFailures.push(...direct.failures);
    directInserted += await insertStatusPageItems(direct.items, {
  ...
  const hasConfiguredTargets = Array.isArray(targets) && targets.length > 0;
  const keywordTargetKeywords = normalizedDirectUrls.length && !hasConfiguredTargets ? [] : normalizedKeywords;
  const results = await mapWithConcurrency(keywordTargetKeywords, SEARCH_CONCURRENCY, async (keyword) => {
    let inserted = 0;
    const failures = [];
...
export const __test__ = {
  ...
  normalizeStatusPageDirectUrls,
  statusPageDedupeKey,
  hostnameFromKeyword,
  asciiBrandToken,
  statusSummaryUrl,
  statusPageSummaryUrlFromDirectUrl,
  statusPageDirectIncidentNeedles,
  statusPageIncidentMatchesDirectUrl,
  collectStatusPageDirectUrlItems,
  normalizeTargets,

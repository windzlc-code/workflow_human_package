...
function normalizeNewsRequestBudget(budget = {}) {
  const raw = budget && typeof budget === "object" ? budget : {};
  const maxPages = clampNumber(raw.maxPagesPerKeyword ?? raw.max_pages_per_keyword, 1, 1, 10);
  const defaultMaxRequests = maxPages <= 1 ? 6 : 12;
  return {
    requestTimeoutMs: clampNumber(
      raw.newsRequestTimeoutMs
        ?? raw.news_request_timeout_ms
    ...
    maxRequestsPerKeyword: clampNumber(
      raw.maxNewsRequestsPerKeyword
        ?? raw.max_news_requests_per_keyword
        ?? raw.maxRequestsPerKeyword
        ?? raw.max_requests_per_keyword,
      defaultMaxRequests,
      1,
      200,
    ),
    maxConsecutiveFailuresPerKeyword: clampNumber(
      raw.maxConsecutiveNewsFailuresPerKeyword
        ?? raw.max_consecutive_news_failures_per_keyword
        ?? raw.maxConsecutiveFailuresPerKeyword
        ?? raw.max_consecutive_failures_per_keyword,
      3,
      1,
      20,
    ),
...
async function scrapeNewsRss(keywords, { proxyUrl = "", enrich = true, budget = {}, deepBudget = null, since = "", newsEngines = DEFAULT_GOOGLE_NEWS_ENGINES, newsMarkets = undefined, platform = "google_news", domainControls = {}, contentControls = {}, failoverAttribution = [] } = {}) {
  ...
  const requestBudget = normalizeNewsRequestBudget(budget);
  const normalizedDeepBudget = normalizeNewsDeepBudget(deepBudget);
  ...
  const results = await mapWithConcurrency(keywords, KEYWORD_CONCURRENCY, async (keyword) => {
    ...
    const failures = [];
    const diagnostics = {
      news_request_timeout_ms: requestBudget.requestTimeoutMs,
      news_max_requests_per_keyword: requestBudget.maxRequestsPerKeyword,
      news_max_consecutive_failures_per_keyword: requestBudget.maxConsecutiveFailuresPerKeyword,
      news_requested_target_count: 0,
      news_executed_target_count: 0,
      news_skipped_target_count: 0,
      news_request_failure_count: 0,
      news_failure_breaker_triggered_count: 0,
    };
    try {
      const items = [];
      const seenUrls = new Set();
      const engineTargets = newsEngineUrls(keyword, newsEngines, newsMarkets, normalizedDeepBudget);
      const executableTargets = engineTargets.slice(0, requestBudget.maxRequestsPerKeyword);
      diagnostics.news_requested_target_count += engineTargets.length;
      diagnostics.news_executed_target_count += executableTargets.length;
      diagnostics.news_skipped_target_count += Math.max(0, engineTargets.length - executableTargets.length);
      let consecutiveFailures = 0;
      for (const engineTarget of executableTargets) {
        let res;
        try {
          res = await fetchPublicSource(engineTarget.url, {
            headers: {
              "User-Agent": "Mozilla/5.0 (compatible; OpinXCraw/1.0)",
              "Accept": "application/rss+xml, application/xml, text/xml;q=0.9, */*;q=0.8",
            },
            signal: AbortSignal.timeout(requestBudget.requestTimeoutMs),
          }, proxyUrl);
        } catch (err) {
          const message = formatSourceError(err, proxyUrl);
          diagnostics.news_request_failure_count++;
          consecutiveFailures++;
          failures.push({ keyword, engine: engineTarget.engine, market: engineTarget.market, message });
          if (consecutiveFailures >= requestBudget.maxConsecutiveFailuresPerKeyword) {
            diagnostics.news_failure_breaker_triggered_count++;
            break;
          }
          continue;
        }
        if (!res.ok) {
          diagnostics.news_request_failure_count++;
          consecutiveFailures++;
          failures.push({ keyword, engine: engineTarget.engine, market: engineTarget.market, message: httpFailure(res) });
          if (consecutiveFailures >= requestBudget.maxConsecutiveFailuresPerKeyword) {
            diagnostics.news_failure_breaker_triggered_count++;
            break;
          }
    ...
    return { inserted, failures, diagnostics };
  });
  ...
  return {
    ...result,
    diagnostics: results.reduce((acc, result) => {
      ...
      for (const [key, value] of Object.entries(diagnostics)) {
        ...
        if (key.endsWith("_count")) {
          ...
          acc[key] = Math.max(Number(acc[key] || 0), Number(value || 0));
        }
      }
      return acc;
    }, {}),

 ...
 */
export async function scrapePTT(keywords, { proxyUrl = "", budget = {}, boards = [], directUrls = [] } = {}) {
  const normalizedDirectUrls = normalizePttDirectUrls(directUrls, 20);
  ...
  const directSeenUrls = new Set();

  if (normalizedDirectUrls.length) {
    const directKeyword = keywords[0] || "ptt-direct-url";
    for (const direct of normalizedDirectUrls) {
      try {
        const enriched = await fetchPTTArticleDetails({
          url: direct.url,
          board: direct.board,
          title: direct.title,
          content: "",
        ...
        if (enriched.error) directFailures.push({ keyword: directKeyword, target: direct.url, message: enriched.error });
        const item = {
          ...direct,
          ...enriched,
          board: direct.board,
          url: direct.url,
          title: enriched.title || direct.title,
          author: enriched.author || "",
          publishedAt: enriched.publishedAt || new Date().toISOString(),
          rawCommentCount: enriched.rawCommentCount || enriched.comments?.length || 0,
        };
        if (directSeenUrls.has(item.url)) continue;
        directSeenUrls.add(item.url);
        if (insertPTTArticleItem(item, directKeyword, {
          evidenceType: "ptt_direct_article",
          collectionMode: "ptt_direct_url",
  ...
  for (const result of settled) {
    ...
    for (const article of articles) {
      ...
      const item = {
        ...article,
        ...enriched,
        title: enriched.title || article.title,
        author: enriched.author || article.author,
        publishedAt: enriched.publishedAt || article.publishedAt,
      };
      if (insertPTTArticleItem(item, keyword, {
        rawResultCount: item.searchRawResultCount,
        evidenceType: "ptt_article",
        collectionMode: "ptt_search",
      })) inserted++;
    }
  }

  return scraperResult(directInserted + inserted, [...directFailures, ...failures]);
}

function insertPTTArticleItem(item, keyword, {
  rawResultCount = 0,
  evidenceType = "ptt_article",
  collectionMode = "ptt_search",
  directUrl = "",
  ...
  if (!isRecentDate(item.publishedAt) || !isTaiwanRelatedText(item.title, item.content, item.url, item.author)) return false;
  ...
  const metrics = {
    board: item.board || "",
    search_scope: directCollector ? "direct_url" : (item.board ? "board" : "default-board"),
    search_board: item.board || "",
    comment_count: item.comments.length,
    ptt_search_raw_result_count: Number(rawResultCount || 0),
    ...
    ...(directCollector ? {
      ...
      direct_url: directUrl || item.url,
      ptt_direct_url: directUrl || item.url,
      ptt_board: item.board || "",
      ptt_article_id: pttArticleIdFromUrl(item.url),
    } : {}),
  ...
  const insertResult = insertSentimentItem({
    ...
    keyword,
    keywords: [keyword, ...pttKeywordNeedles(keyword)],
...
function parsePTTArticlePage(html) {
  ...
  const metalines = [...main.matchAll(/<div class="article-metaline">([\s\S]*?)<\/div>/gi)]
    .map(match => stripTags(match[1]));
  const metaValue = (label) => {
    const line = metalines.find(item => item.startsWith(label));
  ...
  const pushRegex = /<div class="push">([\s\S]*?)<\/div>/gi;
  let match;
  ...
  const rawPublishedAt = metaValue("時間");
  const parsedPublishedAt = rawPublishedAt ? new Date(rawPublishedAt) : null;
  return {
    title: metaValue("標題"),
    author: metaValue("作者").replace(/\s*\([^)]*\)\s*$/g, ""),
    publishedAt: parsedPublishedAt && !Number.isNaN(parsedPublishedAt.getTime())
...
function normalizePttUrl(value = "") {
...
function pttArticleIdFromUrl(value = "") {
  try {
    const url = new URL(normalizePttUrl(value));
...
function parsePttArticleUrl(value = "") {
  const url = normalizePttUrl(value);
  if (!url) return null;
  try {
    const parsed = new URL(url);
    const segments = parsed.pathname.split("/").filter(Boolean);
    if (segments[0] !== "bbs" || !segments[1] || !segments[2]) return null;
    if (!/^M\.[^.]+\.[A-Z]\.[A-Z0-9]+\.html$/i.test(segments[2])) return null;
    return {
      board: segments[1],
      articleId: segments[2].replace(/\.html$/i, ""),
      title: `${segments[1]} ${segments[2].replace(/\.html$/i, "")}`,
      url: `https://www.ptt.cc/bbs/${segments[1]}/${segments[2]}`,
      dedupeKey: `${segments[1].toLowerCase()}|${segments[2].toLowerCase()}`,
    };
  } catch {
    return null;
  }
...
function normalizePttDirectUrls(values = [], limit = 20) {
  ...
  for (const value of raw) {
    const parsed = parsePttArticleUrl(value);

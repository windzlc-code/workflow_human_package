...
function normalizeApplePodcastDirectUrls(directUrls = []) {
  ...
  for (const value of values) {
    ...
    if (!["podcasts.apple.com", "itunes.apple.com"].includes(host)) continue;
    if (!/\/podcast\//i.test(parsed.pathname || "")) continue;
    const collectionId = cleanText(parsed.pathname.match(/(?:^|\/)id(\d{5,})(?:$|[/?#])/i)?.[1] || "", 80);
    const trackId = cleanText(parsed.searchParams.get("i") || parsed.searchParams.get("episode") || "", 80);
    if (!collectionId && !trackId) continue;
    ...
    const directUrl = normalizeApplePodcastDedupeUrl(parsed.toString());
    const key = `${lookupId}:${country || "*"}`;
...
function normalizeSpotifyPodcastDirectUrls(directUrls = []) {
  const values = Array.isArray(directUrls) ? directUrls : String(directUrls || "").split(/[\s，、]+/);
  const out = [];
  const seen = new Set();
  for (const value of values) {
    ...
    const match = parsed.pathname.match(/\/(?:intl-[a-z]{2}\/)?(episode|show)\/([A-Za-z0-9]+)(?:[/?#]|$)/i);
    if (!match) continue;
    const type = match[1].toLowerCase();
    const spotifyId = cleanText(match[2], 120);
    if (!spotifyId) continue;
    const canonical = new URL(`https://open.spotify.com/${type}/${spotifyId}`);
    const key = `${type}:${spotifyId}`;
    if (seen.has(key)) continue;
    seen.add(key);
    out.push({
      type,
      spotifyId,
      directUrl: canonical.toString(),
      originalUrl: parsed.toString(),
    });
  }
  return out;
...
function htmlMetaContent(html = "", names = []) {
  ...
  for (const name of names) {
    ...
    const reverseRe = new RegExp(`<meta\\b[^>]*content=["']([^"']*)["'][^>]*(?:property|name)=["']${escaped}["'][^>]*>`, "i");
    const match = source.match(re) || source.match(reverseRe);
...
function parseSpotifyPodcastHtml(html = "", keyword = "", target = {}, { since = "" } = {}) {
  const title = cleanText(htmlMetaContent(html, ["og:title", "twitter:title"]) || htmlTitle(html), 280)
    .replace(/\s*\|\s*Podcast on Spotify\s*$/i, "")
    .replace(/\s*[|-]\s*Spotify\s*$/i, "")
    .trim();
  const description = cleanText(htmlMetaContent(html, ["og:description", "description", "twitter:description"]), 1800);
  const canonicalUrl = normalizeUrl(htmlMetaContent(html, ["og:url"]) || target.directUrl || "");
  const publishedAt = normalizePublishedAt(
    htmlMetaContent(html, ["music:release_date", "article:published_time", "release_date", "datePublished"])
  );
  const content = cleanText([
    description,
    "Spotify podcast episode audio discussion",
    target.type === "show" ? "Spotify podcast show" : "Spotify podcast episode",
  ].filter(Boolean).join(" "), 1800);
  if (!title || !canonicalUrl) return null;
  if (!textMatchesKeyword({ title, content }, keyword)) return null;
  if (!textHasPodcastContext({ title, content })) return null;
  if (!isAfterSince(publishedAt, since)) return null;
  const item = {
    url: normalizeUrl(canonicalUrl) || target.directUrl,
    title,
    content,
    author: cleanText(htmlMetaContent(html, ["music:musician", "author", "article:author"]) || "Spotify Podcasts", 160),
    publishedAt,
    metrics: {
      ...
      spotify_podcast_direct_url: target.directUrl,
      spotify_podcast_direct_original_url: target.originalUrl,
      spotify_podcast_id: target.spotifyId,
      spotify_podcast_type: target.type,
      podcast_track_id: target.type === "episode" ? target.spotifyId : "",
      podcast_collection_id: target.type === "show" ? target.spotifyId : "",
      podcast_collection: target.type === "show" ? title : "",
      podcast_artist: "Spotify Podcasts",
      podcast_genre: "Podcast",
      audio_url: target.directUrl,
      direct_original_source_recovery: 1,
      spotify_podcast_html_meta_title_signal: title ? 1 : 0,
      spotify_podcast_html_meta_description_signal: description ? 1 : 0,
    },
  };
  Object.assign(item.metrics, applePodcastAudioRiskSignals(item));
  return item;
}
...
export async function scrapeApplePodcastSearch(keywords, { proxyUrl = "", budget = {}, since = "", country = "us", language = "en_us", discoveredPodcastShows = [], discovered_podcast_shows = [], directUrls = [], direct_urls = [], domainControls = {}, contentControls = {} } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedShows = normalizeDiscoveredPodcastShows([...discoveredPodcastShows, ...discovered_podcast_shows]);
  const rawDirectUrls = [...(
    Array.isArray(directUrls) ? directUrls : String(directUrls || "").split(/[\s，、]+/)
  ), ...(
    Array.isArray(direct_urls) ? direct_urls : String(direct_urls || "").split(/[\s，、]+/)
  )];
  const directTargets = normalizeApplePodcastDirectUrls(rawDirectUrls);
  const spotifyDirectTargets = normalizeSpotifyPodcastDirectUrls(rawDirectUrls);
  if (!normalizedKeywords.length && !normalizedShows.length && !directTargets.length && !spotifyDirectTargets.length) return scraperResult(0);
  const { maxItemsPerKeyword, maxPagesPerKeyword } = normalizeBudget(budget);
  let inserted = 0;
  const failures = [];
  const seenItemUrls = new Set();

  for (const target of spotifyDirectTargets) {
    try {
      const res = await fetchPublicSource(target.directUrl, {
        headers: {
      ...
      if (!res.ok) {
        failures.push({ target: `spotify-podcast-direct:${target.spotifyId}`, message: httpFailure(res) });
        continue;
      }
      const html = await res.text();
      const lookupKeywords = normalizedKeywords.length ? normalizedKeywords : [target.spotifyId];
      for (const keyword of lookupKeywords) {
        const item = parseSpotifyPodcastHtml(html, keyword, target, { since });
        if (!item) continue;
        inserted += await insertApplePodcastItems([item], {
          keyword,
          domainControls,
          contentControls,
          seenItemUrls,
      ...
      failures.push({ target: `spotify-podcast-direct:${target.spotifyId}`, message });
      console.warn(`[CRM/ApplePodcastSearch] Spotify 直鏈查詢失敗 id=${target.spotifyId}: ${message}`);
    }
  }

  for (const target of directTargets) {
    const lookupCountry = target.country || country;
    const url = buildApplePodcastLookupUrl(target.lookupId, {
    ...
    try {
      ...
      if (!res.ok) {
        failures.push({ target: `itunes-direct-lookup:${target.lookupId}`, message: httpFailure(res) });
        continue;
      }
      const payload = await res.json();
      const rawResultCount = Array.isArray(payload?.results) ? payload.results.length : Number(payload?.resultCount || 0);
      const lookupKeywords = normalizedKeywords.length ? normalizedKeywords : [target.directUrl].filter(Boolean);
      for (const keyword of lookupKeywords) {
        const items = parseApplePodcastResults(payload, keyword, { limit: maxItemsPerKeyword, since }).map(item => ({
          ...
          metrics: {
            ...
            apple_podcast_direct_url: target.directUrl,
            apple_podcast_direct_original_url: target.originalUrl,
            apple_podcast_direct_lookup_id: target.lookupId,
            apple_podcast_direct_collection_id: target.collectionId,
            apple_podcast_direct_track_id: target.trackId,
      ...
      const message = formatSourceError(err, proxyUrl);
      failures.push({ target: `itunes-direct-lookup:${target.lookupId}`, message });
      console.warn(`[CRM/ApplePodcastSearch] 直鏈查詢失敗 lookupId=${target.lookupId}: ${message}`);
    }
  }
...
export const __test__ = {
  ...
  normalizeApplePodcastDirectUrls,
  normalizeSpotifyPodcastDirectUrls,
  normalizeBudget,
  parseSpotifyPodcastHtml,

...
function normalizeYouTubeDirectUrls(values = [], limit = 20) {
  const out = [];
  for (const raw of Array.isArray(values) ? values : []) {
    const url = normalizeYouTubeDedupeUrl(raw);
    const videoId = extractVideoIdFromUrl(url);
    if (!videoId || out.includes(url)) continue;
    out.push(url);
...
async function fetchYouTubeDirectVideo(url = "", { proxyUrl = "", keyword = "", maxComments = MAX_YOUTUBE_COMMENTS, maxRelated = MAX_RELATED_VIDEOS, since = "" } = {}) {
  const canonicalUrl = normalizeYouTubeDedupeUrl(url);
  const videoId = extractVideoIdFromUrl(canonicalUrl);
  if (!videoId) return null;
  const res = await fetchPublicSource(canonicalUrl, {
    headers: {
      "User-Agent": USER_AGENT,
      "Accept": "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8",
      "Accept-Language": "zh-TW,zh-Hant;q=0.9,en;q=0.8",
  ...
  const title = metaContent(html, ["og:title", "twitter:title", "name"]) || stripTags((html.match(/<title[^>]*>([\s\S]*?)<\/title>/i) || [])[1] || "", 240);
  const content = metaContent(html, ["og:description", "twitter:description", "description"]);
  const author = metaContent(html, ["author"]) || stripTags((html.match(/"ownerChannelName"\s*:\s*"([^"]+)"/i) || [])[1] || "", 120) || "YouTube";
  const publishedAt = normalizePublishedAt(metaContent(html, ["datePublished", "uploadDate"]) || new Date().toISOString());
  ...
  const item = {
    ...
    content,
    author,
    channelId: channelIdFromText(html),
    videoId,
    publishedAt,
  };
  const context = parseYouTubeWatchContext(html, item, { keyword, maxComments, maxRelated, since });
  return { item, context, html };
}
...
export async function scrapeYouTube(keywords, { proxyUrl = "", budget = {}, deepBudget = null, since = "", domainControls = {}, contentControls = {}, directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeYouTubeDirectUrls(directUrls, 20);
  ...
  const directKeyword = normalizedKeywords[0] || "youtube-direct-url";
  for (const directUrl of normalizedDirectUrls) {
    try {
      const remainingItems = Math.max(0, maxItems - inserted - 1);
      const direct = await fetchYouTubeDirectVideo(directUrl, {
        proxyUrl,
      ...
      const result = insertSentimentItem({
        ...
        evidence: {
          ...
          metrics: {
            ...
            related_video_count: direct.context.relatedVideos.length,
            ...
            youtube_direct_url: directUrl,
            ...youtubeKeywordDiagnostics(item, directKeyword),
            ...youtubeVideoSpreadSignals(item, {
              comments,
              relatedVideos: direct.context.relatedVideos,
              rawFeedEntryCount: 0,
              keyword: directKeyword,
            }),
          },
      ...
      for (const related of direct.context.relatedVideos || []) {
        if (inserted >= maxItems) break;
        const relatedDedupeKey = youtubeVideoDedupeKey(related);
        if (!relatedDedupeKey || seenVideoKeys.has(relatedDedupeKey)) continue;
        ...
        const relatedResult = insertSentimentItem({
          ...
          evidence: {
            ...
            metrics: {
              ...
              ...youtubeKeywordDiagnostics(related, directKeyword),
              youtube_canonical_dedupe_url: normalizeYouTubeDedupeUrl(related.url),
      ...
      failures.push({ keyword: directKeyword, url: directUrl, message });
      console.warn(`[Sentiment/YouTube] 直抓影片失敗 url=${directUrl}: ${message}`);
    }
  }

...
function normalizeLinkedInDirectUrls(values = [], limit = 20) {
  const out = [];
  const seen = new Set();
  for (const value of Array.isArray(values) ? values : []) {
    const url = normalizeLinkedInDedupeUrl(value);
    if (!url || !isConcreteLinkedInUrl(url) || seen.has(url)) continue;
    seen.add(url);
    out.push({
      url,
      originalUrl: normalizeLinkedInUrl(value) || url,
      evidenceKind: linkedinEvidenceKind(url),
      dedupeKey: url,
    });
...
function extractMetaContent(html = "", names = []) {
  ...
  for (const name of names) {
    ...
    const match = propertyFirst.exec(source) || contentFirst.exec(source);
    if (match?.[1]) return stripTags(match[1], 1000);
  }
  return "";
}
...
function extractTitle(html = "") {
  ...
  return title?.[1] ? stripTags(title[1], 240) : "";
}

function parseLinkedInDirectPage(html = "", direct = {}, keyword = "") {
  const url = normalizeLinkedInDedupeUrl(direct.url || direct.originalUrl || "");
  if (!url || !isConcreteLinkedInUrl(url)) return null;
  const evidenceKind = direct.evidenceKind || linkedinEvidenceKind(url);
  const title = extractMetaContent(html, ["og:title", "twitter:title"])
    || extractTitle(html)
    || `LinkedIn ${evidenceKind || "page"} ${url}`;
  const content = extractMetaContent(html, ["og:description", "description", "twitter:description"])
    || stripTags(html, 1800)
    || title;
  const item = {
    url,
    title,
    content,
    author: extractMetaContent(html, ["article:author", "author"]) || "LinkedIn 直达公开页",
    ...
    metrics: {
      ...
      direct_url: url,
      linkedin_direct_url: url,
      linkedin_original_direct_url: direct.originalUrl || direct.url || url,
...
async function insertLinkedInItems(items, { keyword, proxyUrl = "", enrich = true, maxDeepPages = 0, domainControls = {}, contentControls = {}, seenItemUrls = null }) {
  ...
  for (const item of items) {
    ...
    const result = insertSentimentItem({
      ...
      evidence: {
        ...(enriched.evidence || {}),
        evidence_type: enriched.evidence?.evidence_type || (finalMetrics.collection_mode === "linkedin_direct_url"
          ? (finalMetrics.linkedin_evidence_kind === "post" ? "linkedin_direct_post" : "linkedin_direct_page")
          : linkedinEvidenceType(finalMetrics.linkedin_evidence_kind)),
        metrics: {
          ...finalMetrics,
...
export async function scrapeLinkedInSearch(keywords, { proxyUrl = "", enrich = true, budget = {}, deepBudget = null, since = "", domainControls = {}, contentControls = {}, directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeLinkedInDirectUrls(directUrls);
  ...
  for (const keyword of (normalizedKeywords.length ? normalizedKeywords : [""])) {
    ...
    for (const direct of normalizedDirectUrls) {
      ...
      try {
        ...
        const item = parseLinkedInDirectPage(html, direct, keyword);
        if (!item || !isAfterSince(item.publishedAt, since)) continue;
        const count = await insertLinkedInItems([item], {
          keyword,
          proxyUrl,
          enrich: false,
          maxDeepPages: 0,
        ...
        console.warn(`[Sentiment/LinkedIn] 直达公开页抓取失敗 url=${direct.url}: ${message}`);
      }
    }
...
export const __test__ = {
  ...
  normalizeLinkedInDirectUrls,
  normalizeLinkedInUrl,
  parseLinkedInDirectPage,

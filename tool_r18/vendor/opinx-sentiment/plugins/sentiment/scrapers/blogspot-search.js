...
function normalizeBlogspotDirectUrls(values = [], limit = 20) {
  ...
  for (const value of raw) {
    const normalized = normalizeBlogspotUrl(value);
    if (!normalized || !isConcreteBlogspotUrl(normalized)) continue;
    const dedupe = normalizeBlogspotDedupeUrl(normalized);
...
function blogspotTitleFromUrl(url = "") {
  try {
    const parsed = new URL(url);
    const segments = parsed.pathname.split("/").filter(Boolean);
    ...
    const title = stripTags(String(slug || "")
      ...
      .replace(/\s+/g, " ")
      .trim(), 240);
    return title || "Blogspot blog post";
  } catch {
    return "Blogspot blog post";
  }
}

function blogspotTitleFromHtml(html = "", fallback = "") {
...
async function insertBlogspotItems(items, {
  ...
  for (const item of items) {
    ...
    const title = blogspotTitleFromHtml(enriched.raw_html || "", item.title) || item.title;
    ...
    const result = insertSentimentItem({
      ...
      keyword,
      keywords: [keyword, ...blogspotKeywordNeedles(keyword)],
      published_at: enriched.published_at || item.publishedAt,
      ai_summary: enriched.ai_summary || content,
      raw_html: enriched.raw_html || "",
      evidence: {
        ...(enriched.evidence || {}),
        evidence_type: item.evidenceType || enriched.evidence?.evidence_type || "blogspot_public_blog_search_result",
        source_key: "blogspotSearch",
        metrics: {
          ...finalMetrics,
...
export async function scrapeBlogspotSearch(keywords, { proxyUrl = "", enrich = true, budget = {}, deepBudget = null, since = "", domainControls = {}, contentControls = {}, directUrls = [] } = {}) {
  const normalizedKeywords = Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  const normalizedDirectUrls = normalizeBlogspotDirectUrls(directUrls, 20);
  ...
  if (normalizedDirectUrls.length) {
    const directKeyword = normalizedKeywords[0] || "blogspot-direct-url";
    const directItems = normalizedDirectUrls.map(url => ({
      url,
      title: blogspotTitleFromUrl(url),
      content: "",
      author: "Blogspot/Blogger 直達文章",
      publishedAt: new Date().toISOString(),
      evidenceType: "blogspot_direct_blog_post",
      metrics: {
        source_key: "blogspotSearch",
        source_family: "knowledge",
        source_kind: "blogspot_direct_url",
        collection_mode: "blogspot_direct_url",
        deep_collector: "blogspot-direct-url",
        source: "blogspot_direct_blog_post",
        direct_url: url,
        blogspot_direct_url: url,
        blogspot_evidence_kind: blogspotEvidenceKind(url),
      },
    }));
    try {
      inserted += await insertBlogspotItems(directItems, {
      ...
      failures.push({ keyword: directKeyword, target: "blogspot-direct-url", message });
      console.warn(`[Sentiment/Blogspot] 直達文章抓取失敗 keyword=${directKeyword}: ${message}`);
    }
  }
...
export const __test__ = {
  ...
  normalizeBlogspotDirectUrls,
  blogspotSearchDedupeKey,
  blogspotEvidenceKind,
  blogspotTitleFromHtml,
  blogspotTitleFromUrl,

...
function expandFeedsForKeywords(feeds = [], keywords = [], { maxKeywordsPerFeed = 8 } = {}) {
  ...
  for (const feed of normalizedFeeds) {
    ...
    for (const keyword of normalizedKeywords) {
      ...
      expanded.push({
        ...
        tags: [...new Set([...(feed.tags || []), "keyword-search"])],
      });
    }
  }
...
function parseRssFeedItems(xml, feedOrName, { maxItems = DEFAULT_MAX_ITEMS_PER_FEED } = {}) {
  ...
  for (const block of blocks) {
    ...
    const url = rssDescriptionArticleUrl(descriptionRaw, feedUrl);
    const aggregatorUrl = url !== feedUrl && isRssAggregatorNewsUrl(feedUrl) ? feedUrl : "";
    const sourceUrl = rssSourceUrl(block);
    ...
    const author = tagValue(block, "dc:creator")
      ...
      || sourceName
      || feedName;
    const categories = allTagValues(block, "category");
    const mediaUrl = rssMediaUrl(block);
...
function rssEvidenceDepthProfile(item = {}, { enrichmentMetrics = {}, narrativeSignals = {}, feed = {}, sourceWeightTier = "" } = {}) {
  ...
  if (item.comments_url || Number(enrichmentMetrics.engagement_comment_count || 0) > 0) {
    score += 6;
...
export async function scrapeRssFeeds(keywords, { proxyUrl = "", enrich = true, feeds = [], feedPacks = ["chineseNews", "consumerProtection", "regulatoryNotices"], budget = {}, since = "", cursor = {}, conditionalRequests = true, domainControls = {}, contentControls = {}, failoverAttribution = [], metricsEnhancer = null } = {}) {
  ...
  if (!normalizedFeeds.length) return scraperResult(0);

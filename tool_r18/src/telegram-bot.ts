...
function normalizeTelegramChatIdInput(text: string): string | null {
  const value = normalizeTelegramSingleLine(text).replace(/[，。；;]+$/g, "");
  if (!/^-?\d+$/.test(value)) return null;
  return value;
}

function telegramChatTitle(chat: any, fallback: string): string {
  return normalizeTelegramSingleLine(chat?.title || chat?.username || chat?.first_name || fallback) || fallback;
}

function isTelegramGroupContentType(value: unknown): value is TelegramGroupContentType {
  return value === "free" || value === "paid";
}

function resolveKnownTelegramGroupName(groupId: string, groupContentType?: TelegramGroupContentType): string {
  if (groupId === "-1003703865776") return "TG fufei qun";
  if (groupId === "-1003812332642") return "TG群測試";
  return groupContentType === "paid" ? "TG付費群" : groupContentType === "free" ? "TG免費群" : groupId;
}

function formatTelegramGroupBindingDisplay(name?: string, id?: string): string {
  const displayName = normalizeTelegramSingleLine(name || "") || "未綁定";
  const displayId = normalizeTelegramSingleLine(id || "");
  return displayId ? `${displayName}（ID：${displayId}）` : displayName;
}
...
function buildPostImageCandidateKeyboard(args: {
  ...
  if (args.postId) rows.push([{ text: "\uD83D\uDCDD \u67E5\u770B\u63A8\u6587\u5217\u8868", callback_data: buildStoredPostsPageCallback(args.archiveId, listPage) }]);
  else rows.push([{ text: "◀️ 返回人設詳情", callback_data: `pd_${args.archiveId}` }]);
    ...
    for (const entry of entries) {
      ...
      if (
        ...
        map.set(chatId, {
          ...
          contentBranch: state.contentBranch === "r18" || state.contentBranch === "nonr18" ? state.contentBranch : undefined,
          contentTimeSlot: state.contentTimeSlot === "morning" || state.contentTimeSlot === "night" ? state.contentTimeSlot : undefined,
          prompt: typeof state.prompt === "string" ? state.prompt : undefined,
          imageAspectRatio: typeof state.imageAspectRatio === "string" ? state.imageAspectRatio : undefined,
          imageRatioId: typeof state.imageRatioId === "string" ? state.imageRatioId : undefined,
          imageRatioLabel: typeof state.imageRatioLabel === "string" ? state.imageRatioLabel : undefined,
          imageWidth: Number.isFinite(Number(state.imageWidth)) ? Number(state.imageWidth) : undefined,
          imageHeight: Number.isFinite(Number(state.imageHeight)) ? Number(state.imageHeight) : undefined,
          selectedMemoryEntryIds: Array.isArray(state.selectedMemoryEntryIds) ? state.selectedMemoryEntryIds.map(String) : undefined,
          selectedMemorySummaries: Array.isArray(state.selectedMemorySummaries) ? state.selectedMemorySummaries.map(String) : undefined,
          memoryOptions: Array.isArray(state.memoryOptions) ? state.memoryOptions : undefined,
          memoryPage: Number.isFinite(Number(state.memoryPage)) ? Math.max(0, Math.floor(Number(state.memoryPage))) : undefined,
          stage: state.stage,
        });
      }
    }
...
type PendingToolR18TaskState = {
  stage: "await_agent_input" | "await_task_input" | "await_prompt_review" | "await_prompt_adjustment" | "await_custom_prompt_submit" | "await_paid_post_count" | "video_i2v_resolution" | "video_i2v_duration" | "video_i2v_image" | "video_i2v_audio" | "video_i2v_prompt_mode" | "video_i2v_prompt" | "image_edit_input" | "image_edit_reference" | "image_edit_prompt_mode" | "image_edit_prompt" | "face_swap_target" | "face_swap_source" | "face_swap_prompt_mode" | "face_swap_prompt";
...
type PendingPaidR18ImageFirstGroupFlow = {
  archiveId: string;
  archiveName: string;
  total: number;
  currentIndex: number;
...
export function parsePersonaContentTypeCallback(data: string): {
  ...
  const match = data.match(/^(posts|history|pub)_(.+)_ct_(free|paid)$/);
  if (!match) return null;
  return {
    target: match[1] === "pub" ? "publish" : match[1] as PersonaContentTypeTarget,
    archiveId: match[2],
...
function buildStoredPostsPageCallback(archiveId: string, page: number, groupContentType?: TelegramGroupContentType) {
  const suffix = groupContentType ? `_ct_${groupContentType}` : "";
...
export function buildListPaginationRows(args: {
  ...
  const totalPages = Math.max(1, Math.floor(args.totalPages || 1));
  const safePage = Math.min(Math.max(0, Math.floor(args.page || 0)), totalPages - 1);
  if (totalPages <= 1) return [];
  const rows: Array<Array<{ text: string; callback_data: string }>> = [];
  if (safePage > 0) {
    rows.push([
      { text: "⏮ 首頁", callback_data: args.callbackForPage(0) },
      { text: "◀️ 上一頁", callback_data: args.callbackForPage(safePage - 1) },
    ]);
  }
  rows.push([{ text: `${safePage + 1}/${totalPages}`, callback_data: args.callbackForPage(safePage) }]);
  if (safePage < totalPages - 1) {
    rows.push([
      { text: "下一頁 ▶️", callback_data: args.callbackForPage(safePage + 1) },
      { text: "尾頁 ⏭", callback_data: args.callbackForPage(totalPages - 1) },
    ]);
  }
  return rows;
}
...
function buildPersonaHistoryCallback(archiveId: string, groupContentType?: TelegramGroupContentType, page = 0) {
  const suffix = groupContentType ? `_ct_${groupContentType}` : "";
  const pageSuffix = page > 0 ? `_p${Math.max(0, page)}` : "";
  return `history_${archiveId}${suffix}${pageSuffix}`;
}

function buildPersonaPublishCallback(archiveId: string, groupContentType?: TelegramGroupContentType) {
  const suffix = groupContentType ? `_ct_${groupContentType}` : "";
  return `pub_${archiveId}${suffix}`;
}

function parsePersonaScopedCallback(data: string, prefix: "history_" | "pub_") {
  if (!data.startsWith(prefix)) return null;
  const payload = data.slice(prefix.length);
  const pageMatch = payload.match(/^(.*)_p(\d+)$/);
  const rawArchiveId = pageMatch ? pageMatch[1] : payload;
  const branchMatch = rawArchiveId.match(/^(.*)_ct_(free|paid)$/);
  return {
    archiveId: branchMatch ? branchMatch[1] : rawArchiveId,
    ...(branchMatch ? { groupContentType: branchMatch[2] as TelegramGroupContentType } : {}),
    page: pageMatch ? Math.max(0, Number(pageMatch[2] || 0)) : 0,
  };
}

function normalizeTelegramGroupContentType(value: unknown): TelegramGroupContentType | undefined {
  return value === "paid" ? "paid" : value === "free" ? "free" : undefined;
}

function contentBranchToTelegramGroupContentType(branch?: GeneratePostContentBranch): TelegramGroupContentType | undefined {
  return branch === "r18" ? "paid" : branch === "nonr18" ? "free" : undefined;
}

function telegramGroupContentTypeToContentBranch(groupContentType?: TelegramGroupContentType): GeneratePostContentBranch | undefined {
  return groupContentType === "paid" ? "r18" : groupContentType === "free" ? "nonr18" : undefined;
}

function belongsToTelegramGroupContentType(item: { telegramGroupContentType?: "free" | "paid" } | null | undefined, groupContentType?: TelegramGroupContentType) {
  if (!groupContentType) return true;
  const itemType = normalizeTelegramGroupContentType(item?.telegramGroupContentType);
  if (groupContentType === "paid") return itemType === "paid";
  return itemType !== "paid";
}

function filterByTelegramGroupContentType<T extends { telegramGroupContentType?: "free" | "paid" }>(items: T[] = [], groupContentType?: TelegramGroupContentType): T[] {
  return groupContentType ? items.filter((item) => belongsToTelegramGroupContentType(item, groupContentType)) : items;
}

function countByTelegramGroupContentType<T extends { telegramGroupContentType?: "free" | "paid" }>(items: T[] = []) {
  return items.reduce((acc: Record<TelegramGroupContentType, number>, item) => {
    const type: TelegramGroupContentType = item.telegramGroupContentType === "paid" ? "paid" : "free";
    acc[type] += 1;
    return acc;
  }, { free: 0, paid: 0 });
}
...
export function buildStoredPostsListView(
  ...
  return {
    ...
    postIds: scopedPosts.map((post) => post.id),
    text: `<b>\uD83D\uDCDD \u5F85\u767C\u4F48\u63A8\u6587\u5217\u8868</b>${branchTitle ? ` - ${branchTitle}` : ""}\uFF08\u5171 ${scopedPosts.length} \u7BC7\uFF0C\u7B2C ${safePage + 1}/${totalPages} \u9801\uFF09\n\n${lines.join("\n\n")}`,
    keyboard,
  };
}
...
function buildBulkStoredPostsSelectView(args: {
  ...
  const scopedPosts = filterByTelegramGroupContentType(args.posts, args.groupContentType);
  const totalPages = Math.max(1, Math.ceil(scopedPosts.length / pageSize));
...
function resolveTelegramGroupContentTypeForPost(
  ...
  if (post?.telegramGroupContentType === "paid") return "paid";
  if (post?.telegramGroupContentType === "free") return "free";
...
function resolveTelegramTargetForGroupContent(
  ...
  const resolvedType: TelegramGroupContentType = groupContentType === "paid" ? "paid" : "free";
  const chatId = normalizeTelegramSingleLine(
    resolvedType === "paid" ? archive?.boundTelegramPaidGroupId || "" : archive?.boundTelegramFreeGroupId || "",
  );
  const groupName = normalizeTelegramSingleLine(
...
function conciseTaskFailureReason(rawMessage: string): string | null {
  const text = String(rawMessage || "").replace(/\s+/g, " ").trim();
  if (!text) return null;
  if (/(PROHIBITED_CONTENT|content.*policy|content.*moderation|内容审核|內容審核|安全策略|safety.*block|blocked.*safety)/i.test(text)) {
    return "上游內容審核攔截";
  }
  if (/(402|Payment Required|insufficient.*(balance|credit|quota)|quota.*exceeded|余额不足|餘額不足|额度不足|額度不足)/i.test(text)) {
    return "上游 Grok/LLM 額度不足";
  }
  if (/(workflow.*404|404.*workflow|read workflow.*404|讀取工作流失敗\s*404|读取工作流失败\s*404|ComfyUI.*404)/i.test(text)) {
    return "遠端 ComfyUI 工作流 404";
  }
  if (/(ComfyUI.*(refused|拒絕連線|拒绝连接|不可用)|Connection refused|WinError 10061|ECONNREFUSED|127\.0\.0\.1:19000|127\.0\.0\.1:9001)/i.test(text)) {
    return "4090 ComfyUI/網關不可用";
  }
  if (/(only\s+\d+\s*\/\s*4\s+images generated|QA.*(未滿|未满|0\/4|不足)|通過\s*0\/4|通过\s*0\/4|未滿 Telegram 回傳數量要求|未满 Telegram 回传数量要求)/i.test(text)) {
    return "可用圖片未滿 4 張";
  }
  if (/(502\s+Bad Gateway|HTTP\s+502|Bad Gateway)/i.test(text)) {
    return "遠端網關 502";
  }
  if (/(504|timeout|timed out|超時|超时)/i.test(text)) {
    return "等待生成超時";
  }
  if (/(fetch failed|Failed to fetch|NetworkError|ENOTFOUND|EAI_AGAIN|ECONNRESET)/i.test(text)) {
    return "後台或遠端網關連線失敗";
  }
  if (/(chat not found|400 Bad Request.*chat|Bad Request: chat not found)/i.test(text)) {
    return "TG 群 ID 無法訪問";
  }
  if (/(500\s+Internal Server Error|HTTP\s+500|Internal Server Error)/i.test(text)) {
    return "後台內部錯誤，請查看後台日誌";
  }
  return null;
}
...
export function formatUserFacingError(errorOrMessage: unknown, fallback = "操作失败，请稍后重试。"): string {
  ...
  const conciseReason = conciseTaskFailureReason(rawForMatch);
  let message = raw
    .replace(/^Error:\s*/i, "")
  ...
  if (conciseReason) {
    ...
    message = /R18 後台連線失敗|Tool_R18 後台連線失敗/i.test(message)
      ? message
      : "Tool_R18 後台連線失敗，請確認本地後台服務和端口配置正常後重試。";
...
async function runPadThreadsAutoReplyFromTelegram(
  bot: TelegramBot,
  chatId: number,
  padCode: string,
  maxAgeDays = 2,
...
async function runPersonaThreadsAutoReplyFromTelegram(
  bot: TelegramBot,
  chatId: number,
  archiveId: string,
  maxAgeDays = 2,
  ...
  try {
    ...
    for (let i = 0; i < screenshots.length; i += 1) {
      ...
      await bot.sendPhoto(chatId, resolveTelegramPhotoInput(screenshots[i]), {
        caption: `📳 Threads 自動回覆證據 ${i + 1}/${screenshots.length}`,
      }).catch(() => undefined);
    }
  } catch (error: any) {
    stopTyping();
    ...
    if (await sendCloudAccountStateNotice(
      ...
      chatId,
      error,
      { action: "Threads自動回覆", padName: boundPad.padName, padCode: boundPad.padCode },
    ...
    await sendManualInterventionFailure(bot, chatId, error, {
      ...
      fallback: "Threads 自動回覆失敗，請人工檢查目前 Threads 頁面後重試。",
      inlineKeyboard: [[{ text: "◀️ 返回 Threads 帳號", callback_data: returnCallback }]],
    });
  } finally {
...
function buildPersonaPlatformAccountRows(archiveId: string, platform: PersonaAccountPlatform, status?: PersonaAccountRuntimeStatus, archive?: PersonaArchive): Array<Array<{ text: string; callback_data: string }>> {
  ...
  if (platform === "threads") {
    ...
    if (boundPadCode) {
      rows.push(
        [{ text: "🔄 切換登入帳號", callback_data: `acctswitch_threads_${archiveId}` }],
        ...(status?.state === "logged_in" ? [[{ text: "🚪 登出 Threads", callback_data: `acctlogout_threads_${archiveId}` }]] : []),
        [
          { text: "🔗 Threads 簡介新增連結", callback_data: `acctprofile_link_${archiveId}` },
          { text: "📝 修改 Threads 簡介", callback_data: `acctprofile_bio_${archiveId}` },
        ],
        [
    ...
    const actionRows: Array<Array<{ text: string; callback_data: string }>> = [
      [{ text: `🔍 檢測 ${platformLabel} 登入狀態`, callback_data: `acctquery_${platform}_${archiveId}` }],
    ];
    if (status?.state === "logged_in") {
      ...
      actionRows.push([{ text: `🔐 登入 ${platformLabel}`, callback_data: `acctlogin_${platform}_${archiveId}` }]);
    }
    rows.push(
      [{ text: "✍️ 設定 Telegram 登入資料", callback_data: `acctset_${platform}_${archiveId}` }],
      [{ text: "🧹 清除已保存登入資料", callback_data: `acctclear_${platform}_${archiveId}` }],
      ...actionRows,
    );
  }
...
function parseGeneratePostModeCallback(data: string): {
  ...
  return {
    archiveId: match[1],
    mode: match[2] as "textonly" | "withimage",
    ...(match[3] ? { contentBranch: match[3] as GeneratePostContentBranch } : {}),
  };
}

function buildGeneratePostCustomCallback(archiveId: string, contentBranch?: GeneratePostContentBranch) {
  const suffix = contentBranch ? `_ct_${contentBranch}` : "";
  return `genpost_custom_${archiveId}${suffix}`;
}

function parseGeneratePostCustomCallback(data: string): { archiveId: string; contentBranch?: GeneratePostContentBranch } | null {
  if (!data.startsWith("genpost_custom_")) return null;
  const payload = data.slice("genpost_custom_".length);
  const match = payload.match(/^(.*)_ct_(nonr18|r18)$/);
  return {
    archiveId: match ? match[1] : payload,
    ...(match ? { contentBranch: match[2] as GeneratePostContentBranch } : {}),
  };
}

function buildGeneratePostHotCallback(archiveId: string, contentBranch?: GeneratePostContentBranch) {
  const branchToken = contentBranch === "r18" ? "r" : contentBranch === "nonr18" ? "n" : "x";
  return `gph_${branchToken}_${archiveId}`;
}

function parseGeneratePostHotCallback(data: string): { archiveId: string; contentBranch?: GeneratePostContentBranch } | null {
  const match = data.match(/^gph_([nrx])_(.+)$/);
  if (!match?.[2]) return null;
  return {
    archiveId: match[2],
    contentBranch: match[1] === "r" ? "r18" : match[1] === "n" ? "nonr18" : undefined,
  };
}

function rememberSentimentHotAction(chatId: number, archiveId: string) {
  const key = crypto.randomBytes(6).toString("hex");
  sentimentHotActionKeys.set(key, { chatId, archiveId });
  if (sentimentHotActionKeys.size > 200) {
    const first = sentimentHotActionKeys.keys().next().value;
    if (first) sentimentHotActionKeys.delete(first);
  }
  return key;
}
...
async function sendGeneratePostModePickerForArchive(bot: TelegramBot, chatId: number, archiveId: string, messageId?: number, contentBranch?: GeneratePostContentBranch) {
  ...
  if (!archive) {
    await safeEditOrSend(bot, chatId, messageId, "人设不存在或已被删除，请重新选择。", {
      reply_markup: { inline_keyboard: [[{ text: "返回人设列表", callback_data: "list_personas" }]] },
    });
    return;
  }
  const scopedPosts = contentBranch
    ? filterByTelegramGroupContentType(archive.posts || [], contentBranchToTelegramGroupContentType(contentBranch))
    : archive.posts || [];
  const branchLine = contentBranch ? ["內容類型：" + generatePostContentBranchLabel(contentBranch)] : [];
  const backTo = contentBranch ? "genpost_branch_" + archiveId : "pd_" + archiveId;
  const backText = contentBranch ? "◀️ 返回內容類型" : "◀️ 返回人設詳情";
...
function pickPaidR18TimeSlot(): GeneratePostTimeSlot {
  return Math.random() < 0.5 ? "morning" : "night";
}

function ensurePaidR18TimeSlot(timeSlot?: GeneratePostTimeSlot): GeneratePostTimeSlot {
  return timeSlot === "morning" || timeSlot === "night" ? timeSlot : pickPaidR18TimeSlot();
}
...
function buildGenerateTimeSlotKeyboard(state: PendingGeneratePostState) {
  ...
  if (state.contentBranch === "r18") {
    rows.push([{ text: "圖生視頻", callback_data: "paidr18_group_video" }]);
  }
...
async function advanceGeneratePostAfterMemorySelection(
  bot: TelegramBot,
  chatId: number,
  ...
  const nextState = {
    ...state,
    contentTimeSlot: state.contentBranch === "r18" ? ensurePaidR18TimeSlot(state.contentTimeSlot) : state.contentTimeSlot,
    stage: "await_time_slot" as const,
  };
  setPendingGeneratePost(chatId, nextState);
  if (nextState.contentBranch === "r18") {
    await sendPaidR18EntryPicker(bot, chatId, messageId, nextState);
    return;
  }
...
function defaultToolR18UploadHostDir() {
  ...
  const projectRoot = path.basename(process.cwd()).toLowerCase() === "tool_r18"
    ...
    : fs.existsSync(path.join(process.cwd(), "tool_r18", "package.json"))
      ...
      : process.cwd();
  return path.join(path.dirname(projectRoot), "webapp_data", "tool_r18_uploads");
}

function defaultToolR18UploadContainerDir(hostDir: string) {
  if (process.platform === "win32") return hostDir.replace(/\\/g, "/");
  return "/app/webapp_data/tool_r18_uploads";
}

const TOOL_R18_UPLOAD_HOST_DIR = process.env.TOOL_R18_UPLOAD_HOST_DIR || defaultToolR18UploadHostDir();
const TOOL_R18_UPLOAD_CONTAINER_DIR = process.env.TOOL_R18_UPLOAD_CONTAINER_DIR || defaultToolR18UploadContainerDir(TOOL_R18_UPLOAD_HOST_DIR);
...
async function toolR18JsonRequest(method: "GET" | "POST", pathName: string, body?: any, query?: Record<string, string | number | boolean | undefined>) {
  ...
  let response: any;
  try {
    response = await fetchFn(url, { method, headers, body: method === "POST" ? JSON.stringify(body || {}) : undefined });
  } catch (error: any) {
    const detail = String(error?.message || error || "").trim();
    throw new Error(`Tool_R18 後台連線失敗：${url.origin}${url.pathname} 無法連線。${detail ? `原因：${detail}` : ""}`);
  }
...
function buildPaidR18ImageWorkflowKeyboard() {
  return [
    [{ text: "文生圖", callback_data: "paidr18_task_text_to_image" }, { text: "單圖編輯", callback_data: "paidr18_task_single_image_edit" }],
    [{ text: "圖片編輯", callback_data: "paidr18_task_get_nano_banana" }, { text: "人物換臉", callback_data: "paidr18_task_face_swap" }],
    [{ text: "◀️ 返回付費內容", callback_data: "paidr18_entry" }],
  ];
}

function buildPaidR18EntryKeyboard() {
  return [
    [{ text: "圖片生成", callback_data: "paidr18_group_image" }, { text: "圖生視頻", callback_data: "paidr18_group_video" }],
    [{ text: "◀️ 返回記憶選擇", callback_data: "paidr18_back_memory" }],
  ];
}

async function sendPaidR18EntryPicker(bot: TelegramBot, chatId: number, messageId: number | undefined, state: PendingGeneratePostState) {
  const contentTimeSlot = ensurePaidR18TimeSlot(state.contentTimeSlot);
  pendingPaidR18ImageFlows.set(chatId, {
    archiveId: state.archiveId,
    archiveName: state.archiveName,
    contentTimeSlot,
    selectedMemoryEntryIds: state.selectedMemoryEntryIds,
    selectedMemorySummaries: Array.isArray((state as any).selectedMemorySummaries) ? (state as any).selectedMemorySummaries.map(String) : selectedMemorySummariesFromState(state),
  });
  const selectedCount = state.selectedMemoryEntryIds?.length || 0;
  await safeEditOrSend(bot, chatId, messageId, [
    "✍️ *新建推文*",
    ...
    "",
    "請選擇付費內容類型：",
    "時間段提示詞會自動隨機寫入圖片與文案提示詞，不需要手動選擇。",
  ].join(String.fromCharCode(10)), {
    parse_mode: "Markdown",
    reply_markup: { inline_keyboard: buildPaidR18EntryKeyboard() },
  });
}

async function sendPaidR18ImageWorkflowPicker(bot: TelegramBot, chatId: number, messageId: number | undefined, state: PendingGeneratePostState) {
  const contentTimeSlot = ensurePaidR18TimeSlot(state.contentTimeSlot);
  pendingPaidR18ImageFlows.set(chatId, {
    archiveId: state.archiveId,
    archiveName: state.archiveName,
  ...
  await safeEditOrSend(bot, chatId, messageId, [
    ...
    "指定記憶：" + (selectedCount ? `${selectedCount} 條` : "不指定"),
    "",
    "請先選擇 R18 圖片工作流，按原流程一步一步生成圖片。",
    "圖片完成後，先選中要插入推文的圖片；選中後才會根據該人物圖片生成付費群文案。",
  ].join(String.fromCharCode(10)), {
    parse_mode: "Markdown",
    reply_markup: { inline_keyboard: buildPaidR18ImageWorkflowKeyboard() },
  });
}

function paidR18ImageFlowParams(flow: PendingPaidR18ImageFlowState, taskType: ToolR18TaskType) {
  return {
    r18_paid_post_context: {
      ...
      selectedMemorySummaries: flow.selectedMemorySummaries || [],
      taskType,
    },
    tg_content_branch: "r18",
    tg_prompt_safety_profile: "r18_paid",
    tg_suppress_auto_notify: true,
    source: "telegram-paid-r18-image-workflow",
  };
}

function hasPaidR18ImageFlowContext(params?: Record<string, any>) {
  return Boolean(String((params || {}).r18_paid_post_context?.archiveId || "").trim());
}

function paidR18GenerateStateFromParams(params?: Record<string, any>): PendingGeneratePostState | null {
...
function paidR18GenerateStateFromFlow(flow?: PendingPaidR18ImageFlowState): PendingGeneratePostState | null {
  if (!flow?.archiveId) return null;
  return {
    archiveId: flow.archiveId,
    archiveName: flow.archiveName,
    count: 1,
...
async function sendPaidR18ImageWorkflowFromContext(bot: TelegramBot, chatId: number, messageId?: number, includeSavedFlow = true, target: "entry" | "image" = "image") {
  const pendingState = pendingToolR18Tasks.get(chatId);
  const fromPending = paidR18GenerateStateFromParams(pendingState?.params);
  if (fromPending) {
    if (target === "entry") await sendPaidR18EntryPicker(bot, chatId, messageId, fromPending);
    else await sendPaidR18ImageWorkflowPicker(bot, chatId, messageId, fromPending);
    return true;
  }
  const fromGeneratePost = pendingGeneratePosts.get(chatId);
  if (fromGeneratePost?.contentBranch === "r18") {
    const restored = {
      ...fromGeneratePost,
      contentTimeSlot: ensurePaidR18TimeSlot(fromGeneratePost.contentTimeSlot),
      stage: "await_time_slot" as const,
    };
    pendingPaidR18ImageFlows.set(chatId, {
      archiveId: restored.archiveId,
      archiveName: restored.archiveName,
      contentTimeSlot: restored.contentTimeSlot,
      selectedMemoryEntryIds: restored.selectedMemoryEntryIds,
      selectedMemorySummaries: restored.selectedMemorySummaries,
    });
    setPendingGeneratePost(chatId, restored);
    if (target === "entry") await sendPaidR18EntryPicker(bot, chatId, messageId, restored);
    else await sendPaidR18ImageWorkflowPicker(bot, chatId, messageId, restored);
    return true;
  }
  if (!includeSavedFlow) return false;
  const fromFlow = paidR18GenerateStateFromFlow(pendingPaidR18ImageFlows.get(chatId));
  if (fromFlow) {
    setPendingGeneratePost(chatId, fromFlow);
    if (target === "entry") await sendPaidR18EntryPicker(bot, chatId, messageId, fromFlow);
    else await sendPaidR18ImageWorkflowPicker(bot, chatId, messageId, fromFlow);
    return true;
  }
  return false;
}

function toolR18FlowCancelCallback(params?: Record<string, any>, fallback = "toolr18_group_image") {
  return hasPaidR18ImageFlowContext(params) ? "paidr18_back_workflow" : fallback;
}

function toolR18ContextBackCallback(chatId: number, fallback = "toolr18_entry") {
  return hasActivePaidR18ImageFlowContext(chatId) ? "paidr18_back_workflow" : fallback;
}

function toolR18TaskBackLabel(chatId: number, params?: Record<string, any>) {
  return hasActivePaidR18ImageFlowContext(chatId, params) ? "◀️ 返回付費內容" : "◀️ 返回";
}

function hasActivePaidR18ImageFlowContext(chatId: number, params?: Record<string, any>) {
  if (hasPaidR18ImageFlowContext(params) || pendingPaidR18ImageFlows.has(chatId)) return true;
  const pendingGenerate = pendingGeneratePosts.get(chatId);
  if (pendingGenerate?.contentBranch === "r18") return true;
  const pendingTask = pendingToolR18Tasks.get(chatId);
  return hasPaidR18ImageFlowContext(pendingTask?.params);
}

function toolR18ContextBackButton(chatId: number, params?: Record<string, any>, fallback = "toolr18_entry") {
  const paidContext = hasActivePaidR18ImageFlowContext(chatId, params);
  return {
    text: paidContext ? "◀️ 返回付費內容" : "◀️ 返回",
    callback_data: paidContext ? "paidr18_back_workflow" : fallback,
  };
}

function toolR18ContextCancelButton(chatId: number, params?: Record<string, any>, fallback = "toolr18_entry") {
  const backButton = toolR18ContextBackButton(chatId, params, fallback);
  return { text: "取消", callback_data: backButton.callback_data };
}

function toolR18TaskContextParams(task: any): Record<string, any> | undefined {
  const candidates = [task?.input, task?.params, task?.payload, task?.request, task?.metadata];
  for (const candidate of candidates) {
    if (candidate && typeof candidate === "object" && hasPaidR18ImageFlowContext(candidate)) {
      return candidate;
    }
  }
  return undefined;
...
function restorePaidR18ImageFlowFromParams(chatId: number, params?: Record<string, any>) {
  const state = paidR18GenerateStateFromParams(params);
  if (!state) return false;
  const restored = {
    ...state,
    contentTimeSlot: ensurePaidR18TimeSlot(state.contentTimeSlot),
    stage: "await_time_slot" as const,
  };
  pendingPaidR18ImageFlows.set(chatId, {
    archiveId: restored.archiveId,
...
function buildToolR18SubmittedKeyboard(chatId: number, taskId: string, params?: Record<string, any>) {
  const rows = [[{ text: "查看任務", callback_data: "toolr18_detail_" + taskId }]];
  if (hasActivePaidR18ImageFlowContext(chatId, params)) {
    rows.push([{ text: "◀️ 返回付費內容", callback_data: "paidr18_back_workflow" }]);
  } else {
    rows.push([{ text: "繼續R18", callback_data: "toolr18_entry" }, { text: "工作臺狀態", callback_data: "toolr18_status" }]);
  }
  return rows;
}

function buildToolR18ResubmittedKeyboard(chatId: number, taskId: string, params?: Record<string, any>) {
  const rows = [[{ text: "查看任務", callback_data: "toolr18_detail_" + taskId }]];
  if (hasActivePaidR18ImageFlowContext(chatId, params)) {
    rows.push([{ text: "◀️ 返回付費內容", callback_data: "paidr18_back_workflow" }]);
  } else {
    rows.push([{ text: "最近任務", callback_data: "toolr18_tasks" }, { text: "繼續R18", callback_data: "toolr18_entry" }]);
  }
  return rows;
}

function buildPaidR18TimeVisualInstruction(contentTimeSlot?: unknown) {
  const slot = String(contentTimeSlot || "").trim();
  if (slot === "morning") {
    return "Paid group time slot: morning copy. The image must visibly read as morning or daytime: soft natural morning window light, clean bright room tone, fresh wake-up / morning greeting atmosphere, lighter color temperature, no deep-night darkness, neon, bar, or late-night mood.";
  }
  if (slot === "night") {
    return "Paid group time slot: night copy. The image must visibly read as night or late night: warm bedside lamp or low indoor light, bedroom/private room after dark, intimate late-night benefit atmosphere, deeper shadows and warmer color temperature, no bright daylight, morning window sun, or outdoor daytime mood.";
  }
  return "";
}

function buildPaidR18VisualContextInstruction(params?: Record<string, any>) {
  const source = params || {};
  const context = source.r18_paid_post_context || {};
  const archiveName = String(context.archiveName || "").trim();
  const timeInstruction = buildPaidR18TimeVisualInstruction(context.contentTimeSlot);
  const memories = Array.isArray(context.selectedMemorySummaries)
    ? context.selectedMemorySummaries.map((item: any) => String(item || "").trim()).filter(Boolean)
    : [];
  const persona = String(source.persona_label || source.persona_lora || "").trim();
  return [
    archiveName ? "Paid R18 content image for persona/archive: " + archiveName + ". The image is generated first, and the paid group copy will be written from the selected image afterward." : "",
    timeInstruction,
    memories.length ? "Selected memory context: " + memories.join(" | ") : "",
    persona ? "Matched persona LoRA: " + persona + ". Keep this persona identity and body style consistent." : "",
    archiveName ? "Make the visual details strong enough for later copywriting: clear scene, clothing, pose, mood, lighting, time period, and image-specific hooks." : "",
  ].filter(Boolean).join(String.fromCharCode(10));
}

async function startPaidR18ImageWorkflow(bot: TelegramBot, chatId: number, messageId: number | undefined, taskType: ToolR18TaskType) {
  const flow = pendingPaidR18ImageFlows.get(chatId);
  if (!flow) {
    await safeEditOrSend(bot, chatId, messageId, "付費群圖片流程已失效，請重新從「我的人設 → 新建推文 → 付費群內容」進入。", {
      reply_markup: { inline_keyboard: [[{ text: "◀️ 返回人設列表", callback_data: "list_personas" }]] },
    });
    return;
  }
  const seedParams = paidR18ImageFlowParams(flow, taskType);
  if (taskType === "text_to_image") {
    await sendToolR18TextToImageRatioStep(bot, chatId, messageId, seedParams);
  } else if (taskType === "video_i2v") {
    await sendToolR18VideoI2vResolutionStep(bot, chatId, messageId, seedParams);
  } else if (taskType === "single_image_edit" || taskType === "get_nano_banana") {
    await sendToolR18ImageEditInputStep(bot, chatId, messageId, taskType, seedParams);
  } else if (taskType === "face_swap") {
    await sendToolR18FaceSwapTargetStep(bot, chatId, messageId, seedParams);
  } else {
    await sendPaidR18ImageWorkflowPicker(bot, chatId, messageId, {
      archiveId: flow.archiveId,
      archiveName: flow.archiveName,
      count: 1,
      textOnly: false,
...
function buildToolR18VideoKeyboard(chatId?: number) {
  const backCallback = typeof chatId === "number" && pendingPaidR18ImageFlows.has(chatId)
    ? "paidr18_back_workflow"
    : "toolr18_entry";
  return [
    [toolR18TaskButton("圖生視頻")],
    [{ text: typeof chatId === "number" && pendingPaidR18ImageFlows.has(chatId) ? "◀️ 返回付費內容" : "◀️ 返回", callback_data: backCallback }],
  ];
}
...
function buildToolR18VideoI2vResolutionKeyboard(backCallback = "toolr18_group_video", backLabel = "◀️ 返回") {
  return [
...
function buildToolR18VideoI2vPromptModeKeyboard() {
  return [
    [{ text: "AI 自由發揮", callback_data: "toolr18_i2v_prompt_free" }],
...
async function sendToolR18VideoI2vResolutionStep(bot: TelegramBot, chatId: number, messageId?: number, seedParams?: Record<string, any>) {
  const existing = pendingToolR18Tasks.get(chatId);
  const params = { ...(await toolR18VideoI2vDefaultParams()), ...(existing?.params || {}), ...(seedParams || {}) };
  pendingToolR18Tasks.set(chatId, { ...(existing || {}), stage: "video_i2v_resolution", taskType: "video_i2v", taskLabel: "圖生視頻", params, files: existing?.files || [] });
  await safeEditOrSend(bot, chatId, messageId, toolR18VideoI2vStatusText("1/5 選擇分辨率", params), {
    reply_markup: { inline_keyboard: buildToolR18VideoI2vResolutionKeyboard(toolR18FlowCancelCallback(params, "toolr18_group_video"), hasPaidR18ImageFlowContext(params) ? "◀️ 返回付費內容" : "◀️ 返回") },
  });
}
...
async function sendToolR18VideoI2vPromptModeStep(bot: TelegramBot, chatId: number, messageId?: number) {
  ...
  await safeEditOrSend(bot, chatId, messageId, toolR18VideoI2vStatusText("5/5 選擇提示詞方式", params) + String.fromCharCode(10) + String.fromCharCode(10) + "請選擇 AI 自由發揮、讓 Grok 生成提示詞，或輸入自定義提示詞提交。", {
...
function buildToolR18ImageStepKeyboard(backCallback?: string, keepCurrent = false, cancelCallback = "toolr18_group_image") {
  const rows: any[] = [];
  ...
  rows.push([{ text: "取消", callback_data: cancelCallback }]);
  return rows;
}
...
function buildToolR18ImageEditPromptModeKeyboard() {
  return [
    ...
    [{ text: "AI 自由發揮", callback_data: "toolr18_imgedit_prompt_free" }],
...
async function sendToolR18ImageEditInputStep(bot: TelegramBot, chatId: number, messageId?: number, taskType?: ToolR18TaskType, seedParams: Record<string, any> = {}) {
  ...
  const params = { ...toolR18ImageEditParams(existing), ...seedParams };
  ...
  await safeEditOrSend(bot, chatId, messageId, toolR18ImageEditStatusText(meta.label, "1/" + meta.totalSteps + " 上傳原圖", params) + String.fromCharCode(10) + String.fromCharCode(10) + suffix, {
    reply_markup: { inline_keyboard: buildToolR18ImageStepKeyboard(undefined, Boolean(params.input_image_local_path), toolR18FlowCancelCallback(params, "toolr18_group_image")) },
  });
}
...
async function sendToolR18ImageEditReferenceStep(bot: TelegramBot, chatId: number, messageId?: number) {
  ...
  await safeEditOrSend(bot, chatId, messageId, toolR18ImageEditStatusText("圖片編輯", "2/4 上傳參考圖", params) + String.fromCharCode(10) + String.fromCharCode(10) + suffix, {
    reply_markup: { inline_keyboard: buildToolR18ImageStepKeyboard("toolr18_imgedit_back_input", Boolean(params.reference_image_local_path), toolR18FlowCancelCallback(params, "toolr18_group_image")) },
  });
}
...
function buildToolR18FaceSwapStepKeyboard(backCallback?: string, keepCurrent = false, cancelCallback = "toolr18_group_image") {
  const rows: any[] = [];
...
function buildToolR18FaceSwapPromptModeKeyboard() {
  return [
    ...
    [{ text: "AI 自由發揮", callback_data: "toolr18_faceswap_prompt_free" }],
    [{ text: "輸入自定義換臉要求", callback_data: "toolr18_faceswap_prompt_custom" }],
    [{ text: "◀️ 上一步", callback_data: "toolr18_faceswap_back_source" }],
  ];
}

async function sendToolR18FaceSwapTargetStep(bot: TelegramBot, chatId: number, messageId?: number, seedParams: Record<string, any> = {}) {
  const existing = pendingToolR18Tasks.get(chatId);
  const params = { ...toolR18FaceSwapParams(existing), ...seedParams };
  ...
  await safeEditOrSend(bot, chatId, messageId, toolR18FaceSwapStatusText("1/4 上傳原圖", params) + String.fromCharCode(10) + String.fromCharCode(10) + suffix, {
    reply_markup: { inline_keyboard: buildToolR18FaceSwapStepKeyboard(undefined, Boolean(params.target_image_local_path), toolR18FlowCancelCallback(params, "toolr18_group_image")) },
  });
}
...
async function sendToolR18FaceSwapSourceStep(bot: TelegramBot, chatId: number, messageId?: number) {
  ...
  await safeEditOrSend(bot, chatId, messageId, toolR18FaceSwapStatusText("2/4 上傳人臉參考圖", params) + String.fromCharCode(10) + String.fromCharCode(10) + suffix, {
    reply_markup: { inline_keyboard: buildToolR18FaceSwapStepKeyboard("toolr18_faceswap_back_target", Boolean(params.source_image_local_path), toolR18FlowCancelCallback(params, "toolr18_group_image")) },
  });
}
...
async function sendToolR18ContextOrMenu(bot: TelegramBot, chatId: number, messageId?: number, target: "entry" | "image" = "image") {
  if (await sendPaidR18ImageWorkflowFromContext(bot, chatId, messageId, true, target)) return;
  await sendToolR18Menu(bot, chatId, messageId);
}
...
async function sendToolR18Status(bot: TelegramBot, chatId: number, messageId?: number) {
  ...
  const backCallback = toolR18ContextBackCallback(chatId);
  await safeEditOrSend(bot, chatId, messageId, text, { reply_markup: { inline_keyboard: [[{ text: "查看最近任務", callback_data: "toolr18_tasks" }], [{ text: toolR18TaskBackLabel(chatId), callback_data: backCallback }]] } });
}
...
async function sendToolR18Tasks(bot: TelegramBot, chatId: number, messageId?: number) {
  ...
  await safeEditOrSend(bot, chatId, messageId, tasks.length ? ["R18 最近任務", "", tasks.map(formatToolR18TaskLine).join(String.fromCharCode(10) + String.fromCharCode(10))].join(String.fromCharCode(10)) : "R18 最近還沒有任務。", { reply_markup: { inline_keyboard: [...rows, [{ text: toolR18TaskBackLabel(chatId), callback_data: toolR18ContextBackCallback(chatId) }]] } });
}

function buildToolR18TaskDetailKeyboard(task: any, chatId: number) {
  const taskId = String(task?.id || "").slice(0, 42);
  const taskParams = toolR18TaskContextParams(task);
  const backCallback = toolR18ContextBackCallback(chatId, hasPaidR18ImageFlowContext(taskParams) ? "paidr18_back_workflow" : "toolr18_entry");
  const backLabel = toolR18TaskBackLabel(chatId, taskParams);
  if (String(task?.status || "") === "failed" && taskId) {
    return [
      [{ text: "\u91cd\u65b0\u63d0\u4ea4\u4efb\u52d9", callback_data: "toolr18_resubmit_" + taskId }],
      [{ text: "\u6700\u8fd1\u4efb\u52d9", callback_data: "toolr18_tasks" }, { text: backLabel, callback_data: backCallback }],
    ];
  }
  return [
    [{ text: "\u6700\u8fd1\u4efb\u52d9", callback_data: "toolr18_tasks" }, { text: backLabel, callback_data: backCallback }],
...
async function sendToolR18TaskDetail(bot: TelegramBot, chatId: number, taskId: string, messageId?: number) {
  ...
  restorePaidR18ImageFlowFromParams(chatId, toolR18TaskContextParams(task));
  ...
  await safeEditOrSend(bot, chatId, messageId, text, { reply_markup: { inline_keyboard: buildToolR18TaskDetailKeyboard(task, chatId) } });
}
...
async function submitToolR18ImageRerunOrContinue(bot: TelegramBot, chatId: number, state: PendingToolR18TaskState, messageId?: number) {
  ...
  await safeEditOrSend(bot, chatId, messageId, ["\u2705 R18 \u4efb\u52d9\u5df2\u63d0\u4ea4", "\u985e\u578b\uff1a" + (state.taskType === "r18_text_to_image_continue" ? "\u7e7c\u7e8c\u751f\u6210\u5716\u7247" : "\u91cd\u65b0\u751f\u6210\u5716\u7247"), "ID\uff1a" + result.id].filter(Boolean).join(String.fromCharCode(10)), {
    reply_markup: { inline_keyboard: buildToolR18SubmittedKeyboard(chatId, String(result.id || ""), params) },
  });
}
...
async function resubmitToolR18Task(bot: TelegramBot, chatId: number, taskId: string, messageId?: number) {
  ...
  await safeEditOrSend(bot, chatId, messageId, ["\u2705 R18 \u4efb\u52d9\u5df2\u91cd\u65b0\u63d0\u4ea4", "\u985e\u578b\uff1a" + toolR18TaskLabel(result.task_type || task.type), "ID\uff1a" + result.id].filter(Boolean).join(String.fromCharCode(10)), {
    reply_markup: { inline_keyboard: buildToolR18ResubmittedKeyboard(chatId, String(result.id || ""), params) },
  });
}
...
const TOOL_R18_T2I_PERSONA_OPTIONS = [
  { id: "0", label: "人設1撈女1金君雅", path: "Character Setting\\人设1捞女1金君雅.safetensors", personaKey: "jinjunya" },
];
const TOOL_R18_PERSON_T2I_RETURN_COUNT = 4;
...
function normalizeToolR18PersonaKey(value: unknown) {
  return String(value || "").trim().toLowerCase().replace(/[^a-z0-9]+/g, "");
}

async function resolveToolR18AutoPersonaOption(state: PendingToolR18TaskState | undefined, profile: string) {
  const context = (state?.params || {}).r18_paid_post_context || {};
  const archiveId = String(context.archiveId || "").trim();
  if (!archiveId) return null;
  const archive = await loadPersonaArchive(archiveId).catch(() => null);
  const setup: any = archive?.setup || {};
  const workflow: any = setup.imageWorkflow || {};
  const personaKey = normalizeToolR18PersonaKey(workflow.personaKey || setup.personaKey || archive?.id || archive?.name || setup.personaName);
  const haystack = [
    workflow.personaKey,
    ...
    archive?.name,
    setup.personaName,
  ].map((item) => normalizeToolR18PersonaKey(item)).filter(Boolean);
  return toolR18TextToImagePersonaOptions(profile).find((option: any) => {
    const optionKeys = [option.personaKey, option.id, option.label, option.path].map((item) => normalizeToolR18PersonaKey(item)).filter(Boolean);
    return optionKeys.some((key) => key && (key === personaKey || haystack.includes(key) || haystack.some((item) => item.includes(key))));
  }) || null;
}
...
function toolR18TextToImageQaEnabled(params?: Record<string, any>) {
  const value = params?.text_to_image_auto_qa_enabled;
...
function buildToolR18TextToImageRatioKeyboard(profile: string, backCallback = "toolr18_group_image", qaEnabled = false) {
  ...
  rows.push([{ text: qaEnabled ? "✅ QA 審查：開啟" : "☑️ QA 審查：關閉", callback_data: "toolr18_t2i_toggle_qa" }]);
  rows.push([{ text: "◀️ 返回", callback_data: backCallback }]);
  return rows;
}

async function freeGeneratedPostImageRatioProfile() {
...
async function sendFreeGeneratedPostImageRatioPicker(bot: TelegramBot, chatId: number, msgId: number | undefined, state: PendingGeneratePostState) {
  ...
  await safeEditOrSend(bot, chatId, msgId, [
    ...
    "",
    "\u4EBA\u8A2D\uFF1A" + state.archiveName,
    "\u6A21\u5F0F\uFF1A" + generatePostModeLabel(state.textOnly, state.contentBranch, state.contentTimeSlot),
    "\u6578\u91CF\uFF1A" + state.count + " \u7BC7",
    "",
    "\u2B50 \u8ACB\u9078\u64C7\u514D\u8CBB\u7FA4\u914D\u5716\u756B\u9762\u6BD4\u4F8B \u2B50",
    "\u9078\u5B8C\u5F8C\u518D\u767C\u9001\u672C\u6B21\u751F\u6210\u63D0\u793A\u8A5E\u3002",
...
async function sendGeneratePostPromptInputStep(bot: TelegramBot, chatId: number, msgId: number | undefined, state: PendingGeneratePostState) {
  const modeLabel = generatePostModeLabel(state.textOnly, state.contentBranch, state.contentTimeSlot);
  const ratioLine = state.imageAspectRatio ? `\n\u756B\u9762\u6BD4\u4F8B\uFF1A${state.imageAspectRatio}${state.imageRatioLabel ? `\uFF08${state.imageRatioLabel}\uFF09` : ""}` : "";
  await safeEditOrSend(bot, chatId, msgId, `\u270D\uFE0F *\u65B0\u5EFA\u63A8\u6587*\n\n\u4EBA\u8A2D\uFF1A${state.archiveName}\n\u6A21\u5F0F\uFF1A${modeLabel}\n\u6578\u91CF\uFF1A${state.count} \u7BC7${ratioLine}\n\n\u2B50 \u8ACB\u767C\u9001\u672C\u6B21\u751F\u6210\u7684\u63D0\u793A\u8A5E \u2B50\n\u3000\u3000\u4E5F\u53EF\u4EE5\u8DF3\u904E\u63D0\u793A\u8A5E\uFF0C\u8B93 AI \u6839\u64DA\u4EBA\u8A2D\u81EA\u7531\u767C\u5C55\u3002\n\n\u4F8B\u5982\uFF1A\u570D\u7E5E\u6559\u5E2B\u751F\u6D3B\uFF0C\u5BEB\u5F97\u50CF\u7FA4\u5167\u65E9\u5B89\u65E5\u5E38\u3002`, {
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: [
        [{ text: "\u23ED \u8DF3\u904E\u63D0\u793A\u8A5E\uFF0C\u8B93 AI \u81EA\u7531\u767C\u5C55", callback_data: "genpost_prompt_skip" }],
        [{ text: "\u25C0\uFE0F \u8FD4\u56DE" + (shouldSelectFreeGeneratedPostImageRatio(state) ? "\u756B\u9762\u6BD4\u4F8B" : "\u751F\u6210\u6578\u91CF"), callback_data: shouldSelectFreeGeneratedPostImageRatio(state) ? "genpost_ratio_back" : `genpost_mode_${state.archiveId}_${state.textOnly ? "textonly" : "withimage"}` }],
      ],
    },
  });
}
...
function buildToolR18TextToImagePromptModeKeyboard(backCallback = "toolr18_t2i_back_before_prompt") {
  return [
    [{ text: "AI 自由發揮", callback_data: "toolr18_t2i_prompt_free" }],
...
async function sendToolR18TextToImageRatioStep(bot: TelegramBot, chatId: number, messageId?: number, seedParams: Record<string, any> = {}) {
  ...
  const params = { ...seedParams, text_to_image_auto_qa_enabled: toolR18TextToImageQaEnabled(seedParams), text_to_image_workflow_profile: profile, text_to_image_workflow_path: runtime.workflowPath };
  ...
  await safeEditOrSend(bot, chatId, messageId, ["R18 / 文生圖設定", "目前步驟：1/" + toolR18TextToImageStepTotal(profile) + " 請選擇圖像比例"].join(String.fromCharCode(10)), {
    reply_markup: { inline_keyboard: buildToolR18TextToImageRatioKeyboard(profile, toolR18FlowCancelCallback(params, "toolr18_group_image"), toolR18TextToImageQaEnabled(params)) },
  });
}
...
async function sendToolR18TextToImageResolutionStep(bot: TelegramBot, chatId: number, messageId: number | undefined, optionId?: string) {
  ...
  const outputCount = profile === "person_t2i" ? TOOL_R18_PERSON_T2I_RETURN_COUNT : 1;
  const baseParams = {
    ...(previous?.params || {}),
    ...
    telegram_return_count: outputCount,
    text_to_image_return_count: outputCount,
    text_to_image_qa_target_count: outputCount,
    text_to_image_auto_qa_enabled: toolR18TextToImageQaEnabled(previous?.params),
    text_to_image_auto_qa_max_attempts: outputCount,
    target_count: outputCount,
...
async function sendToolR18TextToImagePersonaStep(bot: TelegramBot, chatId: number, messageId: number | undefined) {
  ...
  const paidContext = (state?.params || {}).r18_paid_post_context;
  if (paidContext) {
    const persona = await resolveToolR18AutoPersonaOption(state, profile);
    if (!persona) {
      await safeEditOrSend(bot, chatId, messageId, "未找到目前人設對應的文生圖 LoRA，已停止避免套錯人物。請先在後台補齊此人設的 LoRA 映射。", {
        reply_markup: { inline_keyboard: [[{ text: "◀️ 返回付費內容", callback_data: "paidr18_entry" }]] },
      });
      return;
    }
    pendingToolR18Tasks.set(chatId, {
      ...
      params: {
        ...
        persona_lora: persona.path,
        persona_label: persona.label,
        persona_auto_matched: true,
      },
    });
    await sendToolR18TextToImagePromptModeStep(bot, chatId, messageId);
    return;
...
async function sendToolR18TextToImagePromptStep(bot: TelegramBot, chatId: number, messageId: number | undefined, useGrok: boolean) {
  ...
  await safeEditOrSend(bot, chatId, messageId, ["R18 / 文生圖", "目前步驟：輸入圖片需求或上傳參考圖", "", useGrok ? "將由 Grok 生成最終提示詞。" : "將按你輸入的自定義提示詞提交。", "需要參考圖時，請把圖片作為下一條訊息發送，並在文字裡寫要求。"].join(String.fromCharCode(10)), {
    reply_markup: { inline_keyboard: [[{ text: "取消", callback_data: "toolr18_t2i_back_before_prompt" }]] },
  });
}
...
function toolR18PromptBackCallback(taskType?: ToolR18TaskType) {
  if (taskType === "video_i2v") return "toolr18_i2v_back_prompt_mode";
  if (taskType === "single_image_edit" || taskType === "get_nano_banana") return "toolr18_imgedit_back_prompt_mode";
  if (taskType === "face_swap") return "toolr18_faceswap_back_prompt_mode";
  return "toolr18_t2i_back_before_prompt";
}

function buildToolR18PromptFailureKeyboard(state: PendingToolR18TaskState) {
  return [
    [{ text: "🔁 重試生成提示詞", callback_data: "toolr18_prompt_regen" }],
    [{ text: "◀️ 返回上一步", callback_data: toolR18PromptBackCallback(state.taskType) }],
  ];
}
...
function toolR18TextToImageInputGuidance(state: PendingToolR18TaskState) {
  ...
  if (!String(params.aspect_ratio || "").trim()) {
    return {
      ...
      keyboard: buildToolR18TextToImageRatioKeyboard(profile, toolR18FlowCancelCallback(params, "toolr18_group_image"), toolR18TextToImageQaEnabled(params)),
    };
  }
...
function toolR18PendingInputGuidance(state: PendingToolR18TaskState, hasMedia: boolean) {
  ...
  if (state.stage === "video_i2v_prompt_mode") {
    return { text: "請先點擊按鈕選擇提示詞方式：AI 自由發揮、讓 Grok 生成提示詞，或輸入自定義提示詞提交。", keyboard: buildToolR18VideoI2vPromptModeKeyboard() };
  }
...
function buildToolR18PreviewParams(state: PendingToolR18TaskState, requestText: string) {
  ...
  if (state.taskType === "text_to_image") {
    ...
    const paidVisualContext = buildPaidR18VisualContextInstruction(base);
...
async function sendToolR18PromptPreview(bot: TelegramBot, chatId: number, state: PendingToolR18TaskState, requestText: string) {
  ...
  try {
    ...
    const failedState: PendingToolR18TaskState = {
      ...state,
      stage: state.stage || "await_task_input",
      text: requestText,
      originalRequest: state.originalRequest || requestText,
    };
    pendingToolR18Tasks.set(chatId, failedState);
    await bot.sendMessage(chatId, [
      "❌ Grok 生成提示詞失敗。",
      "原因：" + formatUserFacingError(error, "請稍後重試，或返回上一步改用自定義提示詞。"),
      "",
      "目前流程已保留，可以重試或返回上一步，不需要從頭開始。",
    ].join(String.fromCharCode(10)), {
      reply_markup: { inline_keyboard: buildToolR18PromptFailureKeyboard(failedState) },
    });
  } finally {
...
async function submitToolR18PromptReview(bot: TelegramBot, chatId: number, state: PendingToolR18TaskState, promptText?: string) {
  ...
  const params = {
    ...
    tg_original_user_request: String(existingParams.tg_original_user_request || state.originalRequest || state.text || finalPrompt),
    tg_user_instruction: finalPrompt,
...
async function downloadToolR18TelegramMedia(bot: TelegramBot, msg: TelegramBot.Message, media: any) {
  ...
  const tmpDir = path.join(TOOL_R18_UPLOAD_HOST_DIR, ".telegram_tmp", String(chatId));
  fs.mkdirSync(tmpDir, { recursive: true });
  let downloadedPath = "";
  try {
    downloadedPath = await bot.downloadFile(fileId, tmpDir);
    fs.copyFileSync(downloadedPath, hostPath);
  } catch (error: any) {
    const detail = String(error?.message || error || "").trim();
    throw new Error("下載 Telegram 素材失敗，請確認本地 Bot 代理可用後重新上傳。原因：" + (detail || "Telegram 文件下載失敗"));
  } finally {
...
function buildToolR18PendingKeyboard(state: PendingToolR18TaskState) {
  ...
  rows.push([{ text: "清空重來", callback_data: "toolr18_clear" }, { text: "取消", callback_data: toolR18FlowCancelCallback(state.params, "toolr18_entry") }]);
  return rows;
}

function buildToolR18SubmitFailureKeyboard(state: PendingToolR18TaskState) {
  const rows = buildToolR18PendingKeyboard(state);
  if (toolR18RequiresPromptPreview(state.taskType)) {
    rows.unshift([{ text: "◀️ 返回上一步", callback_data: toolR18PromptBackCallback(state.taskType) }]);
  }
  return rows;
}
...
async function startPaidR18ImageFirstFlowFromToolTask(bot: TelegramBot, chatId: number, state: PendingToolR18TaskState, count: number) {
  const params = state.params || {};
  const context = params.r18_paid_post_context || {};
  const archiveId = String(context.archiveId || "").trim();
  if (!archiveId) throw new Error("付費內容上下文缺失，請重新從「新建推文 → 付費內容」進入。");
  const archiveName = String(context.archiveName || "").trim() || "R18";
  const total = Math.min(Math.max(1, count), 20);
  const qaEnabled = toolR18TextToImageQaEnabled(params);
  const targetText = qaEnabled
  ...
  pendingPaidR18ImageFirstGroupFlows.set(chatId, {
    ...
    params: {
      ...
      tg_prompt_safety_profile: "r18_paid",
      tg_suppress_auto_notify: true,
      tg_source_bot: "tool_r18",
      source: "telegram-r18-paid-image-first",
    },
    contentTimeSlot: context.contentTimeSlot === "morning" || context.contentTimeSlot === "night" ? context.contentTimeSlot : ensurePaidR18TimeSlot(),
    selectedMemoryEntryIds: Array.isArray(context.selectedMemoryEntryIds) ? context.selectedMemoryEntryIds.map(String) : undefined,
    selectedMemorySummaries: Array.isArray(context.selectedMemorySummaries) ? context.selectedMemorySummaries.map(String) : undefined,
    startedAt: Date.now(),
  });
  await telegramBestEffort("generatePosts.r18ImageFirstStartFromToolTask", bot.sendMessage(chatId, [
    "⏳ 付費群內容將按篇生成 R18 候選配圖。",
    "",
    "數量：" + total + " 篇",
    targetText,
    "流程：先返回一組候選圖 → 你選 1 張 → 根據該圖生成付費文案 → 再生成下一組。",
  ].join(String.fromCharCode(10)), {
    reply_markup: { inline_keyboard: [[{ text: "◀️ 返回付費內容", callback_data: "paidr18_back_workflow" }]] },
  }), 15_000);
  await generateCurrentPaidR18ImageFirstGroup(bot, chatId);
}
...
async function submitPendingToolR18Task(bot: TelegramBot, chatId: number, state: PendingToolR18TaskState) {
  if (state.stage === "await_agent_input") {
    ...
    await bot.sendMessage(chatId, submitted ? ["✅ R18 智能任務已提交", "類型：" + toolR18TaskLabel(result.task_type), "ID：" + result.id].join(String.fromCharCode(10)) : ["R18 智能提交需要補充資訊：", String(result?.reply || result?.summary || "請補充任務類型和素材。")].join(String.fromCharCode(10)), { reply_markup: { inline_keyboard: submitted ? buildToolR18SubmittedKeyboard(chatId, String(result.id || ""), state.params) : buildToolR18MenuKeyboard(chatId) } });
    return;
  }
  if (!state.taskType) throw new Error("R18 任務類型缺失");
...
async function handlePendingToolR18Input(bot: TelegramBot, msg: TelegramBot.Message, state: PendingToolR18TaskState, media: any, text: string) {
  ...
  if (nextState.stage === "image_edit_input") {
    ...
    await bot.sendMessage(chatId, meta.label + String.fromCharCode(10) + "步驟 1/" + meta.totalSteps + "：請上傳需要編輯的原圖。", {
      reply_markup: { inline_keyboard: buildToolR18ImageStepKeyboard(undefined, Boolean(params.input_image_local_path), toolR18FlowCancelCallback(params, "toolr18_group_image")) },
    });
    return;
  }
  ...
  if (nextState.stage === "image_edit_reference") {
    ...
    await bot.sendMessage(chatId, "圖片編輯" + String.fromCharCode(10) + "步驟 2/4：請上傳參考圖或素材圖。", {
      reply_markup: { inline_keyboard: buildToolR18ImageStepKeyboard("toolr18_imgedit_back_input", Boolean(params.reference_image_local_path), toolR18FlowCancelCallback(params, "toolr18_group_image")) },
    });
    return;
  }
  ...
  if (nextState.stage === "face_swap_target") {
    ...
    await bot.sendMessage(chatId, "人物換臉" + String.fromCharCode(10) + "步驟 1/4：請上傳原圖，也就是需要被換臉的圖片。", {
      reply_markup: { inline_keyboard: buildToolR18FaceSwapStepKeyboard(undefined, Boolean(params.target_image_local_path), toolR18FlowCancelCallback(params, "toolr18_group_image")) },
    });
    return;
  }
  ...
  if (nextState.stage === "face_swap_source") {
    ...
    await bot.sendMessage(chatId, "人物換臉" + String.fromCharCode(10) + "步驟 2/4：請上傳人臉參考圖。", {
      reply_markup: { inline_keyboard: buildToolR18FaceSwapStepKeyboard("toolr18_faceswap_back_target", Boolean(params.source_image_local_path), toolR18FlowCancelCallback(params, "toolr18_group_image")) },
    });
    return;
  }
  ...
  if (nextState.taskType === "text_to_image" && nextState.params?.tg_use_llm_prompt === false) {
    const customPrompt = String(text || "").trim();
    if (!customPrompt) {
      await bot.sendMessage(chatId, "請直接輸入自定義提示詞；需要參考圖時，請把圖片和文字要求一起發送。", {
        reply_markup: { inline_keyboard: [[toolR18ContextBackButton(chatId, nextState.params, "toolr18_t2i_back_before_prompt")]] },
      });
      return;
    }
    await submitToolR18PromptReview(bot, chatId, {
      ...nextState,
      stage: "await_prompt_review",
      text: customPrompt,
      originalRequest: customPrompt,
    }, customPrompt);
    return;
  }
...
function buildCustomPersonaPostBackKeyboard(state: {
  archiveId: string;
  contentBranch?: GeneratePostContentBranch;
}) {
  ...
  return {
    inline_keyboard: [
      ...
      [{ text: "◀️ 返回人設詳情", callback_data: `pd_${state.archiveId}` }],
    ],
  };
}

async function handlePendingCustomPersonaPostInput(
  bot: TelegramBot,
  chatId: number,
  state: NonNullable<ReturnType<typeof pendingCustomPersonaPosts.get>>,
  text: string,
  media: any,
  mediaKind?: "photo" | "video" | "document",
) {
  ...
  if (!nextState.text && !nextState.mediaUrl) {
    await bot.sendMessage(chatId, [
      ...
      "目前步驟：等待推文內容",
      "",
      "請發送純文字推文，或發送圖片/視頻並附帶文案。",
    ].join(String.fromCharCode(10)), {
      reply_markup: buildCustomPersonaPostBackKeyboard(nextState),
    });
    return true;
  }
  ...
  const saved = await appendCustomPersonaArchivePost({
    ...
    telegramGroupContentType: contentBranchToTelegramGroupContentType(nextState.contentBranch),
  }).catch(() => null);
  ...
  await bot.sendMessage(
    ...
    `✅ 已寫入推文庫\n人設：${nextState.archiveName}${nextState.contentBranch ? `\n內容類型：${generatePostContentBranchLabel(nextState.contentBranch)}` : ""}\n類型：${nextState.mediaUrl ? "圖文/視頻文案" : "純文字"}\n目前待發佈：${saved.posts.length} 篇`,
    {
      reply_markup: {
        inline_keyboard: [
          [{ text: "📝 查看推文列表", callback_data: buildStoredPostsPageCallback(nextState.archiveId, 0, contentBranchToTelegramGroupContentType(nextState.contentBranch)) }],
          [{ text: "🧩 繼續自訂新建", callback_data: buildGeneratePostCustomCallback(nextState.archiveId, nextState.contentBranch) }],
...
async function sendPostImageCandidateMessage(bot: TelegramBot, chatId: number, args: {
  ...
  const caption = args.paidR18Context
    ? `\uD83D\uDDBC \u7B2C${args.displayIndex}\u7BC7\u4ED8\u8CBB R18 \u5019\u9078\u5716 ${args.candidateIndex}/${args.totalCandidates}\n\u8ACB\u5148\u9078\u64C7\u8981\u63D2\u5165\u63A8\u6587\u7684\u5716\u7247\uFF1B\u9078\u4E2D\u5F8C\u624D\u6703\u6839\u64DA\u9019\u5F35\u4EBA\u7269\u5716\u751F\u6210\u4ED8\u8CBB\u7FA4\u6587\u6848\u3002`
    : `\uD83D\uDDBC \u7B2C${args.displayIndex}\u7BC7\u5019\u9078\u914D\u5716 ${args.candidateIndex}/${args.totalCandidates}\n\u8ACB\u9EDE\u4E0B\u65B9\u6309\u9215\u9078\u64C7\u5BEB\u5165\u63A8\u6587\u3002`;
...
function buildPostImageGroupCandidateKeyboard(args: {
  archiveId: string;
  postId?: string;
  content: string;
  imageUrls: string[];
  ...
  const rows: Array<Array<{ text: string; callback_data: string }>> = [];
  const candidateRows: Array<{ text: string; callback_data: string }> = [];
  for (let index = 0; index < args.imageUrls.length; index += 1) {
    const imageUrl = args.imageUrls[index];
    const key = rememberPostImageCandidateSelectionAction({
      archiveId: args.archiveId,
      postId: args.postId,
      content: args.content,
      imageUrl,
    ...
    candidateRows.push({ text: `\u2705 \u9078\u64C7\u7B2C ${index + 1} \u5F35`, callback_data: buildPostImageCandidateSelectCallback(key) });
  }
  for (let i = 0; i < candidateRows.length; i += 2) rows.push(candidateRows.slice(i, i + 2));
  const listPage = Math.max(0, Math.floor((Math.max(1, args.displayIndex) - 1) / STORED_POSTS_PAGE_SIZE));
  rows.push([{ text: "\uD83D\uDCDD \u67E5\u770B\u63A8\u6587\u5217\u8868", callback_data: args.postId ? buildStoredPostsPageCallback(args.archiveId, listPage) : `posts_${args.archiveId}` }]);
  return { inline_keyboard: rows };
}

async function sendPostImageCandidateGroupMessage(bot: TelegramBot, chatId: number, args: {
  archiveId: string;
  postId?: string;
  content: string;
  imageUrls: string[];
...
async function submitGeneratedPostImageCandidateTask(args: {
  ...
  if (qaEnabled) {
    generationParams.text_to_image_qa_target_count = GENERATED_POST_IMAGE_TARGET_COUNT;
    generationParams.text_to_image_auto_qa_max_attempts = GENERATED_POST_IMAGE_TARGET_COUNT;
  }
...
async function promptNextFreeGeneratedPostImageGroup(bot: TelegramBot, chatId: number, flow: PendingGeneratedPostImageGroupFlow) {
  const nextIndex = flow.currentIndex + 1;
  if (nextIndex >= flow.posts.length) {
    pendingGeneratedPostImageGroupFlows.delete(chatId);
    await telegramBestEffort("generatePosts.freeImageGroupsDone", bot.sendMessage(chatId, `\u2705 \u914D\u5716\u9078\u64C7\u5B8C\u6210\uFF1A${flow.posts.length}/${flow.posts.length} \u7BC7`, {
      reply_markup: {
        inline_keyboard: [
          [{ text: "\uD83D\uDCDD \u67E5\u770B\u63A8\u6587\u5217\u8868", callback_data: flow.generatedPostsListCallback }],
          [{ text: "\u25C0\uFE0F \u8FD4\u56DE\u4EBA\u8A2D\u8A73\u60C5", callback_data: `pd_${flow.archiveId}` }],
        ],
      },
    }), 45_000);
    return;
  }
  pendingGeneratedPostImageGroupFlows.set(chatId, { ...flow, currentIndex: nextIndex });
  await telegramBestEffort("generatePosts.freeImageGroupsNextPrompt", bot.sendMessage(chatId, `\u2705 \u5DF2\u9078\u5B8C\u7B2C ${nextIndex}/${flow.posts.length} \u7BC7\u7684\u914D\u5716\u3002\n\n\u9EDE\u64CA\u4E0B\u65B9\u6309\u9215\u751F\u6210\u7B2C ${nextIndex + 1}/${flow.posts.length} \u7D44\u5716\u7247\u3002`, {
    reply_markup: {
      inline_keyboard: [
        [{ text: `\uD83D\uDDBC \u751F\u6210\u7B2C ${nextIndex + 1} \u7D44\u5716\u7247`, callback_data: "genpost_imggrp_next" }],
        [{ text: "\uD83D\uDCDD \u67E5\u770B\u63A8\u6587\u5217\u8868", callback_data: flow.generatedPostsListCallback }],
      ],
    },
  }), 45_000);
...
async function generateCurrentFreeGeneratedPostImageGroup(bot: TelegramBot, chatId: number) {
  const flow = pendingGeneratedPostImageGroupFlows.get(chatId);
  if (!flow) {
    await bot.sendMessage(chatId, "\u914D\u5716\u5206\u7D44\u72C0\u614B\u5DF2\u5931\u6548\uFF0C\u8ACB\u91CD\u65B0\u751F\u6210\u63A8\u6587\u3002", {
      reply_markup: { inline_keyboard: [[{ text: "\uD83D\uDCDD \u67E5\u770B\u63A8\u6587\u5217\u8868", callback_data: "list_personas" }]] },
    });
    return;
  }
  const post = flow.posts[flow.currentIndex];
  if (!post) {
    await promptNextFreeGeneratedPostImageGroup(bot, chatId, flow);
    return;
  }
  const displayIndex = flow.currentIndex + 1;
  ...
  const results = await withTimeout(generateImagesForGeneratedPosts({
    ...
    imageAspectRatio: flow.imageAspectRatio,
...
async function generateR18ImagesForGeneratedPosts(args: { bot: TelegramBot; chatId: number; archiveId: string; archiveName: string; posts: Array<{ id?: string; content: string }>; visualInstruction?: string; sourceBot: "tool_r18" | "automatic_script"; taskType?: ToolR18TaskType; baseParams?: Record<string, any>; }) {
  ...
  for (let index = 0; index < args.posts.length; index += 1) {
    ...
    const requestText = [
      "為付費群內容生成 4 張 R18 候選配圖，必須先生成圖片，後續文案會依照圖片內容再生成。",
      "圖片需要服務付費群導流：福利感、限定感、轉化導向；畫面可以更有誘惑力，但仍要保持平台安全與畫面合理。",
      "必須保留同一個人設身份、臉部特徵、年齡感、身形比例、髮型和整體氣質。",
      "先分析使用者要求，再補齊服裝結構、顏色材質、場景、姿勢、情緒、鏡頭和光線。",
      args.visualInstruction ? "本次整體要求：" + args.visualInstruction : "",
      "人設：" + args.archiveName,
      "圖片內容方向：" + post.content,
    ].filter(Boolean).join(String.fromCharCode(10));
    try {
      const qaEnabled = toolR18TextToImageQaEnabled(args.baseParams);
      const generationParams: Record<string, any> = {
          ...(args.baseParams || {}),
          prompt: requestText,
          prompt_text: requestText,
          ...
          tg_user_instruction: requestText,
          tg_original_user_request: requestText,
          tg_generation_context: "Paid R18 image-first post for archive " + args.archiveId + ", item " + (post.id || index + 1),
          tg_use_llm_prompt: true,
          tg_latest_prompt_only: true,
          tg_prompt_confirmed: false,
          tg_content_branch: "r18",
      ...
      const submit = await toolR18JsonRequest("POST", "/api/internal/tg/submit", {
        task_type: args.taskType || "text_to_image",
        tg_chat_id: args.chatId,
        params: generationParams,
      });
      const taskId = String(submit?.id || "").trim();
...
function buildPaidR18ImageFirstVisualDirection(args: {
  archiveName: string;
  prompt: string;
  contentTimeSlot?: GeneratePostTimeSlot;
  index: number;
  ...
  const timeLabel = args.contentTimeSlot ? generatePostTimeSlotLabel(args.contentTimeSlot) : "付費群內容";
  const timeVisualInstruction = buildPaidR18TimeVisualInstruction(args.contentTimeSlot);
  const userPrompt = String(args.prompt || "").replace(/\s+/g, " ").trim();
  return [
    `付費群 R18 圖片先行流程，第 ${args.index}/${args.total} 組。`,
    `人設：${args.archiveName}`,
    `內部隨機時段：${timeLabel}。這個時段只用來控制圖片光線、場景和氛圍，不需要顯示給使用者選擇。`,
    timeVisualInstruction,
    userPrompt ? `使用者要求：${userPrompt}` : "使用者未指定主題，請根據人設生成付費群福利感圖片。",
    "先生成符合付費群內容方向的圖片，再由圖片內容反推短文案。",
    "圖片方向：付費群內容先生成圖片，畫面需要有清楚可引用的照片特徵；服裝、姿態、場景、光線和情緒都要能支撐後續直白誘惑的短文案。",
    "同組 4 張圖保持同一人設，但構圖、距離、姿態或場景細節要有變化。",
  ].filter(Boolean).join("\n");
}
...
function buildPaidR18FallbackPostContent(args: {
  ...
  const outfit = firstPaidR18PromptMatch(source, [
    ...
    [/短裙|包臀裙|窄裙|裙/i, "裙裝"],
    [/外套|開衫|罩衫|上衣|衣服/i, "上衣"],
    [/蕾絲|蕾丝|絲襪|丝袜|黑絲|黑丝/i, "蕾絲絲襪"],
    [/內衣|内衣|bra/i, "內衣"],
    [/比基尼|泳裝|泳衣/i, "泳裝"],
  ], "這身衣服");
  const exposureFocus = firstPaidR18PromptMatch(source, [
    [/敞開|敞开|拉開|拉开|掀開|掀开|解開|解开/i, "敞開得很明顯"],
    [/低胸|深V|露胸|胸口/i, "胸口線條很搶眼"],
    [/露腿|大腿|腿部/i, "腿部線條很會勾人"],
    [/露肩|肩膀|鎖骨|锁骨/i, "肩頸露出很抓眼"],
    [/透明|半透|透視/i, "透出感很強"],
...
async function attachGeneratedImageCandidatesToArchivePost(args: {
  archiveId: string;
  postId?: string;
  content: string;
  imageUrls: string[];
  ...
  for (const imageUrl of args.imageUrls) {
    ok = await attachGeneratedImageToArchivePost({
      archiveId: args.archiveId,
      postId: args.postId,
      content: args.content,
      imageUrl,
...
async function saveAndSendPaidR18SelectedImagePost(args: {
  ...
  if (caption && captionPrefix.length + caption.length <= 1024) {
    await args.bot.sendPhoto(args.chatId, resolveTelegramPhotoInput(args.imageUrl), {
...
async function executePaidR18ImageFirstGeneratePosts(args: {
  ...
  const total = Math.max(1, args.count || 1);
  const qaEnabled = false;
  const targetText = qaEnabled
    ? `每篇目標：通過 QA 的 ${GENERATED_POST_IMAGE_TARGET_COUNT} 張圖`
    : `每篇目標：生成 ${GENERATED_POST_IMAGE_TARGET_COUNT} 張候選圖`;
  pendingPaidR18ImageFirstGroupFlows.set(args.chatId, {
    archiveId: args.archiveId,
    archiveName: args.archiveName,
    total,
    currentIndex: 0,
  ...
  await telegramBestEffort("generatePosts.r18ImageFirstStart", args.bot.sendMessage(
    args.chatId,
    `⏳ 付費群內容將先生成 R18 候選配圖。\n\n數量：${total} 篇\n${targetText}\n\n圖片完成後，請手動選擇要插入推文的圖片；選中後才會根據該人物圖片生成付費群文案。`,
    { reply_markup: { inline_keyboard: [[{ text: "◀️ 返回人設詳情", callback_data: `pd_${args.archiveId}` }]] } },
  ));
  await generateCurrentPaidR18ImageFirstGroup(args.bot, args.chatId);
}

async function generateCurrentPaidR18ImageFirstGroup(bot: TelegramBot, chatId: number) {
  const flow = pendingPaidR18ImageFirstGroupFlows.get(chatId);
  if (!flow) {
    await telegramBestEffort("generatePosts.r18ImageFirstMissingFlow", bot.sendMessage(chatId, "付費群候選圖流程已失效，請重新從「新建推文 → 付費內容」進入。", {
  ...
  const qaEnabled = toolR18TextToImageQaEnabled(flow.params);
  const targetText = qaEnabled
    ? `每組需要 QA 通過的 ${GENERATED_POST_IMAGE_TARGET_COUNT} 張圖。`
    : `每組生成 ${GENERATED_POST_IMAGE_TARGET_COUNT} 張候選圖。`;
  await telegramBestEffort("generatePosts.r18ImageGroupStart", bot.sendMessage(
    chatId,
  ...
  if (!imageResult?.ok || imageResult.imageUrls.length < GENERATED_POST_IMAGE_TARGET_COUNT) {
    await telegramBestEffort("generatePosts.r18ImageGroupFailed", bot.sendMessage(chatId, [
      `❌ 第 ${index + 1}/${flow.total} 組付費 R18 候選圖生成失敗。`,
      formatUserFacingError(imageResult?.error || "", "R18 配圖生成失敗，請稍後重試。"),
    ].join(String.fromCharCode(10)), {
      reply_markup: { inline_keyboard: [[{ text: "🔁 重新生成這組", callback_data: "paidr18_imggrp_next" }], [{ text: "◀️ 返回付費內容", callback_data: "paidr18_back_workflow" }]] },
    }), 15_000);
    return;
  }
...
async function loadTelegramImageBuffer(imageUrl: string): Promise<Buffer> {
  const input = resolveTelegramPhotoInput(imageUrl);
...
<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
  <rect x="0" y="0" width="${width}" height="${height}" rx="0" fill="rgba(0,0,0,0.55)"/>
  <text x="${Math.round(width / 2)}" y="${Math.round(height * 0.68)}" text-anchor="middle" font-family="Arial, sans-serif" font-size="${Math.round(height * 0.5)}" font-weight="700" fill="#fff">${label}</text>
</svg>`);
}
...
async function loadSelectablePersonaMemories(archiveId: string): Promise<PersonaMemoryEntry[]> {
  ...
  for (const record of (archive.publishHistory || []).slice(-MAX_SELECTABLE_PERSONA_MEMORIES).reverse()) {
    pushEntry({
      id: record.id,
...
const MAX_SELECTABLE_PERSONA_MEMORIES = 100;

function generateMemoryPageInfo(state: Pick<PendingGeneratePostState, "memoryOptions" | "memoryPage">) {
  const options = state.memoryOptions || [];
  const totalPages = Math.max(1, Math.ceil(options.length / GENERATE_MEMORY_PAGE_SIZE));
  const page = Math.min(Math.max(0, Math.floor(Number(state.memoryPage) || 0)), totalPages - 1);
  const start = page * GENERATE_MEMORY_PAGE_SIZE;
  const end = Math.min(options.length, start + GENERATE_MEMORY_PAGE_SIZE);
  return {
    page,
    totalPages,
    start,
    end,
    pageOptions: options.slice(start, end),
  };
}
...
function buildGenerateMemorySelectionKeyboard(state: PendingGeneratePostState) {
  const selected = new Set(state.selectedMemoryEntryIds || []);
  const options = state.memoryOptions || [];
  const pageInfo = generateMemoryPageInfo(state);
  const rows = pageInfo.pageOptions.map((entry, pageIndex) => {
    const index = pageInfo.start + pageIndex;
...
async function ensureGenerateMemoryOptions(state: PendingGeneratePostState): Promise<PendingGeneratePostState> {
  if (Array.isArray(state.memoryOptions) && state.memoryOptions.length > 0) {
    return {
      ...state,
      memoryPage: Number.isFinite(Number(state.memoryPage)) ? Math.max(0, Math.floor(Number(state.memoryPage))) : 0,
    };
  }
  const memoryOptions = await loadSelectablePersonaMemories(state.archiveId).catch(() => []);
  const existingIds = new Set(memoryOptions.map((entry) => String(entry.id)));
  const selectedMemoryEntryIds = (state.selectedMemoryEntryIds || []).map(String);
  return {
    ...state,
    memoryOptions,
    memoryPage: 0,
    selectedMemoryEntryIds: existingIds.size
      ? selectedMemoryEntryIds.filter((id) => existingIds.has(id))
      : selectedMemoryEntryIds,
  };
}

async function sendPaidR18MemorySelectionFromContext(bot: TelegramBot, chatId: number, messageId?: number) {
  const pending = pendingGeneratePosts.get(chatId);
  const flow = pendingPaidR18ImageFlows.get(chatId);
  let state: PendingGeneratePostState | null = null;
  if (pending?.archiveId) {
    state = {
      ...pending,
      contentBranch: "r18",
      contentTimeSlot: ensurePaidR18TimeSlot(pending.contentTimeSlot),
      stage: "await_memory",
    };
  } else if (flow?.archiveId) {
    state = {
      archiveId: flow.archiveId,
      archiveName: flow.archiveName,
      count: 1,
      textOnly: false,
      contentBranch: "r18",
      contentTimeSlot: ensurePaidR18TimeSlot(flow.contentTimeSlot),
      selectedMemoryEntryIds: flow.selectedMemoryEntryIds,
      selectedMemorySummaries: flow.selectedMemorySummaries,
      stage: "await_memory",
    };
  ...
  if (!state) return false;
  const nextState = await ensureGenerateMemoryOptions(state);
  setPendingGeneratePost(chatId, nextState);
  await safeEditOrSend(bot, chatId, messageId, buildGenerateMemorySelectionText(nextState), {
...
function selectedMemorySummariesFromState(state: Pick<PendingGeneratePostState, "selectedMemoryEntryIds" | "selectedMemorySummaries" | "memoryOptions">): string[] {
  const selected = new Set((state.selectedMemoryEntryIds || []).map((id) => String(id)));
  if (!selected.size) {
    return Array.isArray(state.selectedMemorySummaries)
      ? state.selectedMemorySummaries.map((item) => normalizeTelegramSingleLine(item)).filter(Boolean)
      : [];
  }
  const seen = new Set<string>();
  ...
  if (!summaries.length && Array.isArray(state.selectedMemorySummaries)) {
    for (const item of state.selectedMemorySummaries) {
      const normalized = normalizeTelegramSingleLine(item);
      if (!normalized || seen.has(normalized)) continue;
      seen.add(normalized);
      summaries.push(normalized);
    }
...
async function executeGeneratePostsFromTelegram(args: GeneratePostsTelegramArgs) {
  ...
  if (args.contentBranch === "r18" && !args.textOnly) {
    await sendPaidR18EntryPicker(args.bot, args.chatId, undefined, {
      ...
      textOnly: false,
      contentBranch: "r18",
      contentTimeSlot: ensurePaidR18TimeSlot(args.contentTimeSlot),
      selectedMemoryEntryIds: args.selectedMemoryEntryIds,
      selectedMemorySummaries: args.selectedMemorySummaries,
      stage: "await_time_slot",
  ...
  try {
    ...
    pendingGeneratedPostImageGroupFlows.set(args.chatId, {
      archiveId: args.archiveId,
      archiveName: args.archiveName,
      posts: imageTargets,
      currentIndex: 0,
      prompt: args.prompt,
      generatedPostsListCallback,
      imageAspectRatio: args.imageAspectRatio,
      imageWidth: args.imageWidth,
      imageHeight: args.imageHeight,
      imageRatioLabel: args.imageRatioLabel,
    ...
    await generateCurrentFreeGeneratedPostImageGroup(args.bot, args.chatId);
  } catch (error: any) {
    stopTyping();
...
export function startTelegramBot(token: string, options: TelegramBotInstanceOptions = {}): TelegramBot {
  ...
  const customPublishContentSummary = (state: PendingCustomPublish) => {
    ...
    if (state.fileId) {
      ...
      lines.push(`媒體：已收到${kind}`);
    } else {
      lines.push("媒體：未附加");
    }
    if (state.text?.trim()) {
      const preview = state.text.trim().slice(0, 120);
      lines.push(`文字：${preview}${state.text.trim().length > 120 ? "..." : ""}`);
    } else {
      lines.push("文字：未收到");
    }
    return lines.join("\n");
  ...
  const executeCustomPublish = async (chatId: number, state: PendingCustomPublish) => {
    ...
    const telegramTarget = state.platform === "telegram"
      ? resolveTelegramTargetForGroupContent(archive, state.groupContentType)
      : null;
    if (state.platform === "telegram" && !telegramTarget?.chatId && !telegramTarget?.groupName) {
      await bot.sendMessage(chatId, `❌ 未配置 Telegram ${telegramTarget?.groupContentType === "paid" ? "付費群" : "免費群"} ID，為避免發錯群組已停止發布。\n\n請先進入「人設設定」綁定 TG 群 ID。`, {
        reply_markup: { inline_keyboard: [[{ text: "◀️ 返回人設設定", callback_data: state.archiveId ? `settings_${state.archiveId}` : "list_personas" }]] },
      });
      releasePublishCommand(chatId, publishLockKey, publishScopeKey);
      return;
    }
    ...
    try {
      ...
      const result = await publishPost(
        credentials,
        {
          ...
          mediaUrl: mediaUrl || undefined,
          telegramChatId: chatId,
          telegramTargetChatId: telegramTarget?.chatId || undefined,
          telegramTargetGroupName: telegramTarget?.groupName || undefined,
          telegramGroupContentType: telegramTarget?.groupContentType,
        },
        (progress) => {
          assertPadOperationNotCancelled(padOperationKey);
...
function buildManualPostChoiceRows(
  ...
  rows.push(...buildListPaginationRows({
    page: safePage,
    totalPages,
    callbackForPage: (targetPage) => `mpage_${targetPage}`,
  }));
  return rows;
}
...
function buildQueuePaginationRows(args: { status: "pending" | "failed"; archiveId?: string; scheduledOnly?: boolean; platform?: TelegramPublishPlatform; page: number; totalPages: number }) {
  return buildListPaginationRows({
    page: args.page,
    totalPages: args.totalPages,
    callbackForPage: (targetPage) => buildQueueViewCallback({ ...args, page: targetPage }),
  });
}
...
function sendMainMenu(chatId: number, msgId?: number) {
  ...
  const callbackHandler = async (query: TelegramBot.CallbackQuery) => {
    ...
    if (data.startsWith("pad_threads_auto_reply_") && !data.startsWith("pad_threads_auto_reply_run_") && !data.startsWith("pad_threads_auto_reply_days_")) {
      pendingThreadsAutoReplyDaysInputs.delete(chatId);
    ...
    if (data.startsWith("pad_threads_auto_reply_days_")) {
      const padCode = data.slice("pad_threads_auto_reply_days_".length);
      const pads = await listPadsForThisBot();
      const allowedPad = pads.find((p) => p.padCode === padCode);
      if (!allowedPad) {
        await safeEditOrSend(bot, chatId, msgId, "❌ 這台雲機不屬於目前 Bot 的 VMOS 帳號範圍。", {
    ...
    if (data.startsWith("pad_threads_auto_reply_run_")) {
      const runPayload = data.slice("pad_threads_auto_reply_run_".length);
      const daysMatch = runPayload.match(/^d([1-7])_(.+)$/);
      const maxAgeDays = daysMatch ? Number(daysMatch[1]) : 2;
      const padCode = daysMatch ? daysMatch[2] : runPayload;
      const pads = await listPadsForThisBot();
      const allowedPad = pads.find((p) => p.padCode === padCode);
      if (!allowedPad) {
        await safeEditOrSend(bot, chatId, msgId, "❌ 這台雲機不屬於目前 Bot 的 VMOS 帳號範圍。", {
      ...
      await safeEditOrSend(bot, chatId, msgId, `🌱 *养号设置*\n\n雲機：${prev.padName || padCode}\n浏览數量：${browseCount} 條\n\n請選擇互动策略：\n\n严格执行所选目标；未达到浏览、点赞或留言目标会提示失败并请求人工介入。`, {
        ...
        reply_markup: {
          inline_keyboard: [
            ...
            [{ text: "◀️ 返回改數量", callback_data: warmupStartCallback }],
          ],
        },
      });
      return;
    ...
    if (data.startsWith("warmup_engage_")) {
      ...
      const warmupStartCallback = prev.startCallback || `warmup_start_${platform}_${padCode}`;
      if (mode === "browse") {
        pendingWarmupConfigs.set(chatId, {
          ...prev,
          platform,
    ...
    if (data.startsWith("warmup_run_")) {
      ...
      try {
        ...
        if (await sendCloudAccountStateNotice(
          ...
          [[{ text: "◀️ 返回", callback_data: warmupReturnCallback }]],
        )) {
          return;
        }
        await sendManualInterventionFailure(bot, chatId, error, {
          ...
          inlineKeyboard: [[{ text: "◀️ 返回", callback_data: warmupReturnCallback }]],
        });
      } finally {
        releaseRuntimePadOperation(padCode, padOperationKey);
      }
    ...
    if (accountProfileMatch) {
      ...
      const hint = kind === "link"
        ...
        : kind === "bio"
          ? "請直接發送新的 Threads 個人簡介，建議 150 字以內。"
          : kind === "name"
    ...
    if (data.startsWith("acctwarmup_threads_")) {
      pendingPersonaAccountActions.delete(chatId);
      const id = data.slice("acctwarmup_threads_".length);
      const archive = await loadPersonaForThisBot(id);
      if (!archive) { sendMainMenu(chatId, msgId); return; }
      const boundPad = resolvePersonaBoundPadForAccountAction(archive, await listPadsForThisBot());
      const returnCallback = `acctplatform_threads_${id}`;
      ...
      await safeEditOrSend(bot, chatId, msgId, `🌱 *養號設定*\n\n人設：${archive.name}\n雲機：${boundPad.padName}\n平台：Threads\n留言人設：${commentPersona.name || archive.name}\n\n執行規則：\n- 每次滑動瀏覽 7-10 分鐘，每天最多 2 次\n- 每滑過約 2-3 篇，隨機挑選 1 篇點讚或留言\n- 不會每篇都互動；達到風險閾值會停止並提示人工介入\n\n請選擇互動策略：`, {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: "只滑動", callback_data: `warmup_engage_${platform}_${boundPad.padCode}_browse` }],
            [{ text: "滑動 + 隨機點讚", callback_data: `warmup_engage_${platform}_${boundPad.padCode}_like` }],
    ...
    if (data.startsWith("acctautoreply_") && !data.startsWith("acctautoreply_run_") && !data.startsWith("acctautoreply_days_")) {
      pendingThreadsAutoReplyDaysInputs.delete(chatId);
      pendingPersonaAccountActions.delete(chatId);
      const id = data.slice("acctautoreply_".length);
      const archive = await loadPersonaForThisBot(id);
      if (!archive) { sendMainMenu(chatId, msgId); return; }
      const boundPad = resolvePersonaBoundPadForAccountAction(archive, await listPadsForThisBot());
      const returnCallback = `acctplatform_threads_${id}`;
      ...
      await safeEditOrSend(bot, chatId, msgId, `💬 *Threads 自動回覆*\n\n人設：${archive.name}\n雲機：${boundPad.padName}\n\n執行規則：\n- 預設只查看 2 天內自己發布且有評論角標的推文。\n- 可調整查看天數，最多 7 天；沒有評論會直接報告退出。\n- 先過濾廣告、辱罵、釣魚、純表情和過短留言。\n- 只挑和人設或推文內容相關的留言回覆。\n- 回覆會使用自然口語，不做客服式套話。\n\n預設最多掃描 3 篇推文，最多回覆 3 條留言。`, {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: "✅ 開始（預設 2 天）", callback_data: `acctautoreply_run_d2_${id}` }],
            [{ text: "✍️ 輸入查看天數", callback_data: `acctautoreply_days_${id}` }],
            [{ text: "◀️ 返回 Threads 帳號", callback_data: returnCallback }],
          ],
        },
      });
    ...
    if (data.startsWith("acctautoreply_days_")) {
      pendingPersonaAccountActions.delete(chatId);
      const id = data.slice("acctautoreply_days_".length);
      const archive = await loadPersonaForThisBot(id);
      if (!archive) { sendMainMenu(chatId, msgId); return; }
      const boundPad = resolvePersonaBoundPadForAccountAction(archive, await listPadsForThisBot());
      const returnCallback = `acctplatform_threads_${id}`;
    ...
    if (data.startsWith("acctautoreply_run_")) {
      pendingPersonaAccountActions.delete(chatId);
      const runPayload = data.slice("acctautoreply_run_".length);
      const daysMatch = runPayload.match(/^d([1-7])_(.+)$/);
      const maxAgeDays = daysMatch ? Number(daysMatch[1]) : 2;
    ...
    if (data.startsWith("acctswitch_threads_") || data.startsWith("acctset_threads_")) {
      ...
      if (action === "login" && platform === "telegram") {
        ...
        if (!telegram?.phone) {
          await safeEditOrSend(bot, chatId, msgId, "尚未設定 Telegram 手機號，請先點「設定 Telegram 登入資料」。", {
            reply_markup: { inline_keyboard: buildPersonaPlatformAccountRows(id, "telegram", undefined, archive) },
          });
          return;
        }
    ...
    if (data.startsWith("genpost_branch_")) {
      pendingCustomPersonaPosts.delete(chatId);
      const archiveId = data.slice("genpost_branch_".length);
      const archive = await loadPersonaArchive(archiveId).catch(() => null);
      if (!archive) { sendMainMenu(chatId, msgId); return; }
    ...
    if (data === "toolr18_generate_posts") {
      ...
      const nextState = { ...pending, contentBranch: "r18" as const, contentTimeSlot: ensurePaidR18TimeSlot(), stage: "await_time_slot" as const };
      setPendingGeneratePost(chatId, nextState);
    ...
    if (data === "toolr18_group_image") {
      if (await sendPaidR18ImageWorkflowFromContext(bot, chatId, msgId, false)) return;
    ...
    if (data === "toolr18_group_video") {
      if (await sendPaidR18ImageWorkflowFromContext(bot, chatId, msgId, false, "entry")) return;
      await safeEditOrSend(bot, chatId, msgId, "R18 / 視頻生成", { reply_markup: { inline_keyboard: buildToolR18VideoKeyboard(chatId) } });
      return;
    }
    ...
    if (data === "toolr18_submit") {
      ...
      if (!pending) {
        await sendToolR18ContextOrMenu(bot, chatId, msgId);
        return;
      }
      try {
        ...
        await safeEditOrSend(bot, chatId, msgId, "R18 提交失敗：" + formatUserFacingError(error, "請檢查素材和參數後重試。"), {
          reply_markup: { inline_keyboard: buildToolR18SubmitFailureKeyboard(pending) },
        });
      }
      return;
    }
    ...
    if (data === "toolr18_clear") {
      const pending = pendingToolR18Tasks.get(chatId);
      pendingToolR18Tasks.delete(chatId);
      if (hasPaidR18ImageFlowContext(pending?.params) || pendingPaidR18ImageFlows.has(chatId)) {
        if (await sendPaidR18ImageWorkflowFromContext(bot, chatId, msgId)) return;
      }
      await sendToolR18Menu(bot, chatId, msgId);
      return;
    }
    ...
    if (data === "toolr18_prompt_submit") {
      ...
      try {
        ...
        pendingToolR18Tasks.set(chatId, pending);
        await safeEditOrSend(bot, chatId, msgId, "R18 提交失敗：" + formatUserFacingError(error, "請檢查素材和參數後重試。"), {
          reply_markup: { inline_keyboard: buildToolR18SubmitFailureKeyboard(pending) },
        });
      }
    ...
    if (data === "toolr18_t2i_toggle_qa") {
      const state = pendingToolR18Tasks.get(chatId);
      const params = { ...(state?.params || {}) };
      params.text_to_image_auto_qa_enabled = !toolR18TextToImageQaEnabled(params);
      pendingToolR18Tasks.set(chatId, {
        ...(state || { stage: "await_task_input", taskType: "text_to_image", taskLabel: "文生圖" } as PendingToolR18TaskState),
        stage: "await_task_input",
        taskType: "text_to_image",
        taskLabel: state?.taskLabel || "文生圖",
        params,
      });
      await sendToolR18TextToImageRatioStep(bot, chatId, msgId, params);
      await callback.answer(params.text_to_image_auto_qa_enabled ? "QA 審查已開啟" : "QA 審查已關閉");
      return;
    }
    ...
    if (data === "toolr18_t2i_prompt_free") {
      const state = pendingToolR18Tasks.get(chatId) || { stage: "await_task_input", taskType: "text_to_image", taskLabel: "文生圖" } as PendingToolR18TaskState;
      const requestText = "AI 自由發揮：根據已選比例、人設 LoRA、付費群圖片方向和目前配置，生成一張適合後續付費群推文的 R18 人物圖片。";
      const params = { ...(state.params || {}), tg_use_llm_prompt: true, tg_llm_prompt_enhanced: true, tg_latest_prompt_only: true };
      await sendToolR18PromptPreview(bot, chatId, { ...state, stage: "await_task_input", taskType: "text_to_image", taskLabel: state.taskLabel || "文生圖", text: requestText, originalRequest: requestText, params }, requestText);
      return;
    }
    ...
    if (data === "toolr18_i2v_prompt_free") {
      const state = pendingToolR18Tasks.get(chatId);
      const params = toolR18VideoI2vParams(state);
      if (!String(params.image_local_path || "").trim()) {
        await safeEditOrSend(bot, chatId, msgId, "請先上傳一張參考圖，再使用 AI 自由發揮生成圖生視頻提示詞。", {
          reply_markup: { inline_keyboard: buildToolR18VideoI2vImageKeyboard(params) },
        });
        return;
      }
      ...
      params.prompt_mode_selected = true;
      params.prompt_mode_label = "AI 自由發揮";
      params.tg_use_llm_prompt = true;
      params.tg_latest_prompt_only = true;
      const requestText = [
        "AI 自由發揮：根據已上傳的參考圖和圖生視頻規範生成完整視頻提示詞。",
        "把參考圖作為第一幀，保持人物、構圖、場景、光線、服裝與鏡頭比例一致。",
        "自行補齊合理的鏡頭運動、人物動作、表情變化、環境氛圍和轉場節奏。",
        "分辨率：" + String(params.resolution || "720p") + "；時長：" + String(params.duration_seconds || params.mulerouter_wan_i2v_duration || 2) + "秒。",
        params.audio_local_path ? "本次有音頻，動作節奏需要配合音頻。" : "本次無音頻，請以畫面動作節奏為主。",
      ].join(String.fromCharCode(10));
      const promptState: PendingToolR18TaskState = {
        ...(state || { stage: "await_task_input", taskType: "video_i2v", taskLabel: "圖生視頻" }),
        stage: "await_task_input",
        taskType: "video_i2v",
        taskLabel: state?.taskLabel || "圖生視頻",
        text: requestText,
        originalRequest: requestText,
        params,
      };
      await sendToolR18PromptPreview(bot, chatId, promptState, requestText);
      return;
    }
    ...
    if (data === "toolr18_imgedit_prompt_free") {
      const state = pendingToolR18Tasks.get(chatId) || { taskType: "get_nano_banana", taskLabel: "圖片編輯", params: {} } as PendingToolR18TaskState;
      const meta = toolR18ImageEditMeta(state.taskType);
      const requestText = "AI 自由發揮：根據已上傳的原圖/參考圖、付費群圖片方向和目前配置，生成一個合理的 R18 圖片編輯提示詞。";
      const params = { ...toolR18ImageEditParams(state), tg_use_llm_prompt: true, tg_latest_prompt_only: true };
      await sendToolR18PromptPreview(bot, chatId, { ...state, stage: "await_task_input", taskType: meta.taskType, taskLabel: meta.label, text: requestText, originalRequest: requestText, params }, requestText);
      return;
    }
    ...
    if (data === "toolr18_faceswap_prompt_free") {
      const state = pendingToolR18Tasks.get(chatId);
      const params = toolR18FaceSwapParams(state);
      params.tg_use_llm_prompt = true;
      params.tg_latest_prompt_only = true;
      const requestText = "AI 自由發揮：根據目標圖與人臉參考圖自然換臉，保持姿勢、服裝、光線和背景合理，生成適合付費群內容的最終換臉提示詞。";
      const promptState: PendingToolR18TaskState = { ...(state || { stage: "await_task_input", taskType: "face_swap", taskLabel: "人物換臉" }), stage: "await_task_input", taskType: "face_swap", taskLabel: state?.taskLabel || "人物換臉", text: requestText, originalRequest: requestText, params };
      await sendToolR18PromptPreview(bot, chatId, promptState, requestText);
      return;
    }
    ...
    if (data.startsWith("toolr18_task_")) {
      ...
      if (!item) { await sendToolR18ContextOrMenu(bot, chatId, msgId); return; }
      if (
        pendingPaidR18ImageFlows.has(chatId)
        && ["text_to_image", "single_image_edit", "get_nano_banana", "face_swap", "video_i2v"].includes(taskType)
      ) {
        await startPaidR18ImageWorkflow(bot, chatId, msgId, taskType);
        return;
      }
      if (taskType === "text_to_image") {
        const pending = pendingToolR18Tasks.get(chatId);
        await sendToolR18TextToImageRatioStep(bot, chatId, msgId, hasPaidR18ImageFlowContext(pending?.params) ? { ...(pending?.params || {}) } : {});
        return;
      }
      ...
      if (taskType === "r18_text_to_image_continue") {
        ...
        await safeEditOrSend(bot, chatId, msgId, ["R18 / 繼續生成圖片", "", "請發送新的生成要求。", "我會基於最近一次文生圖任務繼續生成，並保留上一輪的風格與場景連續性。"].join(String.fromCharCode(10)), {
          reply_markup: { inline_keyboard: [[toolR18ContextCancelButton(chatId, nextPending.params)]] },
        });
        return;
      }
      if (["r18_config", "r18_rerun_latest", "r18_text_to_image_reroll", "r18_image_edit_continue", "r18_image_edit_rerun", "r18_face_swap_upscale", "r18_face_swap_rerun"].includes(taskType)) {
        try {
          ...
          await safeEditOrSend(bot, chatId, msgId, "R18 指令處理失敗：" + formatUserFacingError(error, "請稍後重試。"), {
            reply_markup: { inline_keyboard: [[{ text: "最近任務", callback_data: "toolr18_tasks" }], [toolR18ContextBackButton(chatId, nextPending.params)]] },
          });
        }
        return;
      }
    ...
    if (data === "toolr18_status") {
      ...
      catch (error: any) { await safeEditOrSend(bot, chatId, msgId, "R18 狀態查詢失敗：" + formatUserFacingError(error, "請稍後重試。"), { reply_markup: { inline_keyboard: [[toolR18ContextBackButton(chatId)]] } }); }
      return;
    }

    if (data === "toolr18_tasks") {
      try { await sendToolR18Tasks(bot, chatId, msgId); }
      catch (error: any) { await safeEditOrSend(bot, chatId, msgId, "R18 最近任務查詢失敗：" + formatUserFacingError(error, "請稍後重試。"), { reply_markup: { inline_keyboard: [[toolR18ContextBackButton(chatId)]] } }); }
      return;
    }

    if (data.startsWith("toolr18_detail_")) {
      try { await sendToolR18TaskDetail(bot, chatId, data.slice("toolr18_detail_".length), msgId); }
      catch (error: any) { await safeEditOrSend(bot, chatId, msgId, "R18 \u4efb\u52d9\u8a73\u60c5\u67e5\u8a62\u5931\u6557\uff1a" + formatUserFacingError(error, "\u8acb\u7a0d\u5f8c\u91cd\u8a66\u3002"), { reply_markup: { inline_keyboard: [[{ text: "\u6700\u8fd1\u4efb\u52d9", callback_data: "toolr18_tasks" }, toolR18ContextBackButton(chatId)]] } }); }
      return;
    }
    ...
    if (data.startsWith("toolr18_resubmit_")) {
      try { await resubmitToolR18Task(bot, chatId, data.slice("toolr18_resubmit_".length), msgId); }
        ...
        await safeEditOrSend(bot, chatId, msgId, "R18 \u91cd\u65b0\u63d0\u4ea4\u5931\u6557\uff1a" + formatUserFacingError(error, "\u8acb\u78ba\u8a8d 4090 \u7db2\u95dc\u6062\u5fa9\u5f8c\u518d\u91cd\u8a66\u3002"), {
          reply_markup: { inline_keyboard: [[{ text: "\u91cd\u65b0\u63d0\u4ea4\u4efb\u52d9", callback_data: "toolr18_resubmit_" + failedTaskId }], [{ text: "\u6700\u8fd1\u4efb\u52d9", callback_data: "toolr18_tasks" }, toolR18ContextBackButton(chatId)]] },
        });
      }
      return;
    }
    ...
    if (data === "toolr18_cancel") {
      try {
        ...
        await safeEditOrSend(bot, chatId, msgId, "R18 強制停止：" + String(result?.message || (result?.cancelled ? "已提交取消。" : "沒有可取消任務。")), {
          reply_markup: { inline_keyboard: [[{ text: "查看狀態", callback_data: "toolr18_status" }], [toolR18ContextBackButton(chatId)]] },
        });
      } catch (error: any) {
        await safeEditOrSend(bot, chatId, msgId, "R18 強制停止失敗：" + formatUserFacingError(error, "請稍後重試。"), { reply_markup: { inline_keyboard: [[toolR18ContextBackButton(chatId)]] } });
      }
      return;
    }
    ...
    if (data === "genpost_toggle_qa") {
      ...
      await bot.answerCallbackQuery(query.id, { text: enabled ? "QA \u5BE9\u67E5\u5DF2\u958B\u555F" : "QA \u5BE9\u67E5\u5DF2\u95DC\u9589" }).catch(() => undefined);
      return;
    }

    const hotParsed = parseGeneratePostHotCallback(data);
    if (hotParsed) {
      await sendSentimentHotCandidatePicker({
        bot,
        chatId,
        messageId: msgId,
        archiveId: hotParsed.archiveId,
        contentBranch: hotParsed.contentBranch,
        refresh: true,
      });
      return;
    }

    if (data.startsWith("shrf_")) {
      const actionKey = data.slice("shrf_".length);
    ...
    if (
      ...
      && !data.startsWith("genpost_r18_")
      && !data.startsWith("genpost_mode_")
      && !data.startsWith("genpost_count_")
      && !data.startsWith("genpost_ratio_")
      && data !== "genpost_ratio_back"
      && data !== "genpost_imggrp_next"
      && data !== "genpost_imggrp_retry"
    ...
    if (data.startsWith("genpost_custom_")) {
      const parsedCustom = parseGeneratePostCustomCallback(data);
      const archiveId = parsedCustom?.archiveId || "";
      const archive = await loadPersonaArchive(archiveId).catch(() => null);
      if (!archive) { sendMainMenu(chatId, msgId); return; }
      const customContentBranch: GeneratePostContentBranch | undefined = parsedCustom?.contentBranch;
      deletePendingGeneratePost(chatId);
    ...
    if (data.startsWith("genpost_mode_") || data.startsWith("gpm_")) {
      ...
      const contentBranch: GeneratePostContentBranch | undefined = parsedMode?.contentBranch;
    ...
    if (data.startsWith("genmem_page_")) {
      const pending = pendingGeneratePosts.get(chatId);
      ...
      const requestedPage = Number(data.slice("genmem_page_".length));
      const pageInfo = generateMemoryPageInfo(pending);
      const nextPage = Math.min(Math.max(0, Math.floor(Number.isFinite(requestedPage) ? requestedPage : pageInfo.page)), pageInfo.totalPages - 1);
      const nextState = { ...pending, memoryPage: nextPage };
      setPendingGeneratePost(chatId, nextState);
    ...
    if (data.startsWith("genmem_toggle_")) {
      ...
      const nextState = { ...pending, selectedMemoryEntryIds: [...selected], memoryPage: generateMemoryPageInfo(pending).page };
      setPendingGeneratePost(chatId, nextState);
      await safeEditOrSend(bot, chatId, msgId, buildGenerateMemorySelectionText(nextState), {
        reply_markup: buildGenerateMemorySelectionKeyboard(nextState),
      });
    ...
    if (data === "genmem_time_morning" || data === "genmem_time_night" || data === "genmem_time_back") {
      ...
      const nextState = {
        ...
        stage: pending.contentBranch === "r18" ? "await_time_slot" as const : "await_count" as const,
      };
      setPendingGeneratePost(chatId, nextState);
      if (nextState.contentBranch === "r18") {
        await sendPaidR18EntryPicker(bot, chatId, msgId, nextState);
    ...
    if (data === "paidr18_entry" || data === "paidr18_back_time_slot") {
      const flow = pendingPaidR18ImageFlows.get(chatId);
      if (!flow) {
        await safeEditOrSend(bot, chatId, msgId, "付費群圖片流程已失效，請重新選擇新建推文。", {
          reply_markup: { inline_keyboard: [[{ text: "◀️ 返回人設列表", callback_data: "list_personas" }]] },
        });
        return;
      }
      const entryState: PendingGeneratePostState = {
        archiveId: flow.archiveId,
        archiveName: flow.archiveName,
        count: 1,
        textOnly: false,
      ...
      setPendingGeneratePost(chatId, entryState);
      await sendPaidR18EntryPicker(bot, chatId, msgId, entryState);
      return;
    }

    if (data === "paidr18_group_image" || data === "paidr18_group_video") {
      const flow = pendingPaidR18ImageFlows.get(chatId);
      if (!flow) {
        await safeEditOrSend(bot, chatId, msgId, "付費群圖片流程已失效，請重新從「我的人設 → 新建推文 → 付費群內容」進入。", {
          reply_markup: { inline_keyboard: [[{ text: "👤 我的人設", callback_data: "list_personas" }]] },
        });
        return;
      }
      const state: PendingGeneratePostState = {
        archiveId: flow.archiveId,
        archiveName: flow.archiveName,
        count: 1,
        textOnly: false,
      ...
      if (data === "paidr18_group_image") {
        await sendPaidR18ImageWorkflowPicker(bot, chatId, msgId, state);
      } else {
        await startPaidR18ImageWorkflow(bot, chatId, msgId, "video_i2v");
      }
      return;
    }

    if (data === "paidr18_back_workflow") {
      if (await sendPaidR18ImageWorkflowFromContext(bot, chatId, msgId)) return;
      await safeEditOrSend(bot, chatId, msgId, "付費群圖片流程已失效，請重新從「我的人設 → 新建推文 → 付費群內容」進入。", {
        reply_markup: { inline_keyboard: [[{ text: "👤 我的人設", callback_data: "list_personas" }]] },
      });
    ...
    if (data.startsWith("paidr18_task_")) {
      const taskType = data.slice("paidr18_task_".length) as ToolR18TaskType;
      if (!["text_to_image", "single_image_edit", "get_nano_banana", "face_swap", "video_i2v"].includes(taskType)) {
        await safeEditOrSend(bot, chatId, msgId, "暫不支援這個付費群圖片工作流。", {
          reply_markup: { inline_keyboard: buildPaidR18ImageWorkflowKeyboard() },
        });
        return;
      }
    ...
    if (data === "genpost_imggrp_next" || data === "genpost_imggrp_retry") {
      await generateCurrentFreeGeneratedPostImageGroup(bot, chatId);
      return;
    }

    if (data === "paidr18_imggrp_next") {
      await generateCurrentPaidR18ImageFirstGroup(bot, chatId);
      return;
    }

    if (data === "genpost_ratio_back" || data.startsWith("genpost_ratio_")) {
      const pending = pendingGeneratePosts.get(chatId);
      if (!pending || !["await_ratio", "await_prompt"].includes(pending.stage)) {
        await safeEditOrSend(bot, chatId, msgId, "生成配置已失效，請重新選擇新建推文。", {
          reply_markup: { inline_keyboard: [[{ text: "◀️ 返回人設列表", callback_data: "list_personas" }]] },
    ...
    if (data === "genpost_count_back") {
      const pending = pendingGeneratePosts.get(chatId);
      if (!pending || !["await_count", "await_ratio"].includes(pending.stage)) {
        await safeEditOrSend(bot, chatId, msgId, "生成配置已失效，請重新選擇新建推文。", {
          reply_markup: { inline_keyboard: [[{ text: "◀️ 返回人設列表", callback_data: "list_personas" }]] },
        });
        return;
      }
      if (pending.stage === "await_ratio") {
        const countState: PendingGeneratePostState = {
          ...pending,
          imageRatioId: undefined,
          imageAspectRatio: undefined,
          imageRatioLabel: undefined,
    ...
    if (data.startsWith("history_") && !data.startsWith("history_branch_")) {
      const parsedHistory = parsePersonaScopedCallback(data, "history_");
      const id = parsedHistory?.archiveId || "";
      const archive = await loadPersonaForThisBot(id);
      if (!archive) { sendMainMenu(chatId, msgId); return; }
      const history = filterByTelegramGroupContentType(
        ...
        parsedHistory?.groupContentType,
      );
      if (!history.length) {
      ...
      const pageSize = STORED_POSTS_PAGE_SIZE;
      const totalPages = Math.max(1, Math.ceil(history.length / pageSize));
      const safePage = Math.min(Math.max(0, parsedHistory?.page || 0), totalPages - 1);
      const start = safePage * pageSize;
    ...
    if (historyRequeueKey) {
      ...
      if (!updated) {
        await safeEditOrSend(bot, chatId, msgId, "\u91CD\u5165\u968A\u5931\u6557\uFF0C\u672A\u627E\u5230\u5C0D\u61C9\u767C\u5E03\u6B77\u53F2\u3002", {
          reply_markup: { inline_keyboard: [[{ text: "\u25C0\uFE0F \u8FD4\u56DE\u6B77\u53F2", callback_data: buildPersonaHistoryCallback(action.archiveId, action.groupContentType) }]] },
        });
        return;
      }
      await safeEditOrSend(bot, chatId, msgId, `\uD83D\uDD01 \u5DF2\u5C07\u767C\u5E03\u6B77\u53F2\u91CD\u65B0\u52A0\u5165\u5F85\u767C\u5E03\u5217\u8868\uFF0C\u76EE\u524D\u5F85\u767C\u5E03 ${updated.posts.length} \u7BC7\u3002`, {
        reply_markup: {
          inline_keyboard: [
            [{ text: "\uD83D\uDCDD \u67E5\u770B\u5F85\u767C\u4F48\u63A8\u6587", callback_data: buildStoredPostsPageCallback(action.archiveId, 0, action.groupContentType) }],
            [{ text: "\u25C0\uFE0F \u8FD4\u56DE\u6B77\u53F2", callback_data: buildPersonaHistoryCallback(action.archiveId, action.groupContentType) }],
          ],
        },
      });
      return;
    ...
    if (data.startsWith("posts_branch_") || data.startsWith("history_branch_") || data.startsWith("pub_branch_")) {
      const target: PersonaContentTypeTarget = data.startsWith("history_branch_")
        ? "history"
        : data.startsWith("pub_branch_")
          ? "publish"
          : "posts";
    ...
    if (contentTypeSelection?.target === "posts") {
      ...
      data = buildPersonaHistoryCallback(contentTypeSelection.archiveId, contentTypeSelection.groupContentType);
    ...
    if (data.startsWith("posts_")) {
      ...
      pendingPostSelections.set(chatId, {
        ...
        groupContentType: parsed?.groupContentType,
      });
    ...
    if (bulkPublishStart || bulkDeleteStart) {
      ...
      const state: PendingBulkPostAction = { archiveId: parsed.archiveId, mode, selectedPostIds: [], page: parsed.page, groupContentType: parsed.groupContentType };
      pendingBulkPostActions.set(chatId, state);
      const view = buildBulkStoredPostsSelectView({ archiveId: parsed.archiveId, posts: archive.posts, mode, selectedPostIds: [], page: parsed.page, groupContentType: parsed.groupContentType });
    ...
    if (
      ...
      const selected = new Set(state.selectedPostIds.filter((id) => scopedPosts.some((post) => post.id === id)));
      const visiblePage = Math.max(0, Math.min(state.page || 0, Math.max(0, Math.ceil(scopedPosts.length / STORED_POSTS_PAGE_SIZE) - 1)));
      const pageStart = visiblePage * STORED_POSTS_PAGE_SIZE;
      const pagePostIds = scopedPosts.slice(pageStart, pageStart + STORED_POSTS_PAGE_SIZE).map((post) => post.id);
      ...
      const renderSelection = async (nextState: PendingBulkPostAction) => {
        ...
        const view = buildBulkStoredPostsSelectView({
          ...
          page: nextState.page,
          groupContentType: nextState.groupContentType,
        });
        await safeEditOrSend(bot, chatId, msgId, view.text, {
          reply_markup: { inline_keyboard: view.keyboard },
        });
      ...
      if (data.startsWith("btog_")) {
        ...
        const postId = scopedPosts[postIndex]?.id;
        if (postId) {
    ...
    if (postImageCandidateSelectKey) {
      ...
      try {
        if (action.paidR18Context) {
          await telegramBestEffort("paidR18ImageCandidate.writeStart", bot.sendMessage(chatId, "\u23F3 \u5DF2\u9078\u4E2D\u9019\u5F35 R18 \u5716\u7247\uFF0C\u6B63\u5728\u6839\u64DA\u5716\u7247\u5167\u5BB9\u751F\u6210\u4ED8\u8CBB\u7FA4\u6587\u6848..."), 15_000);
          const result = await executePaidR18PostFromSelectedImage({
            bot,
            chatId,
            archiveId: action.archiveId,
        ...
        const flow = pendingGeneratedPostImageGroupFlows.get(chatId);
        const currentPost = flow?.posts?.[flow.currentIndex];
        const matchesCurrentFlow = Boolean(
          flow
    ...
    if (data.startsWith("pubpost_") || data.startsWith("pp_") || data === "post_action") {
      ...
      if (!post) {
        await safeEditOrSend(bot, chatId, msgId, "没有找到这篇推文。", {
          reply_markup: { inline_keyboard: [[{ text: "◀️ 返回推文列表", callback_data: buildStoredPostsPageCallback(archiveId, 0, groupContentType) }]] },
        });
        return;
      }
      pendingPostActions.set(chatId, { archiveId, postId, groupContentType });
    ...
    if (data.startsWith("bindtg_")) {
      const parts = data.slice("bindtg_".length).split("_");
      const groupContentType = parts[0] as TelegramGroupContentType;
      const id = parts.slice(1).join("_");
      if (!isTelegramGroupContentType(groupContentType) || !id) {
        await safeEditOrSend(bot, chatId, msgId, "❌ 群組設定參數無效。", {
          reply_markup: { inline_keyboard: [[{ text: "🏠 主選單", callback_data: "back_main" }]] },
        });
        return;
      }
    ...
    if (data.startsWith("pub_")) {
      const parsedPublish = parsePersonaScopedCallback(data, "pub_");
      const id = parsedPublish?.archiveId || "";
      const archive = await loadPersonaArchive(id).catch(() => null);
      if (!archive) {
        sendMainMenu(chatId, msgId);
        return;
      }
      const scopedPosts = filterByTelegramGroupContentType(archive.posts || [], parsedPublish?.groupContentType);
      const branchTitle = parsedPublish?.groupContentType === "paid" ? "付費內容" : parsedPublish?.groupContentType === "free" ? "免費內容" : "";
      pendingCustomPublishes.set(chatId, {
        ...(pendingCustomPublishes.get(chatId) || {}),
        source: "persona_detail",
        sourceArchiveId: id,
    ...
    if (data === "sp" || data.startsWith("sp_") || data.startsWith("stored_publish_")) {
      ...
      const groupContentType = prevManual?.archiveId === id ? prevManual.groupContentType : undefined;
      const scopedPosts = archive ? filterByTelegramGroupContentType(archive.posts || [], groupContentType) : [];
      if (!archive || scopedPosts.length === 0) {
        await safeEditOrSend(bot, chatId, msgId, "暂无待发布推文。发送「帮我生成3篇推文」来生成。", {
          reply_markup: { inline_keyboard: id ? [[{ text: "◀️ 返回发布方式", callback_data: buildPersonaPublishCallback(id, groupContentType) }]] : [[{ text: "◀️ 返回主菜单", callback_data: "fresh_main_menu" }]] },
        });
        return;
      }
      pendingManualPublishes.set(chatId, {
      ...
      const branchTitle = groupContentType === "paid" ? "付費內容" : groupContentType === "free" ? "免費內容" : "";
      await safeEditOrSend(bot, chatId, msgId, `🚀 *从存储推文发布*\n\n人設：${archive.name}\n${branchTitle ? `內容類型：${branchTitle}\n` : ""}目前待發佈：${scopedPosts.length} 篇\n\n请先选择发布平台：`, {
        parse_mode: "Markdown",
        reply_markup: {
          inline_keyboard: [
            ...buildAllowedPublishPlatformRows("mp_"),
            [{ text: "◀️ 返回发布方式", callback_data: buildPersonaPublishCallback(id, groupContentType) }],
          ],
        },
      });
      return;
    ...
    if (data.startsWith("manualpub_platform_") || parseShortManualPlatform(data)) {
      ...
      const groupContentType = prevManual?.archiveId === archiveId ? prevManual.groupContentType : undefined;
      const scopedPosts = archive ? filterByTelegramGroupContentType(archive.posts || [], groupContentType) : [];
      if (!archive || scopedPosts.length === 0) {
        await safeEditOrSend(bot, chatId, msgId, "当前没有可人工发布的推文。", {
          reply_markup: { inline_keyboard: [[{ text: "◀️ 返回", callback_data: archiveId ? buildPersonaPublishCallback(archiveId, groupContentType) : "fresh_main_menu" }]] },
        });
        return;
      }
      pendingManualPublishes.set(chatId, {
    ...
    if (data.startsWith("mpage_")) {
      ...
      const archive = archiveId ? await loadPersonaForThisBot(archiveId) : null;
      const scopedPosts = archive ? filterByTelegramGroupContentType(archive.posts || [], prevManual?.groupContentType) : [];
      if (!archive || scopedPosts.length === 0 || !platform) {
        await safeEditOrSend(bot, chatId, msgId, "当前没有可人工发布的推文。", {
          reply_markup: { inline_keyboard: [[{ text: "◀️ 返回", callback_data: archiveId ? buildPersonaPublishCallback(archiveId, prevManual?.groupContentType) : "fresh_main_menu" }]] },
        });
        return;
      }
      pendingManualPublishes.set(chatId, {
    ...
    if (data.startsWith("mpick_")) {
      ...
      const scopedPosts = archive ? filterByTelegramGroupContentType(archive.posts || [], prevManual?.groupContentType) : [];
      if (!archive || scopedPosts.length === 0 || !isAllowedPublishPlatformValue(platform) || startIndex >= scopedPosts.length) {
        await safeEditOrSend(bot, chatId, msgId, "当前没有可人工发布的推文。", {
          reply_markup: { inline_keyboard: [[{ text: "◀️ 返回", callback_data: archiveId ? buildPersonaPublishCallback(archiveId, prevManual?.groupContentType) : "fresh_main_menu" }]] },
        });
        return;
    ...
    if (data.startsWith("manualpub_start_") || data.startsWith("ms_")) {
      ...
      const remaining = Math.max(0, scopedPosts.length - startIndex);
      pendingManualPublishes.set(chatId, {
        archiveId,
        archiveName: archive.name,
        platform,
    ...
    if (data.startsWith(MANUAL_CONFIRM_PREFIX) || data.startsWith(MANUAL_CONFIRM_COMPACT_PREFIX) || data === "mconfirm") {
      ...
      try {
        ...
        if (await sendCloudAccountStateNotice(
          ...
          [[{ text: "🔄 重试", callback_data: retryCallback }], [{ text: "◀️ 返回", callback_data: buildPersonaPublishCallback(archiveId, groupContentType) }]],
        )) {
          return;
        }
        await sendManualInterventionFailure(bot, chatId, error, {
    ...
    if (data.startsWith("custom_publish_platform_")) {
      ...
      pendingCustomPublishes.set(chatId, nextState);
      const telegramTarget = platform === "telegram"
        ? resolveTelegramTargetForGroupContent(await loadPersonaForThisBot(nextState.archiveId || "").catch(() => null), nextState.groupContentType)
        : null;
    ...
    if (data === "custom_publish_clear_content") {
      const prev = pendingCustomPublishes.get(chatId);
      if (!prev) {
        sendMainMenu(chatId, msgId);
        return;
    ...
    if (queueView) {
      ...
      keyboard.push(...buildQueuePaginationRows({ ...queueView, page: pageInfo.page, totalPages: pageInfo.totalPages }));
  ...
  const messageHandler = async (msg: TelegramBot.Message) => {
    ...
    if (pendingAutoReplyDays) {
      if (Date.now() - pendingAutoReplyDays.createdAt > 10 * 60 * 1000) {
        pendingThreadsAutoReplyDaysInputs.delete(chatId);
        await bot.sendMessage(chatId, "❌ 自動回覆天數輸入已逾時，請重新進入 Threads 自動回覆。");
        return;
      }
      ...
      pendingThreadsAutoReplyDaysInputs.delete(chatId);
      if (pendingAutoReplyDays.type === "pad" && pendingAutoReplyDays.padCode) {
        await runPadThreadsAutoReplyFromTelegram(bot, chatId, pendingAutoReplyDays.padCode, days);
        return;
      }
    ...
    if (pendingToolR18) {
      if (pendingToolR18.stage === "await_paid_post_count") {
        ...
        try {
          await startPaidR18ImageFirstFlowFromToolTask(bot, chatId, pendingToolR18, count);
        } catch (error: any) {
          await bot.sendMessage(chatId, "❌ 付費群候選圖流程啟動失敗：" + formatUserFacingError(error, "請返回重新確認提示詞。"), {
            reply_markup: { inline_keyboard: [[toolR18ContextBackButton(chatId, pendingToolR18.params)]] },
          });
        }
        return;
      }
      ...
      try {
        ...
        const errorText = formatUserFacingError(error, "請檢查素材和參數後重試。");
        const title = /下載 Telegram 素材失敗|Telegram 文件下載失敗|素材/i.test(errorText)
          ? "R18 素材處理失敗："
          : "R18 提交失敗：";
        await bot.editMessageText(title + errorText, {
          chat_id: chatId,
    ...
    if (customPersonaPost?.stage === "await_input") {
      await handlePendingCustomPersonaPostInput(
        ...
        text,
        media,
        photo ? "photo" : video ? "video" : documentMedia ? "document" : undefined,
      );
      return;
    }
    ...
    if (generatePostState) {
      ...
      if (generatePostState.stage === "await_count") {
        ...
        const nextState: PendingGeneratePostState = {
          ...generatePostState,
          count,
          stage: shouldSelectFreeGeneratedPostImageRatio(generatePostState) ? "await_ratio" : "await_prompt",
        };
        setPendingGeneratePost(chatId, nextState);
        if (shouldSelectFreeGeneratedPostImageRatio(nextState)) {
          await sendFreeGeneratedPostImageRatioPicker(bot, chatId, undefined, nextState);
        } else {
          await telegramBestEffort("generatePost.awaitPrompt", sendGeneratePostPromptInputStep(bot, chatId, undefined, nextState));
        }
        return;
      }
      ...
      if (generatePostState.stage === "await_prompt") {
        ...
        await telegramBestEffort("generatePost.awaitWordCount", bot.sendMessage(chatId, `✅ 已收到提示詞：${text}\n\n⭐ 請輸入每篇推文的目標字數 ⭐\n只需要發送數字即可。\n\n例如：120`));
        return;
      }
      ...
      await executeGeneratePostsFromTelegram({
        ...
        imageAspectRatio: generatePostState.imageAspectRatio,
        imageWidth: generatePostState.imageWidth,
        imageHeight: generatePostState.imageHeight,
        imageRatioLabel: generatePostState.imageRatioLabel,
      });
      return;
    }
    ...
    if (customPublish?.stage === "ready_to_publish") {
      ...
      if (nextState.publishWithGeneratedImage) {
        if (media?.file_id) {
          await bot.sendMessage(chatId, "⚠️ 当前是「根据文字生成配图再发布」阶段，只接收純文字文案。\n\n你刚发送的是图片/视频，不会被使用。请发送要发布的文字，或返回发布方式改用直发。", {
            reply_markup: { inline_keyboard: [[customPublishBackToMethodButton(nextState)], [customPublishResetButton]] },
          });
          return;
        }
        if (!nextState.text) {
          await bot.sendMessage(chatId, "📝 目前步驟：1/2 發送文字文案\n\n請發送純文字推文內容，我會根據文字生成配圖，下一步再讓你確認發布。", {
            reply_markup: { inline_keyboard: [[customPublishBackToMethodButton(nextState)]] },
          });
          return;
        }
      ...
      if (nextState.fileId) {
        await bot.sendMessage(chatId, `🖼 已收到媒體\n\n${customPublishContentSummary(nextState)}\n\n目前步驟：等待配套文字\n請繼續發送要和這個媒體一起發布的文字。`, {
          reply_markup: { inline_keyboard: [[customPublishResetButton], [customPublishBackToMethodButton(nextState)]] },
        });
        return;
      }

      await bot.sendMessage(chatId, "📝 目前步驟：1/2 發送內容\n\n請發送文字，或發送圖片/視頻並在 caption 裡寫文案。", {
        reply_markup: { inline_keyboard: [[customPublishBackToMethodButton(nextState)]] },
      });
      return;
    }
    ...
    if (profileState && text && profileState.stage !== "await_avatar") {
      const profileReturnCallback = profileState.returnCallback || `pad_detail_${profileState.padCode}`;
      const operation =
